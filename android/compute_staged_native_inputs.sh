...
emit_output() {
    local output_path="$1"
    local gmp_source_dir="$2"
    local c47_fingerprint="$3"
    local decnumber_fingerprint="$4"
    local generated_fingerprint="$5"
    local gmp_fingerprint="$6"
    local combined_fingerprint="$7"
    local temp_output=""
