...
void fnPause(uint16_t dur) {
  ...
  if (duration ==
      99) { // 99 signifies infinity, which is 2 hours on battery, and
            // (2^31-1)/10 seconds = 59652 hours on USB. Note that this is
            // determined at the start of pause, not changing during timing
  ...
  lcd_refresh();
  for (int32_t i = 0;
       i < duration && (programRunStop == PGM_PAUSED ||
    ...
    if (key > 0) {
      if ((key == 36 || key == 33) &&
          previousProgramRunStop == PGM_RUNNING) { // JM R/S or EXIT
  ...
  for (int32_t i = 0;
    ...
    // Yield and sleep 100ms UNLOCKED so UI thread can draw/process keys
  ...
  while (gTime <= duration &&
    ...
    if (gTime == i) { // arrive here every 100ms, do nothing, just increment the
                      // coounter to trap the next 100ms
      i++;
      if (previousProgramRunStop != PGM_RUNNING) {
        screenUpdatingMode &= ~SCRUPD_MANUAL_STATUSBAR;
        refreshScreen(12);
...
#pragma GCC diagnostic ignored "-Wimplicit-fallthrough"
  ...
  default: {
    ...
    sprintf(errorMessage, "cannot use %s for the parameter of CASE",
...
void fnKeyType(uint16_t regist) {
  ...
  switch (keyCode) {
  case 82:
    uInt32ToLongInteger(0u, kt);
    break;
  case 72:
    uInt32ToLongInteger(1u, kt);
    break;
  case 73:
    uInt32ToLongInteger(2u, kt);
    break;
  case 74:
    uInt32ToLongInteger(3u, kt);
    break;
  case 62:
    uInt32ToLongInteger(4u, kt);
    break;
  case 63:
    uInt32ToLongInteger(5u, kt);
    break;
  case 64:
    uInt32ToLongInteger(6u, kt);
    break;
  case 52:
    uInt32ToLongInteger(7u, kt);
    break;
  case 53:
    uInt32ToLongInteger(8u, kt);
    break;
  case 54:
    uInt32ToLongInteger(9u, kt);
    break;

  case 43:
  case 44:
  case 83:
    uInt32ToLongInteger(10u, kt);
    break;

  case 35:
  case 36:
    uInt32ToLongInteger(11u, kt);
    break;
