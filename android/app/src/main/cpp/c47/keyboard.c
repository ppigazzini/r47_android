...
TO_QSPI static const char bugScreenNonexistentMenu[] =
    "In function determineFunctionKeyItem: nonexistent menu specified!";
TO_QSPI static const char bugScreenItemNotDetermined[] =
    "In function determineItem: item was not determined!";

static void executeFunction(const char *data, int16_t item_);

int16_t
determineFunctionKeyItem_C47(const char *data, bool_t shiftF,
                             bool_t shiftG) { // Added itemshift param JM
  ...
  printf(">>>>Z 0090 determineFunctionKeyItem_C47(fn=%d):    data=|%s| "
         "data[0]=%d item=%d itemShift=%d (Global) FN_key_pressed=%d\n",
         fn, data, data[0], item, itemShift, FN_key_pressed);
#endif // VERBOSEKEYS
#if defined(PC_BUILD)
  char tmp[200];
  sprintf(tmp,
          "^^^^determineFunctionKeyItem_C47(fn=%d): itemShift=%d menuId=%d "
          "menuItem=%d",
...
#pragma GCC diagnostic ignored "-Wimplicit-fallthrough"
...
#if defined(VERBOSEKEYS)
  printf(">>>>Z 0090a determineFunctionKeyItem       "
         "-softmenu[menuId].menuItem=%i\n",
...
#if defined(VERBOSEKEYS)
    printf(">>>>Z 0091   case MNU_MyAlpha             data=|%s| data[0]=%d "
           "item=%d itemShift=%d (Global) FN_key_pressed=%d\n",
           data, data[0], item, itemShift, FN_key_pressed);
    printf(">>>>  0092     dynamicMenuItem=%d\n", dynamicMenuItem);
    printf(">>>>  0093     firstItem=%d itemShift=%d fn=%d", firstItem,
           itemShift, fn);
#endif // VERBOSEKEYS
    break;
  }
  ...
  case MNU_PROG: {
    ...
    if (tam.function == ITM_GTOP) {
      ...
      item =
          (dynamicMenuItem >= dynamicSoftmenu[menuId].numItems ? ITM_NOP
                                                               : MNU_DYNAMIC);
    }
    break;
  }
  ...
  case MNU_MVAR: {
    ...
    // These items below are all in the primary function key row, and should not
    // be decoded from actual operating modes, but will be better to be decoded
    // from the visible currentmenu,
    //   It cannot be changed to menu lookups, because when the MVAR is open, we
    //   do not know anymore which menu or function actually opened the MVAR,
    //   hence the method of decoding from operating modes. This is not ideal.
    //   It probably would still work better if we check which menu called the
    //   MVAR, i.e. the menu in currentmenu() == MVAR && menu(1) == whatever.
    ...
    // Solver
      ...
      item = -MNU_Solver_TOOL;
               ...
               (currentSolverStatus & SOLVER_STATUS_INTERACTIVE) &&
               *getNthString(
                   dynamicSoftmenu[softmenuStack[0].softmenuId].menuContent,
                   dynamicMenuItem) == 0) {
      item = ITM_NOP;
    }
  ...
  case MNU_ALLVARS: {
    dynamicMenuItem = firstItem + itemShift + fn;
    item = (dynamicMenuItem >= dynamicSoftmenu[menuId].numItems ? ITM_NOP
            : (tam.mode == TM_DELITM)                           ? MNU_DYNAMIC
                                                                : ITM_RCL);
    break;
  }
    ...
    if (dynamicMenuItem < dynamicSoftmenu[menuId].numItems) {
...
#if defined(VERBOSEKEYS)
        printf(">>>>Z 0093b determineFunctionKeyItem  case MNU_MENUS:     i=%u "
               "softmenu[i].menuItem=%i name:=%s\n",
               i, softmenu[i].menuItem,
               indexOfItems[-softmenu[i].menuItem].itemCatalogName);
#endif // VERBOSEKEYS
        if (compareString(
                (char *)getNthString(dynamicSoftmenu[menuId].menuContent,
                                     dynamicMenuItem),
                indexOfItems[-softmenu[i].menuItem].itemCatalogName,
                CMP_NAME) == 0) {
...
#if defined(VERBOSEKEYS)
            printf(
                ">>>>Z 0093c determineFunctionKeyItem  item = %i:   name:=%s\n",
      ...
      for (uint32_t i = 0; i < numberOfUserMenus; ++i) {
        if (compareString(
                (char *)getNthString(dynamicSoftmenu[menuId].menuContent,
                                     dynamicMenuItem),
                userMenus[i].menuName, CMP_NAME) == 0) {
          if (tam.mode == TM_DELITM) {
            item = MNU_DYNAMIC;
            tam.value = i;
          } else {
  ...
  default: {
    sm = &softmenu[menuId];
    row = min(3, (sm->numItems + modulo(firstItem - sm->numItems, 6)) / 6 -
                     firstItem / 6) -
          1;
    if (itemShift / 6 <= row && firstItem + itemShift + fn < sm->numItems) {
      ...
      if (softmenu[menuId].menuItem == -MNU_ALPHA && calcMode == CM_PEM &&
          item == ITM_ASSIGN) {
        item = ITM_NULL;
      }
    }
...
#if defined(VERBOSEKEYS)
  printf(">>>>Z 0094B    calcMode == %u  data=|%s| data[0]=%d item=%d "
         "itemShift=%d (Global) FN_key_pressed=%d\n",
...
#pragma GCC diagnostic pop
  if (calcMode == CM_ASSIGN && item != ITM_NOP && item != ITM_NULL) {
    ...
    case MNU_PROGS: {
      ...
      printf("0096a PROG or PROGS: registerno:%s\n",
             (char *)getNthString(dynamicSoftmenu[menuId].menuContent,
                                  dynamicMenuItem));
      printf("0096b %d %d %d\n",
             findNamedLabel((char *)getNthString(
                 dynamicSoftmenu[menuId].menuContent, dynamicMenuItem)),
             -FIRST_LABEL, +ASSIGN_LABELS);
#endif // VERBOSEKEYS
      return findNamedLabel((char *)getNthString(
                 dynamicSoftmenu[menuId].menuContent, dynamicMenuItem)) -
             FIRST_LABEL + ASSIGN_LABELS;
    }
    ...
    {
      return findNamedVariable((char *)getNthString(
                 dynamicSoftmenu[menuId].menuContent, dynamicMenuItem)) -
             FIRST_NAMED_VARIABLE + ASSIGN_NAMED_VARIABLES;
    }
    case MNU_MENUS: {
      if (item == -MNU_DYNAMIC) {
...
void btnFnClicked(void *unused, void *data) { executeFunction(data, 0); }
#endif // DMCP_BUILD

#if defined(PC_BUILD)
void btnFnClickedP(GtkWidget *notUsed,
                   gpointer data) { // JM Added this portion to be able to go to
                                    // NOP on emulator
...
void btnFnClickedR(GtkWidget *notUsed,
                   gpointer data) { // JM Added this portion to be able to go to
                                    // NOP on emulator
  GdkEvent mouseButton;
  mouseButton.button.button = 1;
...
//   The previous system closes a normal menu if the CAT main menu happens to be
//   in one of the older menu slots in the menu stack
//     and that happens if you call CAT and then say CLK or any other menu and
//     runn a command from there. Then CAT is present and the menu closed after
//     the command executed.

TO_QSPI const int16_t CatalogMenus[] = {
    //          MNU_CATALOG,  //option to include if we need to close the actual
    //          CAT menu too, i.e. jump back to before CAT (like 42S does
    MNU_ALPHA_OMEGA, MNU_ALPHAMISC, MNU_ALPHA,
    // CAT MENU
    MNU_FCNS, MNU_CONST, MNU_CHARS, MNU_PROGS, MNU_VARS, MNU_MENUS,
    // VARS MENU
    MNU_CONFIGS, MNU_MATRS, MNU_DATES, MNU_TIMES, MNU_SINTS, MNU_STRINGS,
    MNU_NUMBRS, MNU_CPXS, MNU_REALS, MNU_ANGLES, MNU_LINTS, MNU_ALLVARS};
...
static void closeAllCatalogMenus(void) {
  ...
  for (uint_fast16_t i = 0; i < nbrOfElements(CatalogMenus); i++) {
    if (menu == CatalogMenus[i]) {
      popSoftmenu();
      //         closeAllCatalogMenus(); //Option to recurse and close more than
      //         one menu level until all the CAT related menus are out
      break;
    }
  }
}
...
#define lowercaseselected                                                      \
  (bool_t)((alphaCase == AC_LOWER && !lastshiftF) ||                           \
           (alphaCase == AC_UPPER &&                                           \
            lastshiftF /*&& !numLock*/)) // //JM remove last !numlock if you
                                         // want the shift, during numlock, to
                                         // produce lower case
...
void processAimInput(int16_t item) {
  ...
  if (keyReplacements(item, &item1, getSystemFlag(FLAG_NUMLOCK), lastshiftF,
                      lastshiftG) > 0) { // JMvv
    if (item1 > 0) {
  ...
  else if (item == ITM_COLON || item == ITM_COMMA ||
           item == ITM_QUESTION_MARK || item == ITM_SPACE ||
           item == ITM_UNDERSCORE) { // JM vv DIRECT LETTERS
  ...
  showHideAlphaMode();
#if defined(PC_BUILD)
  sprintf(tmp, "^^^^processAimInput:AIM:end %d, processed %d", item,
          keyActionProcessed);
  jm_show_comment(tmp);
#if defined(PAIMDEBUG)
  printf("%s, |%s|\n", tmp, aimBuffer);
...
void btnFnPressed(GtkWidget *notUsed, GdkEvent *event, gpointer data) {
...
#if defined(VERBOSEKEYS)
  printf(">>>>Z 0010 btnFnPressed SET FN_key_pressed            ; data=|%s| "
         "data[0]=%d shiftF=%d shiftG=%d\n",
  ...
  FN_key_pressed =
      *((char *)data) - '0' + 37; // to render 38-43, as per original keypress
  ...
  // Exception, to activate the primary functions of the timer menu, without
  // allowing longpresses and double presses, in order to have quicker
  // activation
  if (!shiftF && /*!shiftG &&*/ currentMenu() ==
                     -MNU_TIMERF) { // do not check for g, to enable the
                                    // g-line fast response on press when
                                    // keyboard shortcuts help line is pressed
    const int16_t *softkeyItem =
        softmenu[softmenuStack[0].softmenuId].softkeyItem;
  ...
  if (calcMode == CM_ASSIGN && itemToBeAssigned != 0 &&
      !(tam.alpha && tam.mode != TM_NEWMENU)) {

#if defined(VERBOSEKEYS)
    printf(">>>>Z 0011 btnFnPressed >>determineFunctionKeyItem_C47; "
           "data=|%s| data[0]=%d shiftF=%d shiftG=%d\n",
           (char *)data, ((char *)data)[0], shiftF, shiftG);
#endif // VERBOSEKEYS
...
#if defined(VERBOSEKEYS)
    printf(">>>>Z 011a btnFnPressed >>determineFunctionKeyItem_C47; "
           "data=|%s| data[0]=%d shiftF=%d shiftG=%d\n",
           (char *)data, ((char *)data)[0], shiftF, shiftG);
#endif // VERBOSEKEYS
...
#pragma GCC diagnostic pop
    _closeCatalog();
  } else if (calcMode != CM_REGISTER_BROWSER && calcMode != CM_FLAG_BROWSER &&
             calcMode != CM_ASN_BROWSER && calcMode != CM_FONT_BROWSER) {

#if defined(VERBOSEKEYS)
    printf(">>>>Z 0012 btnFnPressed >>determineFunctionKeyItem_C47; "
           "data=|%s| data[0]=%d shiftF=%d shiftG=%d\n",
           (char *)data, ((char *)data)[0], shiftF, shiftG);
#endif // VERBOSEKEYS

    //      int16_t item = determineFunctionKeyItem_C47((char *)data, shiftF,
    //      shiftG);
    /*
            if(shiftF || shiftG) {
    ...
    */
    ...
    lastErrorCode = 0;
    btnFnPressed_StateMachine(
        NULL,
        data); // never allow a function key to directly enter into a buffer -
               // always via the key detection btnFnPressed_StateMachine, to
               // pick up longpress or double press conditions
#if defined(VERBOSEKEYS)
    printf(">>>>Z 0013 btnFnPressed >>btnFnPressed_StateMachine; data=|%s| "
           "data[0]=%d shiftF=%d shiftG=%d\n",
           (char *)data, ((char *)data)[0], shiftF, shiftG);
#endif // VERBOSEKEYS

    /*
              if(calcMode != CM_ASSIGN && indexOfItems[item].func ==
       addItemToBuffer) {
                // If we are in the catalog then a normal key press should
       affect the Alpha Selection Buffer to choose
                // an item from the catalog, but a function key press should
       put the item in the AIM (or TAM) buffer
                ...
                if(calcMode == CM_PEM && !tam.mode) {
                  ...
                  fnKeyInCatalog = 1;
                  addItemToBuffer(item);                //this PEM TAM entry
       moved to keyFnRelease, to pick up the long presses fnKeyInCatalog = 0;
                }
                if(calcMode == CM_EIM && !tam.mode) {   //this EIM portion
       moved to after release, to allow longpress and double press
...
static bool_t _assignToMenu(uint8_t *data) {
  ...
  default: {
    displayCalcErrorMessage(ERROR_CANNOT_ASSIGN_HERE, ERR_REGISTER_LINE,
                            NIM_REGISTER_LINE);
#if defined(PC_BUILD)
    moreInfoOnError("In function _assignToMenu:", "the menu",
                    indexOfItems[-currentMenu()].itemCatalogName,
                    "is write-protected.");
#endif // PC_BUILD
  endReturnTrue:
...
void btnFnReleased(GtkWidget *notUsed, GdkEvent *event, gpointer data) {
  ...
  if (calcMode != CM_REGISTER_BROWSER && calcMode != CM_FLAG_BROWSER &&
    ...
    if (calcMode == CM_ASSIGN && itemToBeAssigned != 0 &&
      ...
      //         if( (  (itemToBeAssigned == ITM_SHIFTf || itemToBeAssigned
      //         == ITM_SHIFTg || itemToBeAssigned == KEY_fg) &&
      //                 (previousCalcMode == CM_NORMAL) &&
      //                 (shiftF || shiftG) &&
      //                 ((uint8_t *)data)[0] >= '1' &&
      //                 ((uint8_t *)data)[0] <= '6'
      //                 )  ) {       //prevent "shifts on rows f and g on
      //                 F6 to be overwritten //Allow any normal mode menu
      //                 HOME PFN MyM, except shifts not in f or g line
      //           return;
      //         }
      //         else

      ////prevent "ALPHA" on F6 to be overwritten
      //      if(!(previousCalcMode == CM_AIM && (!shiftG && !shiftF) &&
      //      ((uint8_t *)data)[0] == '6')) {       //prevent "ALPHA" on F6
      //      to be overwritten
    ...
    if (!releaseOverride) {
      btnFnReleased_StateMachine(
          NULL, data); // This function does the longpress differentiation,
                       // and calls ExecuteFunctio below, via fnbtnclicked
      releaseOverride = false;
    }
  ...
  screenUpdatingMode &=
      ~SCRUPD_MANUAL_STATUSBAR; // Ensure status bar is always checked after
                                // fn key release. The new statusbar code
                                // does not clear the entire statusbar, it
                                // only updates if a setting changed
...
static void executeFunction(const char *data, int16_t item_) {
...
#if defined(VERBOSEKEYS)
  printf("keyboard.c: executeFunction %i (beginning of executeFunction): "
         "%i, %s tam.mode=%i calcMode=%u aimBuffer=%s\n",
         item, currentMenu(), indexOfItems[-currentMenu()].itemSoftmenuName,
         tam.mode, calcMode, aimBuffer);
#endif // VERBOSEKEYS
  ...
  if (calcMode != CM_REGISTER_BROWSER && calcMode != CM_FLAG_BROWSER &&
...
#if defined(VERBOSEKEYS)
      printf(">>>> R000AA >>determineFunctionKeyItem_C47 %d |%s| "
             "shiftF=%d, shiftG=%d tam.mode=%i\n",
...
#if defined(VERBOSEKEYS)
      printf(">>>> R000AB >>determineFunctionKeyItem_C47 %d |%s| "
             "shiftF=%d, shiftG=%d tam.mode=%i\n",
             item, data, shiftF, shiftG, tam.mode);
#endif // VERBOSEKEYS
    ...
    // in graph plot menu, wanting to change Normal Mode items, so open the
    // correct menu first and return to Normal Mode, and stop the
    // processing.
    if (calcMode == CM_GRAPH && currentMenu() == -MNU_PLOT_FUNC &&
        (item == VAR_LX || item == VAR_UX)) {
      calcMode = CM_NORMAL;
    ...
    // Update currentUserMenu for user defined menus selected in an existing
    // function
    if ((currentMenu() == -MNU_DYNAMIC) || (currentMenu() == -MNU_HOME) ||
        (currentMenu() == -MNU_PFN)) {
      setCurrentUserMenu(
          item,
          userMenus[currentUserMenu].menuItem[dynamicMenuItem].argumentName);
    }

    // If specific pseudo menus are accessed in PEM, the special function
    // code won't run and instead the associated menu must opne
...
#if defined(VERBOSEKEYS)
    printf(">>>> R000B                                %d |%s| shiftF=%d, "
           "shiftG=%d tam.mode=%i\n",
           item, data, shiftF, shiftG, tam.mode);
#endif // VERBOSEKEYS
#if defined(PC_BUILD) && defined(VERBOSE_MINIMUM)
    printf(">>>Function selected: executeFunction data=|%s| f=%d g=%d "
           "tam.mode=%i\n",
           (char *)data, shiftF, shiftG, tam.mode);
    printf("    item %s 0: calcMode=%u item=%d=%s f=%d g=%d\n",
           (item < 0 ? "< " : ">="), calcMode, item, getItemCatalogName(item),
           shiftF, shiftG);
    fflush(stdout);
#endif // PC_BUILD

    resetShiftState(); // shift cancelling delayed to this point after state
                       // machine

#if defined(VERBOSEKEYS)
    printf(">>>> R000C                                %d |%s| shiftF=%d, "
           "shiftG=%d tam.mode=%i\n",
           item, data, shiftF, shiftG, tam.mode);
#endif // VERBOSEKEYS

    if (/*showFunctionNameItem*/ item != 0) {
      /* //JM vv Rmove the possibility for error by removing code that may
         conflict with the state machine item = showFunctionNameItem;
      */
      showFunctionNameItem = 0;
      if (calcMode != CM_CONFIRMATION &&
          data[0] != 0) { // JM data is used if operation is from the real
                          // keyboard. item is used directly if called from XEQM
        lastErrorCode = 0;
        ...
        if (calcMode != CM_PEM && item == -MNU_Sfdx) {
          ...
          return;
                   ...
                   (item == ITM_INTEGRAL || item == ITM_INTEGRAL_YX)) {
          switch (calcMode) {
          case CM_NIM: {
            closeNim();
            break;
          ...
          if (calcMode == CM_ASSIGN && itemToBeAssigned == 0 &&
            ...
            leaveAsmMode();
            popSoftmenu();
          } else if (calcMode == CM_ASSIGN && itemToBeAssigned != 0 &&
                     (item == -MNU_HOME || item == -MNU_PFN ||
                      item == -MNU_MyMenu)) {
            itemToBeAssigned = item;
            leaveAsmMode();
            showSoftmenu(item);
                     ...
                     !tam.alpha) {
            if ((currentMenu() == -MNU_TAMINDIRECT) &&
                ((item == -MNU_VAR) || (item == -MNU_REG))) {
              showSoftmenu(item);
            } else {
              fnKeyInCatalog = 1;
              if (item < 0) {
...
#if defined(VERBOSEKEYS)
            printf(">>>Function: executeFunction: calcMode=%u "
                   "showSoftmenu(%d)\n",
                   calcMode, item);
#endif // VERBOSEKEYS
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
            ...
            if ((item == -MNU_Solver || item == -MNU_Grapher ||
                 item == -MNU_Sf || item == -MNU_1STDERIV ||
                 item == -MNU_2NDDERIV || item == -MNU_Sf_TOOL ||
                 item == -MNU_Solver_TOOL) &&
                lastErrorCode != 0) {
              popSoftmenu();
              currentSolverStatus &= ~SOLVER_STATUS_INTERACTIVE;
        ...
        if (tam.mode && catalog &&
            (tam.digitsSoFar || isFunctionOldParam16(tam.function) ||
             (!tam.indirect &&
              (tam.mode == TM_VALUE || tam.mode == TM_VALUE_CHB ||
               (tam.mode == TM_KEY && !tam.keyInputFinished))))) {
          // disabled
          ...
          return;
                  ...
                  getItemFunc(item) !=
                      addItemToBuffer) // skip if not label name (TAM) AND a
                                       // bufferized letter
                 && calcMode == CM_PEM &&
                 !catalog && // allow only in case of PEM, and a CAT
                 (tam.mode == TM_FLAGR || tam.mode == TM_FLAGW) &&
                 !(tam.mode &&
                   tam.function == ITM_DELP)) { // TODO: is that correct
                                                // //don't allow DELP
          // printf("tam.function=%d indexOfItems[tam.function].cat=%s
          // item=%d indexOfItems[item].cat=%s (indexOfItems[item].param &
          // 0xff)=%d \n",tam.function,
          // indexOfItems[tam.function].itemCatalogName, item,
          // indexOfItems[item].itemCatalogName , (indexOfItems[item].param
          // & 0xff));
          if ((tam.mode == TM_FLAGR || tam.mode == TM_FLAGW) &&
              (item != ITM_INDIRECTION) && !tam.indirect) {
            ...
            leaveTamModeIfEnabled();
                  ...
                  getItemFunc(item) !=
                      addItemToBuffer) // skip if not label name (TAM) AND a
                                       // bufferized letter
                 && calcMode == CM_PEM && catalog &&
                 catalog !=
                     CATALOG_MVAR && // allow only in case of PEM, and a CAT
                 !(tam.mode &&
                   tam.function == ITM_DELP)) { // TODO: is that correct
                                                // //don't allow DELP

          fnKeyInCatalog = 1;
          if (indexOfItems[item].func == fnGetSystemFlag &&
              (tam.mode == TM_FLAGR || tam.mode == TM_FLAGW) && !tam.indirect) {
            tam.value = (indexOfItems[item].param & 0xff);
            tam.alpha = true;
            addStepInProgram(tamOperation());
            leaveTamModeIfEnabled();
          } else if (indexOfItems[item].func ==
                     addItemToBuffer) { // this section is added, it was
                                        // commented out in btnFnPressed
                                        // line 760, it is moved here, as
                                        // longpress works on release.
            // Here we deal with PEM TAM mode menu entry, i.e. item's sent
            // to buffer. See issue #454 context.
            if (getSystemFlag(FLAG_ALPHA)) {
              processAimInput(item); // sets keyActionProcessed
              if (tam.mode) {
                // printf("cccc tam.mode=%i tam.f=%i Popping
                // menu\n",tam.mode, tam.function);
                popSoftmenu();
              }
            } else {
              addStepInProgram(
                  item); // I am not sure if this can actually be needed: It
                         // was in the btnFnPressed section in line 760
            }
            hourGlassIconEnabled = false;
          } else if (tam.mode) {
          ...
          return;
                   ...
                   (item == ITM_toINT || item == ITM_HASH_JM)) {
          if (aimBuffer[0] != 0) {
        ...
        // If we are in the catalog then a normal key press should affect
        // the Alpha Selection Buffer to choose an item from the catalog,
        // but a function key press should put the item in the AIM (or TAM)
        // buffer Use this variable to distinguish between the two
        fnKeyInCatalog = 1;
        if (tam.mode && catalog &&
            (tam.digitsSoFar || isFunctionOldParam16(tam.function) ||
             (!tam.indirect &&
          ...
          // disabled
                   ...
                   !(tam.mode == TM_VALUE &&
                     (item == ITM_TAMMAX || item == ITM_YY_TRACK ||
                      item == ITM_YY_OFF))) {
          bool_t isInConfig =
              tam.mode == TM_FLAGW &&
              currentMenu() == -MNU_SYSFL; // JM Do not drop out of SYSFLG
          ...
          if ((currentMenu() == -MNU_MODE || currentMenu() == -MNU_PREF) &&
              isInConfig && item != ITM_EXIT1 &&
              item != ITM_BACKSPACE) { // JM do not drop out of SYSFLG
            fnCFGsettings(0);          // JM
          } // JM
        }
        //          else if((calcMode == CM_NORMAL || calcMode == CM_AIM) &&
        ...
        //          }
                 ...
                 ((catalog && catalog != CATALOG_MVAR) ||
                  (currentMenu() ==
                       -MNU_EQ_EDIT && // Allow only EIM enabled functions
                                       // through to be added as text
                   item != ITM_EQ_LEFT &&
                   item != ITM_EQ_RIGHT && item != CHR_num &&
                   item != CHR_case &&
                   (indexOfItems[item].status & EIM_STATUS) == EIM_ENABLED))) {
          if (currentMenu() ==
              -MNU_CONST) { // Add # prefix for constants in equations
        ...
        } else if ((calcMode == CM_NORMAL || calcMode == CM_NIM) &&
                   (ITM_0 <= item && item <= ITM_F) &&
                   (!catalog || catalog == CATALOG_MVAR)) {
          ...
          addItemToNimBuffer(item);
                   ...
                   ((/*item==ITM_DRG ||*/ item == ITM_DMS2 ||
                     item == ITM_dotD) &&
                    !catalog)) { // JM Remove DRG from here, there seems to
                                 // be no need to send DRG to the buffer
          addItemToNimBuffer(item);
        } // JM
        else if (calcMode == CM_MIM && currentMenu() != -MNU_M_EDIT &&
                 (item != ITM_CC && item != ITM_op_j &&
                  item != ITM_op_j_pol)) { // JM added ITM_CC to let it work
                                           // in matrix edit
          addItemToBuffer(item);
        } else if (item > 0) { // function
          if (calcMode == CM_NORMAL &&
              ( // switch off BASE mode using the HEX/DEC... buttons
          ...
          if (calcMode == CM_NIM && item != ITM_CC && item != ITM_op_j &&
              item != ITM_op_j_pol && item != ITM_HASH_JM &&
              item != ITM_toINT &&
              item != ITM_ms) { // JMNIM Allow NIM not closed, so that JMNIM
                                // can change the bases without ierrors
                                // thrown; also change from # to i
            closeNim();
            if (calcMode != CM_NIM) {
          ...
          if (item == ITM_KEYMAP) {
            cursorEnabled = false; // cursor is re-activated automatically
                                   // elsewhere, after button release
          }
          if (calcMode == CM_AIM &&
              !(isAlphabeticSoftmenu() || isJMAlphaOnlySoftmenu() ||
                item == ITM_KEYMAP)) {
            closeAim();
          }
          ...
          if (tam.alpha && calcMode != CM_ASSIGN && tam.mode != TM_NEWMENU &&
              !((tam.mode == TM_STORCL || tam.mode == TM_LABEL ||
                 tam.mode == TM_LBLONLY || tam.mode == TM_KEY ||
                 tam.mode == TM_M_DIM || tam.mode == TM_REGISTER ||
                 tam.mode == TM_CMP) &&
                (item == CHR_num || item == CHR_case || item == ITM_SCR ||
                 item == ITM_USERMODE))) {
            if (calcMode != CM_PEM ||
                item != ITM_NOP) { // Here we left TAM in the context of
                                   // issue #454
              leaveTamModeIfEnabled();
            }
            ...
            if (calcMode == CM_ASSIGN && itemToBeAssigned == 0 &&
              ...
              if (item == CHR_case) {
                ...
                if (item == ITM_XEQ && dynamicMenuItem > -1) {
                  ...
                  if (strcmp(varCatalogItem, "XEQ") != 0) {
                    ...
                    if (regist != INVALID_VARIABLE) {
                      ...
                      displayCalcErrorMessage(ERROR_LABEL_NOT_FOUND,
                                              ERR_REGISTER_LINE, REGISTER_X);
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
                      sprintf(errorMessage, "string '%s' is not a named label",
                              varCatalogItem);
                      moreInfoOnError("In function executeFunction:",
                  ...
                  if (strcmp(varCatalogItem, "RCL") != 0) {
                    ...
                    if (regist != INVALID_VARIABLE) {
                      item = regist - FIRST_NAMED_VARIABLE +
                             ASSIGN_NAMED_VARIABLES;
                    } else {
                      displayCalcErrorMessage(ERROR_LABEL_NOT_FOUND,
                                              ERR_REGISTER_LINE, REGISTER_X);
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
                      sprintf(errorMessage,
                              "string '%s' is not a named variable",
                              varCatalogItem);
                      moreInfoOnError("In function executeFunction:",
                                      errorMessage, NULL, NULL);
#endif // (EXTRA_INFO_ON_CALC_ERROR == 1)
                ...
                if (previousCalcMode == CM_AIM &&
                    isAlphaSubmenu(
                        0)) { // JMvv close menu to allow only one charac
                  popSoftmenu();
                  showSoftmenu(-MNU_MyAlpha); // push MyAlpha in case ALPHA
                                              // is up (likely)
                }
              }
            } else if (calcMode == CM_ASSIGN && tam.alpha &&
                       tam.mode != TM_NEWMENU && item != ITM_NOP) {
              processAimInput(item); // sets keyActionProcessed
              if (stringGlyphLength(aimBuffer) > 6) {
                assignLeaveAlpha();
                assignGetName2();
...
#if defined(VERBOSEKEYS)
              printf("keyboard.c: executeFunction calcmode=%u %i (before "
                     "runfunction): %i, %s tam.mode=%i\n",
                     calcMode, item, currentMenu(),
                     indexOfItems[-currentMenu()].itemSoftmenuName, tam.mode);
#endif // VERBOSEKEYS
              ...
              } else if (((calcMode == CM_PEM && !tam.mode &&
...
#if defined(VERBOSEKEYS)
              printf("keyboard.c: executeFunction calcmode=%u %i (after "
                     "runfunction): %i, %s tam.mode=%i\n",
                     calcMode, item, currentMenu(),
                     indexOfItems[-currentMenu()].itemSoftmenuName, tam.mode);
#endif // VERBOSEKEYS
      ...
      noMoreToDo:
        ...
        fnKeyInCatalog = 0;
                 ...
                 (item == ITM_YES || item == ITM_NO)) {
        runFunction(item);
      }
    } else if (calcMode == CM_CONFIRMATION) {
      temporaryInformation =
          TI_ARE_YOU_SURE; // Keep confirmation message on screen
    }
  }
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
  printf(">>>  refreshScreen3 from keyboard.c executeFunction calcMode=%u "
         "screenUpdatingMode=%u\n",
         calcMode, screenUpdatingMode);
#endif

  refreshScreen(114);
  // TODO 2023-04-15 check here. It needs to be changed not to always
  // refresh the screen.
  ...
  printf("keyboard.c: executeFunction (end): calcmode=%u %i, %s\n", calcMode,
         currentMenu(), indexOfItems[-currentMenu()].itemSoftmenuName);
#endif // VERBOSEKEYS
}

bool_t shiftKeyClearsError = false;
#define stringToKeyNumber(data)                                                \
  ((*((char *)data) - '0') * 10 + *(((char *)data) + 1) -                      \
   '0') // input string = "28", keynumber = 28  (keys 00-36)
...
static int16_t determineItem(const char *data) {
  ...
  //.    if(kbd_usr[36].primaryTam == ITM_EXIT1) //opposite keyboard V43 LT,
  // 43S, V43 RT
  key = getSystemFlag(FLAG_USER) ? (kbd_usr + key_no) : (kbd_std + key_no);
  //.    else
  //.      key = getSystemFlag(FLAG_USER) && ((calcMode == CM_NORMAL) ||
  //(calcMode == CM_AIM) || (calcMode == CM_NIM) || (calcMode == CM_EIM) ||
  //(calcMode == CM_PLOT_STAT) || (calcMode == CM_GRAPH) || (calcMode ==
  // CM_LISTXY)) ? (kbd_usr + key_no) : (kbd_std + key_no);    //JM Added
  //(calcMode == CM_NORMAL) to prevent user substitution in AIM and TAM

  fnTimerExec(TO_FN_EXEC); // dr execute queued fn

#if defined(PC_BUILD)
  sprintf(tmp,
          "^^^^^^^keyboard.c: determineitem: key_no: %u, key->primary1: %d:",
          key_no, key->primary);
  jm_show_comment(tmp);
#endif // PC_BUILD

  if ((key->primary != ITM_SHIFTf) && (key->primary != KEY_fg) &&
      (!SHOWMODE || !(key->primary == ITM_RCL || key->primary == ITM_RS ||
                      key->primary == ITM_UP1 || key->primary == ITM_DOWN1 ||
                      (allowShowDigits && key->primary >= ITM_0 &&
                       key->primary <= ITM_9)))) {
  ...
  if (ShiftOverride ==
      0) { // disable long and double press if Sigma+ is shift g
    Setup_MultiPresses(key->primary);
  }

#if defined(PC_BUILD)
  sprintf(tmp,
          "^^^^^^^keyboard.c: determineitem: key->primary2: %d:", key->primary);
  jm_show_comment(tmp);
#endif // PC_BUILD
  ...
  if ((calcMode == CM_NORMAL || calcMode == CM_AIM || calcMode == CM_NIM ||
       calcMode == CM_MIM || calcMode == CM_EIM || calcMode == CM_PEM ||
       calcMode == CM_PLOT_STAT || calcMode == CM_GRAPH ||
       calcMode == CM_ASSIGN || calcMode == CM_ASN_BROWSER ||
       calcMode == CM_REGISTER_BROWSER || calcMode == CM_FLAG_BROWSER ||
       calcMode == CM_FONT_BROWSER || calcMode == CM_TIMER)) {
  ...
  else if ((key->primary == KEY_fg || key->primary == ITM_SHIFTf ||
            key->primary == ITM_SHIFTg) &&
           (calcMode == CM_PLOT_STAT || calcMode == CM_LISTXY)) {
    if (lastErrorCode != 0)
...
#if defined(PC_BUILD)
  sprintf(tmp,
          "^^^^^^^keyboard.c: determineitem: key->primary3: %d:", key->primary);
  jm_show_comment(tmp);
#endif // PC_BUILD
       // JM shifts
  if (!tam.mode && (calcMode == CM_NIM || calcMode == CM_NORMAL) &&
      (lastIntegerBase >= 2 && getSystemFlag(FLAG_TOPHEX)) &&
      (key_no >= 0 && key_no <= 5)) { // JMNIM vv Added direct A-F for hex entry
    result = shiftF ? key->fShifted : shiftG ? key->gShifted : key->primaryAim;
    switch (result) {
    case ITM_SHIFTf:
    case ITM_SHIFTg:
    case KEY_fg:
    ...
    // printf(">>> ±±±§§§ keys key:%d result:%d Calmode:%d, nimbuffer:%s,
    // lastbase:%d, nimnumberpart:%d\n",key_no, result,
    ...
    if (calcMode == CM_AIM ||
        ...
        calcMode == CM_EIM || tam.alpha ||
        (calcMode == CM_ASSIGN &&
         (previousCalcMode == CM_AIM || previousCalcMode == CM_EIM)) ||
      ...
      } else if ((result == ITM_COMMA || result == ITM_PERIOD) &&
                 (calcMode == CM_EIM || calcMode == CM_AIM) &&
                 getSystemFlag(FLAG_ALPHA)) {
        switch ((shiftG ? 2 : 0) +
                (getSystemFlag(FLAG_NUMLOCK) ? 1 : 0)) { // gSHIFTED numLock
          // case 0: result = key->primaryAim;break;           // 0        0
          // key->primaryAim
        case 1:
          result = RADIX34_MARK_DEC_ITM;
          break; //                                   0        1 decimal
          // case 2: result = RADIX34_MARK_DEC_ITM; break;     // 2        0
          // decimal
        case 3:
          result = RADIX34_MARK_NOT_DEC_ITM;
          break; //                                   2        1      not
                 //                                   the decimal
        default:;
        }
      ...
      result = key->primaryTam; // No shifted function in TAM
               ...
               calcMode == CM_MIM || calcMode == CM_FONT_BROWSER ||
               calcMode == CM_FLAG_BROWSER || calcMode == CM_ASN_BROWSER ||
               calcMode == CM_REGISTER_BROWSER ||
               calcMode == CM_BUG_ON_SCREEN || calcMode == CM_CONFIRMATION ||
               calcMode == CM_PEM || GRAPHMODE || calcMode == CM_ASSIGN ||
               calcMode == CM_TIMER || calcMode == CM_LISTXY) {
      result = shiftF ? key->fShifted : shiftG ? key->gShifted : key->primary;
  ...
  if (result != ITM_SNAP) { // not using the above, the timers need stopping
                            // and if the shifts are cleared from screen the
                            // flags must follow.
    resetShiftState();
  }

  if (calcMode == CM_ASSIGN && itemToBeAssigned != 0 &&
      (result == ITM_NOP || result == ITM_NULL)) {
    result = ITM_LBL;
  }
...
#if defined(PC_BUILD)
void btnClickedP(GtkWidget *w,
                 gpointer data) { // JM PRESSED FOR KEYBOARD F REPEAT
  GdkEvent mouseButton;
  mouseButton.button.button = 1;
  mouseButton.type = 0;
...
void btnPressed(GtkWidget *notUsed, GdkEvent *event, gpointer data) {
  ...
  if (event->type == GDK_DOUBLE_BUTTON_PRESS ||
      event->type == GDK_TRIPLE_BUTTON_PRESS) { // return unprocessed for
...
#endif // VERBOSEKEYS
  if (programRunStop == PGM_RUNNING || programRunStop == PGM_PAUSED) {
    if ((item == ITM_RS || item == ITM_EXIT1) && !getSystemFlag(FLAG_INTING) &&
  ...
  } else if ((keyCode == Norm_Key_00_key) && (keyStateCode == 0) &&
             Norm_Key_00.used &&
             !(lastIntegerBase >= 2 && getSystemFlag(FLAG_TOPHEX))) {
  ...
  if (item != ITM_NOP && item != ITM_NULL) {
    ...
    sprintf(tmp,
            "keyboard.c: btnPressed 1--> processKeyAction(%d) which is "
            "str:%s\n",
            item, (char *)data);
...
#if defined(PC_BUILD_TELLTALE)
    sprintf(tmp,
            "keyboard.c: btnPressed 2--> processKeyAction(%d) which is "
            "str:%s; keyActionProcessed=%u\n",
...
static void convertXYToKey(int x, int y) {
  ...
  for (int i = 0; i < 43; i++) {
    ...
    if (i == 10 && currentBezel == 2 &&
        (tam.mode == TM_LABEL || tam.mode == TM_LBLONLY ||
         (tam.mode == TM_SOLVE &&
          (tam.function != ITM_SOLVE || calcMode != CM_PEM)) ||
         (tam.mode == TM_KEY && tam.keyInputFinished))) {
...
bool_t checkKeyShifts(const char *data) {
  ...
  key = getSystemFlag(FLAG_USER) &&
                ((calcMode == CM_NORMAL) || (calcMode == CM_AIM) ||
                 (calcMode == CM_EIM) || (calcMode == CM_NIM))
            ? (kbd_usr + key_no)
            : (kbd_std + key_no);
...
void btnReleased(GtkWidget *notUsed, GdkEvent *event, gpointer data) {
  ...
  if (SHOWMODE && (lastItem == KEY_fg || lastItem == ITM_SHIFTf) &&
      lastItem != SCREENDUMP) {
    // f is delayed in SHOW to release. fg and f both will perform the
    // f-function. F-DISP will be screen dump.
    fg_processing_jm();
    shiftF = true;
    shiftG = false;
    lastshiftF = shiftF;
  ...
  // printf("release: showFunctionNameItem=%i calcMode=%i lastItem = %i
  // keyActionProcessed=%i showFunctionNameItem=%i releaseOverride=%i
  // tam.mode=%i tamBuffer=%s tamBuffer[0]=%u\n", showFunctionNameItem,
  // calcMode, lastItem, keyActionProcessed, showFunctionNameItem,
  // releaseOverride, tam.mode, tamBuffer, tamBuffer[0]);
  ...
  if (calcMode == CM_ASSIGN && itemToBeAssigned != 0 && tamBuffer[0] == 0) {
    ...
    if (calcMode == CM_NIM && delayCloseNim && item != ITM_ms &&
        item != ITM_CC && item != ITM_op_j && item != ITM_op_j_pol &&
        item != ITM_dotD && item != ITM_HASH_JM && item != ITM_toINT) {
      delayCloseNim = false;
      closeNim(); // JM moved here, from bufferize see JMCLOSE, to
                  // retain NIM if needed for .ms. Only a problem due to
                  // longpress.
#if defined(PC_BUILD)
    ...
    bool_t Norm_Key_00_released =
        !getSystemFlag(FLAG_USER) && (keyStateCode == 0) &&
        (keyCode == Norm_Key_00_key) && Norm_Key_00.used &&
        (!(lastIntegerBase >= 2 && getSystemFlag(FLAG_TOPHEX)));

    char *funcParam = (Norm_Key_00_released
                           ? Norm_Key_00.funcParam
                           : (char *)getNthString((uint8_t *)userKeyLabel,
                                                  keyCode * 6 + keyStateCode));
#if defined(PC_BUILD) && defined(VERBOSE_DETERMINEITEM)
    printf("**[DL]** btnReleased1 - item %d showFunctionNameArg %s "
           "funcParam %s\n",
           item, showFunctionNameArg, funcParam);
#endif // VERBOSE_DETERMINEITEM
    if (showFunctionNameArg != NULL) {
      funcParam = showFunctionNameArg; // Needed when executing a user
                                       // menu from a long pressed key
#if defined(PC_BUILD) && defined(VERBOSE_DETERMINEITEM)
      printf("**[DL]** btnReleased2 - item %d showFunctionNameArg %s "
             "funcParam %s\n",
             item, showFunctionNameArg, funcParam);
#endif // VERBOSE_DETERMINEITEM
    }
    ...
    if (item < 0) {
      ...
      showSoftmenu(item);
      screenUpdatingMode &= ~SCRUPD_MANUAL_MENU;
      // printf("AA2 allowShiftsToClearError=%u !checkShifts=%u
      // screenUpdatingMode=%u
      // temporaryInformation=%u\n",allowShiftsToClearError,
      // !checkShifts((char *)data), screenUpdatingMode,
      // temporaryInformation);
    } else {
#if defined(PC_BUILD)
      ...
      if (item != ITM_NOP && tam.alpha &&
          indexOfItems[item].func != addItemToBuffer && aimBuffer[0] == 0) {
        // We are in TAM mode so need to cancel first (equivalent to
        // EXIT)
        if (item == ITM_EXIT1) {
        ...
        } else if (item != ITM_BACKSPACE) { // [DL] to ensure backspace will
                                            // be processed in tamProcessInput
          leaveTamModeIfEnabled();
        }
      }
      ...
      if (item == ITM_RCL &&
          (getSystemFlag(FLAG_USER) || Norm_Key_00_released) &&
          funcParam[0] != 0) {
        ...
        if (var != INVALID_VARIABLE) {
...
#if defined(PC_BUILD) && defined(VERBOSE_DETERMINEITEM)
            printf("**[DL]** insertUserItemInProgram(item=%d, "
                   "funcParam=%s)\n",
                   item, funcParam);
#endif // VERBOSE_DETERMINEITEM
...
#if defined(PC_BUILD) && defined(VERBOSE_DETERMINEITEM)
            printf("**[DL]** reallyRunFunction(item=%d, var=%d, "
                   "funcParam=%s)\n",
                   item, var, funcParam);
#endif // VERBOSE_DETERMINEITEM
          ...
          displayCalcErrorMessage(ERROR_UNDEF_SOURCE_VAR, ERR_REGISTER_LINE,
                                  REGISTER_X);
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
          sprintf(errorMessage, "string '%s' is not a named variable",
                  funcParam);
          moreInfoOnError("In function btnReleased:", errorMessage, NULL, NULL);
#endif // (EXTRA_INFO_ON_CALC_ERROR == 1)
        }
      } else if (item == ITM_XEQ &&
                 (getSystemFlag(FLAG_USER) || Norm_Key_00_released) &&
                 funcParam[0] != 0) {
...
#if defined(PC_BUILD) && defined(VERBOSE_DETERMINEITEM)
            printf("**[DL]** reallyRunFunction(item=%d, label=%d, "
                   "funcParam=%s)\n",
...
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
        printf("btnReleased 2: Closed NIM (delayed) delayCloseNim=%u, ",
               delayCloseNim);
        printf("runfunction (%d) tam=%i getSystemFlag(FLAG_ALPHA)=%i \n", item,
               tam.mode, getSystemFlag(FLAG_ALPHA));
        printf(">>> btnReleased runfunction(%i) calcMode=%d "
               "previousCalcMode=%d screenUpdatingMode=%d\n",
               item, calcMode, previousCalcMode,
               screenUpdatingMode); // JMYY
#endif                              // PC_BUILD &&MONITOR_CLRSCR
        ...
        if (CM_NIM && (item == ITM_AIM || item == ITM_XEQ || item == ITM_GTO) &&
            nimNumberPart == NP_INT_BASE &&
            aimBuffer[strlen(aimBuffer) - 1] ==
                '#') { // do not allow shortinteger base change input to
                       // exit when alpha is pressed
          goto RELEASE_END;
        }
...
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
        printf(">>> btnReleased ran(%i) calcMode=%d "
               "previousCalcMode=%d screenUpdatingMode=%d\n",
               item, calcMode, previousCalcMode,
               screenUpdatingMode); // JMYY
        printf("    runfunction (%d) tam=%i "
               "getSystemFlag(FLAG_ALPHA)=%i \n",
  ...
  if (programRunStop == PGM_SINGLE_STEP) { // Key pressed was SST
    programRunStop = PGM_STOPPED;
    runProgram(true, INVALID_VARIABLE); // Execute one program step
                                        // after key released
  }
...
RELEASE_END:
  ...
  // printf("BB allowShiftsToClearError=%u !checkShifts=%u
  // screenUpdatingMode=%u\n",allowShiftsToClearError,
  // !checkShifts((char *)data), screenUpdatingMode);
  ...
  bool_t preventRefreshAtTheEndOfReleasedKey =
      ...
      ((lastKeyItemDetermined == ITM_UP1 ||
        lastKeyItemDetermined == ITM_DOWN1) &&
       calcMode == CM_GRAPH);

  if (!preventRefreshAtTheEndOfReleasedKey) {
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
    printf(">>> END of btnReleased after (117) calcMode=%d "
           "previousCalcMode=%d screenUpdatingMode=%d\n",
...
void leavePem(void) {
  if (freeProgramBytes >= 4) { // Push the programs to the end of RAM
    uint32_t newProgramSize = (uint32_t)((uint8_t *)(ram + RAM_SIZE_IN_BLOCKS) -
                                         beginOfProgramMemory) -
                              (freeProgramBytes & 0xfffc);
...
void processKeyAction(int16_t item) {
...
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
  printf(">>>> processKeyAction: calcMode=%u item=%d  programRunStop=%d "
         "lastErrorCode=%u SHOWMODE=%u screenUpdatingMode=%i\n",
  ...
  if (temporaryInformation == TI_VIEW_REGISTER) {
  ...
  } else if (temporaryInformation != TI_NO_INFO && item != ITM_UP1 &&
             item != ITM_DOWN1 && item != ITM_EXIT1 && item != ITM_BACKSPACE &&
             !((item == ITM_RCL || item == ITM_RS ||
                (item >= ITM_0 && item <= ITM_9 && allowShowDigits)) &&
               SHOWMODE)) {
    if (SHOWMODE) {
      closeShowMenu();
    }
    temporaryInformation = TI_NO_INFO;
    screenUpdatingMode =
        SCRUPD_AUTO; // cannot use MENU & STACK update due to being in
                     // NIM, and NIM prevents clearing individually
    screenUpdatingMode |= SCRUPD_SKIP_STATUSBAR_ONE_TIME;
  }

  if (calcMode == CM_GRAPH && currentMenu() == -MNU_PLOT_FUNC &&
      ((item >= ITM_0 && item <= ITM_9) ||
       item == ITM_PERIOD)) { // incoming digit, change modes and go to
                              // GRAPHS input page
    calcMode = CM_NORMAL;
    showSoftmenu(-MNU_GRAPHS);
    screenUpdatingMode &= SCRUPD_MANUAL_MENU;
  ...
  if (item == KEY_COMPLEX &&
      calcMode == CM_MIM) { // JM Allow COMPLEX to function as CC if in Matrix
    item = ITM_CC;
  }
  ...
  if (GRAPHMODE) { // disregard any TI that may be up, as it is not
                   // possible to show in Graph Mode
    temporaryInformation = TI_NO_INFO;
  }

  if (GRAPHMODE && item != ITM_BACKSPACE && item != ITM_EXIT1 &&
      item != ITM_UP1 && item != ITM_DOWN1 && item != ITM_SNAP) {
    keyActionProcessed = true;
  } else if (calcMode == CM_ASN_BROWSER && item != ITM_PERIOD &&
             item != ITM_USERMODE && item != ITM_BACKSPACE &&
             item != ITM_EXIT1 && item != ITM_UP1 && item != ITM_DOWN1) {
    keyActionProcessed = true;
  } else {
    switch (item) {
    case ITM_BACKSPACE: {
      if (calcMode == CM_NIM || calcMode == CM_AIM || calcMode == CM_EIM) {
        ...
        // convert to Release action: let backspace run through to
        // Release and bypass the next fnKeyBackspace()
        break;
      } else {
        // JM No if needed, it does nothing if not in NIM. TO DISPLAY
        // NUMBER KEYPRESS DIRECTLY AFTER PRESS, NOT ONLY UPON RELEASE
        // break;
        keyActionProcessed =
            true; // JM move this to before fnKeyBackspace to allow
                  // fnKeyBackspace to cancel it if needed to allow this
                  // function via timing out to NOP, and this is
                  // incorporated with the CLRDROP
    ...
    case ITM_UP1: {
      if (calcMode != CM_CONFIRMATION) {
        keyActionProcessed =
            true; // JM swapped to before fnKeyUp to be able to check if
                  // key was processed below. Chose to process it here,
                  // as fnKeyUp does not have access to item.
        fnKeyUp(NOPARAM);
        if (!keyActionProcessed) {       // JMvv
          addItemToBuffer(ITM_UP_ARROW); // Let the arrows produce arrow up
                                         // and arrow down in ALPHA mode
        } // JM^^
        if (calcMode != CM_LISTXY &&
            (currentSoftmenuScrolls() ||
             !(calcMode == CM_NORMAL || calcMode == CM_PEM) ||
             temporaryInformation != TI_NO_INFO)) {
    ...
    case ITM_DOWN1: {
      if (calcMode != CM_CONFIRMATION) {
        keyActionProcessed =
            true; // swapped to before fnKeyUp to be able to check if
                  // key was processed below. Chose to process it here,
                  // as fnKeyUp does not have access to item.
        fnKeyDown(NOPARAM);
        if (!keyActionProcessed) {         // JM
          addItemToBuffer(ITM_DOWN_ARROW); // Let the arrows produce arrow up
                                           // and arrow down in ALPHA mode
        } // JM^^
        if (calcMode != CM_LISTXY &&
            (currentSoftmenuScrolls() ||
    ...
    case ITM_EXIT1: {
      if (SHOWMODE ||
          calcMode == CM_LISTXY) { // do action on press, instead of release
        fnKeyExit(NOPARAM);
        keyActionProcessed = true; // Removed to force EXIT on the RELEASE cycle
                                   // to make it do fnKeyExit later to allow NOP
      } else if (calcMode == CM_PEM) {
      ...
      if ((temporaryInformation != TI_NO_INFO) &&
        ...
        screenUpdatingMode = SCRUPD_AUTO;
        refreshRegisterLine(
            ERR_REGISTER_LINE); //[DL] added to force error line refresh
        refreshScreen(139);
        keyActionProcessed = true;
      } else if (temporaryInformation == TI_NO_INFO &&
                 softmenuStack[0].softmenuId == 0) {
        // Test to see if the statusbar flicker speed is hig enough.
        // Exit only refreshes the statusbar when MyM is up and no TI
        screenUpdatingMode &=
    ...
    case ITM_CC: {
#if defined(PC_BUILD)
      // printf("Monitor: item=%i calcMode=%i nimNumberPart=%i char=%c
      // lastIntegerBase=%i\n",item, calcMode, nimNumberPart,
      // aimBuffer[strlen(aimBuffer) - 1], lastIntegerBase);
#endif
      if (calcMode == CM_ASSIGN) {
      ...
      if (calcMode == CM_ASSIGN) {
        if (itemToBeAssigned == 0) {
          if (tam.alpha) {
            ...
            if (previousCalcMode == CM_AIM) {
              showSoftmenu(-MNU_MyAlpha); // JM push MyAlpha in case
                                          // ALPHA is up (likely)
            }
          }
        } else {
        ...
        // User menu name create: ASN + USER 'DDD' has a problem by
        // exiting to MyAlpha
    ...
    default: {
      ...
      printf("Switch - default: processKeyAction: calcMode=%d "
             "itemToBeAssigned=%d item=%d SHOWMODE=%u\n",
      ...
      if (calcMode == CM_ASSIGN && itemToBeAssigned != 0 &&
        ...
        switch (calcMode) { // place modes here which should not work
                            // with SNAP
          // case CM_REGISTER_BROWSER:
          // case CM_FLAG_BROWSER:
          // case CM_FONT_BROWSER:
        ...
        case CM_NORMAL: {
...
#endif
          if (SHOWMODE) {
            // printf("XXXXXXXX @@@@@@ temporaryInformation=%u
            // calcmode=%u showRegis=%u\n", temporaryInformation,
            // calcMode, showRegis);
            if (item == ITM_RCL) {
              keyActionProcessed = true;
              fnRecall(showRegis);
          ...
          else if (item == ITM_EXPONENT || item == ITM_PERIOD ||
                   (ITM_0 <= item && item <= ITM_9)) {
            addItemToNimBuffer(item);
            refreshRegisterLine(REGISTER_X);
            keyActionProcessed = true;
          }
          // Following commands do not timeout to NOP
          else if (item == ITM_UNDO || item == ITM_BST || item == ITM_SST ||
                   item == ITM_PR || item == ITM_AIM || item == ITM_SNAP) {
            runFunction(item);
            keyActionProcessed = true;
          }
          break;
        ...
        case CM_AIM: {
          // JM In AIM, BST and SST is not reaching here, as it is
          // reconfigured for CAPS lock and NUM lock
          if (item == ITM_BST || item == ITM_SST) {
            ...
            processAimInput(item);                  // sets keyActionProcessed
            refreshRegisterLine(AIM_REGISTER_LINE); // TO DISPLAY KEYPRESS
                                                    // DIRECTLY AFTER PRESS,
        ...
        case CM_NIM: {
          ...
          if (item == ITM_BST || item == ITM_SST) {
            ...
            if (calcMode == CM_NIM && (item == ITM_RI || item == ITM_dotD) &&
                (nimNumberPart == NP_INT_10 || nimNumberPart == NP_INT_16) &&
                lastIntegerBase > 0) {
              ...
              keyActionProcessed = true;
            } else if (calcMode == CM_NIM && item == ITM_HASH_JM &&
                       nimNumberPart == NP_INT_BASE &&
                       aimBuffer[strlen(aimBuffer) - 1] == '#') {
              ...
              addItemToNimBuffer(ITM_BACKSPACE);
              keyActionProcessed = true;
            } else if (calcMode == CM_NIM && item == ITM_PERIOD &&
                       nimNumberPart == NP_INT_BASE &&
                       aimBuffer[strlen(aimBuffer) - 1] == '#') {
            ...
            if (((ITM_0 <= item && item <= ITM_9) || item == ITM_toINT ||
                 item == ITM_HASH_JM || item == ITM_ms ||
                 ((ITM_A <= item && item <= ITM_F) && (lastIntegerBase >= 2) &&
                  getSystemFlag(FLAG_TOPHEX))) ||
                item == ITM_CHS || item == ITM_EXPONENT ||
                item == ITM_PERIOD) { // JMvv Direct keypresses; //JMNIM
                                      // Added direct A-F for hex entry
        ...
        case CM_REGISTER_BROWSER: {
          if (item == ITM_PERIOD) {
            ...
            if (rbrMode == RBR_GLOBAL || rbrMode == RBR_LOCAL) {
              ...
              if (currentRegisterBrowserScreen >=
                  FIRST_NAMED_VARIABLE +
                      numberOfNamedVariables) { // Reserved variables
                currentRegisterBrowserScreen -=
                    FIRST_NAMED_VARIABLE + numberOfNamedVariables;
                currentRegisterBrowserScreen +=
                    FIRST_RESERVED_VARIABLE + NUMBER_OF_LETTERED_VARIABLES;
              }
              fnRecall(currentRegisterBrowserScreen);
            }
            setSystemFlag(FLAG_ASLIFT);
            ...
            if (rbr1stDigit) {
              ...
              case RBR_LOCAL: {
                rbrRegister = (rbrRegister >= currentNumberOfLocalRegisters
                                   ? 0
                                   : rbrRegister);
                currentRegisterBrowserScreen =
                    FIRST_LOCAL_REGISTER + rbrRegister;
                break;
              }
              case RBR_NAMED: {
                rbrMode = RBR_GLOBAL;
        ...
        case CM_PEM: {
          if (item == ITM_PR) {
            ...
            keyActionProcessed = true;
            refreshScreen(123);
          } else if (aimBuffer[0] != 0 && !getSystemFlag(FLAG_ALPHA) &&
                     (item == ITM_HASH_JM || item == ITM_toINT ||
                      (nimNumberPart == NP_INT_BASE && item == ITM_RCL))) {
            if (item == ITM_HASH_JM) {
              item = ITM_toINT;
            }
        ...
        case CM_ASSIGN: {
          if (item > 0 && itemToBeAssigned == 0) {
            if (tam.alpha) {
              ...
              if (item == ITM_XEQ && tmpString[0] != 0 &&
                  (getSystemFlag(FLAG_USER) ||
                   ((currentKeyCode == Norm_Key_00_key) &&
                    (keyStateCode == 0) && Norm_Key_00.used))) {
                char label[15];
                ...
                if (regist != INVALID_VARIABLE) {
                  ...
                  sprintf(errorMessage, "string '%s' is not a named label",
                          label);
                  moreInfoOnError("In function processKeyAction:", errorMessage,
                                  NULL, NULL);
#endif // (EXTRA_INFO_ON_CALC_ERROR == 1)
                }
              } else if (item == ITM_RCL && tmpString[0] != 0 &&
                         (getSystemFlag(FLAG_USER) ||
                          ((currentKeyCode == Norm_Key_00_key) &&
                           (keyStateCode == 0) && Norm_Key_00.used))) {
                char var[15];
              ...
              itemToBeAssigned = numlockReplacements(
                  100, item, getSystemFlag(FLAG_NUMLOCK), false, false);
              if (ITM_A <= itemToBeAssigned && itemToBeAssigned <= ITM_Z &&
                  lowercaseselected) {
                itemToBeAssigned += (ITM_a - ITM_A);
              }
#if defined(PC_BUILD) && defined(VERBOSE_DETERMINEITEM)
              printf("**[DL]** itemToBeAssigned = %d %s\n", itemToBeAssigned,
                     indexOfItems[itemToBeAssigned].itemSoftmenuName);
#endif // VERBOSE_DETERMINEITEM

              if (previousCalcMode == CM_AIM)
                softmenuStack[0].softmenuId =
                    1; // JM change ALPHA to MyAlpha to be able to write
                       // ASN target
            }
            keyActionProcessed = true;
...
// printf("AAA: calcMode=%i lastItem = %i keyActionProcessed=%i
// showFunctionNameItem=%i releaseOverride=%i tam.mode=%i tamBuffer=%s
// tamBuffer[0]=%u\n", calcMode, lastItem, keyActionProcessed,
// showFunctionNameItem, releaseOverride, tam.mode, tamBuffer, tamBuffer[0]);
// enable this code to let the HOME and P.FN menus on the keyboard be active
        ...
        default: {
          sprintf(errorMessage,
                  "In function processKeyAction: %" PRIu8
                  " is an unexpected value while processing calcMode!",
                  calcMode);
...
static void menuUp(void) {
  ...
  } else if ((sm == -MNU_ALPHAMISC || sm == -MNU_ALPHAMATH ||
              sm == -MNU_ALPHA) &&
             alphaCase == AC_LOWER && arrowCasechange) { // JMcase
    alphaCase = AC_UPPER;
    showAlphaModeonGui(); // dr JM, see keyboardtweaks
  } else {
    int16_t itemShift = (catalog == CATALOG_NONE ? 18 : 6);
    if ((softmenuStack[0].firstItem + itemShift) <
        (menuId < NUMBER_OF_DYNAMIC_SOFTMENUS ? dynamicSoftmenu[menuId].numItems
                                              : softmenu[menuId].numItems)) {
...
static void menuDown(void) {
  ...
  if ((sm == -MNU_ALPHA_OMEGA || sm == -MNU_ALPHAINTL) &&
    ...
    softmenuStack[0].softmenuId++; // Switch to the lower case menu
              ...
              sm == -MNU_ALPHA) &&
             alphaCase == AC_UPPER && arrowCasechange) { // JMcase
    alphaCase = AC_LOWER;
    showAlphaModeonGui(); // dr JM, see keyboardtweaks
  } else {
    int16_t itemShift = (catalog == CATALOG_NONE ? 18 : 6);
    ...
    if ((softmenuStack[0].firstItem - itemShift) >= 0) {
      ...
      if (menuId < NUMBER_OF_DYNAMIC_SOFTMENUS) {
        softmenuStack[0].firstItem =
            ((dynamicSoftmenu[menuId].numItems - 1) / 6) / (itemShift / 6) *
            itemShift;
      } else {
        softmenuStack[0].firstItem =
            ((softmenu[menuId].numItems - 1) / 6) / (itemShift / 6) * itemShift;
      }
    }
    setCatalogLastPos();
  }
...
void fnKeyEnter(uint16_t unusedButMandatoryParameter) {
  ...
  case CM_NORMAL: {
    ...
    if (!getSystemFlag(FLAG_ERPN) ||
        ...
        (getSystemFlag(FLAG_ERPN) &&
         programRunStop ==
             PGM_RUNNING)) { // vv PHM eRPN 2021-07;   JM corrected eRPN
                             // on 2024-03-19 on master 86fd2a5
      setSystemFlag(FLAG_ASLIFT);
  ...
  case CM_AIM: {
    ...
    if (currentMenu() == -MNU_ALPHA) { // JM get out of the ALPHA menu and go to
                                       // MyM fto satisfy the old enter routines
    ...
    if (aimBuffer[0] == 0) {
      ...
      if (!getSystemFlag(FLAG_ERPN)) { // PHM eRPN 2021-07
        ...
        aimBuffer[0] = 0; // PHM JM Keeping the structure like 43S, to be able
                          // to pick up changes from their side easier
      } else {
        setSystemFlag(FLAG_ASLIFT);
        aimBuffer[0] = 0; // PHM JM Keeping the structure like 43S, to be able
                          // to pick up changes from their side easier
  ...
  case CM_MIM: {
    ...
    //       JM Keeping the structure like in 43S, to be able to pick up
    //       changes
    // JM: 2022-09-04:
    // JM: This code does not seem to be "not-used" See bug report
    // Gitlab #80. code seems needed. JM: PHM is not active in the
    // project anymore. Restored this code:
  case CM_NIM: {
    closeNim();
    ...
    if (aimBuffer[0] != 0) {
      ...
      if (lastErrorCode != 0) { // Stay in Edit mode for the current equation
        const char *equationString =
            TO_PCMEMPTR(allFormulae[currentFormula].pointerToFormulaData);
        if (equationString) {
          xcopy(aimBuffer, equationString,
                stringByteLength(equationString) + 1);
        } else {
          aimBuffer[0] = 0;
        }
        refreshRegisterLine(
  ...
  default: {
    sprintf(errorMessage, commonBugScreenMessages[bugMsgCalcModeWhileProcKey],
            "fnKeyEnter", calcMode, "ENTER");
    displayBugScreen(errorMessage);
  }
  }
  return;
...
static void stayInAIM(void) {
  if (calcMode == CM_AIM &&
      (currentMenu() != -MNU_ALPHA && currentMenu() != -MNU_MyAlpha)) { // JM
    changeToALPHA();
...
void fnKeyExit(uint16_t unusedButMandatoryParameter) {
  ...
  if (tam.mode) { // if in TAM mode
    // [DL] : TM_LABEL specific Alpha Exit logic below replaced by TM
    // generic exit logic, so it should not be needed anymore
    /*if((tam.mode == TM_LABEL || tam.mode == TM_LBLONLY) && (calcMode
      == CM_NORMAL || calcMode == CM_PEM) && getSystemFlag(FLAG_ALPHA)
      && menu(1) == -MNU_TAMALPHA && isAlphaSubmenu(0)) {     //MODJM
        popSoftmenu();
        keyActionProcessed = true;
        return;
      }
      if((tam.mode == TM_LABEL || tam.mode == TM_LBLONLY) && (calcMode
      == CM_NORMAL || calcMode == CM_PEM) && getSystemFlag(FLAG_ALPHA))
      {     //MODJM if(menu(0) == -MNU_TAMALPHA) { popSoftmenu();
          if(isAlphaSubmenu(0)) {
            popSoftmenu();
          }
        }
  ...
  case CM_NORMAL: {
    if (temporaryInformation == TI_VIEW_REGISTER) {
      ...
      if (currentMenu() == -MNU_GRAPHS && menu(1) == -MNU_PLOT_FUNC) {
        ...
        screenUpdatingMode = SCRUPD_AUTO;
      } else if (softmenuStack[0].softmenuId <=
                 1) { // MyMenu or MyAlpha is displayed
        ...
        } // If none selected, do not display any menu, keep the screen
          // blank
      } else { // jm: this is where 43S cleared an error
        popSoftmenu();
        if (currentMenu() == -MNU_MVAR &&
            ((currentSolverStatus & SOLVER_STATUS_EQUATION_MODE) ==
  ...
  case CM_AIM: {
    if (currentMenu() == -MNU_ALPHA) { // JM get out of the ALPHA menu and go to
                                       // MyM fto satisfy the old exit routines
      softmenuStack[0].softmenuId = 1; // JM
    } // JM

    if (softmenuStack[0].softmenuId <= 1 &&
        menu(1) != -MNU_ALPHA) { // MyMenu or MyAlpha is displayed
      closeAim();
#if defined(DEBUGUNDO)
  ...
  case CM_PEM: {
    ...
    if (softmenuStack[0].softmenuId > 1 &&
        currentMenu() != -MNU_PFN) { // not MyMenu and not MyAlpha
      popSoftmenu();
      break;
  ...
  case CM_EIM: {
    if (lastErrorCode != 0) {
      ...
      if (currentMenu() == -MNU_EQ_EDIT) {
        if (allFormulae[currentFormula].pointerToFormulaData != C47_NULL) {
          parseEquation(currentFormula, EQUATION_PARSER_MVAR, aimBuffer,
                        tmpString);
          if (lastErrorCode != 0) {
            deleteEquation(currentFormula);
            lastErrorCode = 0;
  ...
  case CM_ASN_BROWSER: {
    if (previousCalcMode == CM_AIM || tam.alpha) {
      ...
      screenUpdatingMode = SCRUPD_AUTO;
      calcMode = CM_NORMAL; // without changing the refresh scheme:
                            // change to normal mode to get the stack
                            // back, then switch back to CM_EIM
      refreshScreen(0);
    }
    calcMode = previousCalcMode;
    break;
      ...
      for (int16_t ii = 0; ii < 3; ii++) {
        if ((softmenuStack[0].softmenuId > 1) &&
            !((-currentMenu() == MNU_HIST) ||
              ...
              (-currentMenu() == MNU_MODEL) || (-currentMenu() == MNU_REGR))) {
          popSoftmenu();
        }
      }
    } else {
  ...
  case CM_ASSIGN: {
    if ((softmenuStack[0].softmenuId <= 1 &&
         softmenuStack[1].softmenuId <= 1) ||
        (previousCalcMode == CM_EIM &&
         currentMenu() == -MNU_EQ_EDIT)) { // MyMenu or MyAlpha is displayed
      calcMode = previousCalcMode;
      if (tam.alpha) {
        assignLeaveAlpha();
      }
  ...
  default: {
    sprintf(errorMessage, commonBugScreenMessages[bugMsgCalcModeWhileProcKey],
            "fnKeyExit", calcMode, "EXIT");
    displayBugScreen(errorMessage);
  }
  }

  last_CM = calcMode; // ignore this method of prioritising refreshes.
                      // This method is sunsetting.
  screenUpdatingMode &= ~SCRUPD_MANUAL_STATUSBAR;
  //    refreshScreen(127);
  return;
...
void fnKeyCC(uint16_t complex_Type) { // JM Using 'unusedButMandatoryParameter'
                                      // complex_Type=KEY_COMPLEX
  doRefreshSoftMenu = true;           // dr
#if !defined(TESTSUITE_BUILD)
  bool_t polarOk, rectOk;
  // The switch statement is broken up here, due to multiple conditions.
  // //JM
  ...
  if (calcMode == CM_NORMAL ||
      ((calcMode == CM_NIM) && (complex_Type == KEY_COMPLEX))) { // JM
...
#define isRadius(typ, tag)                                                     \
  (typ == dtLongInteger || (typ == dtReal34 && tag == amNone))
    if (getSystemFlag(FLAG_POLAR) && isAngle(sdataTypeY, sdataAtagY) &&
        isRadius(sdataTypeX, sdataAtagX)) {
      fnSwapXY(0);
    } else if (!getSystemFlag(FLAG_POLAR) && isAngle(sdataTypeY, sdataAtagY) &&
               isRadius(sdataTypeX, sdataAtagX)) {
      fnSwapXY(0);
      setSystemFlag(FLAG_POLAR);
      toClearPolar = true;
    } else if (!getSystemFlag(FLAG_POLAR) && isAngle(sdataTypeX, sdataAtagX) &&
               isRadius(sdataTypeY, sdataAtagY)) {
      setSystemFlag(FLAG_POLAR);
      toClearPolar = true;
    }
    ...
    polarOk = isRadius(sdataTypeY, sdataAtagY) &&
              isValidAngle(sdataTypeX, sdataAtagX) && getSystemFlag(FLAG_POLAR);
    rectOk = isRadius(sdataTypeY, sdataAtagY) &&
             isRadius(sdataTypeX, sdataAtagX) && !getSystemFlag(FLAG_POLAR);
    ...
    if (polarOk || rectOk) { // imag not allowed to be an angle if rect entry
      ...
      if ((!polarOk && getSystemFlag(FLAG_POLAR)) ||
          (!rectOk && !getSystemFlag(FLAG_POLAR))) {
        displayCalcErrorMessage(
            ERROR_INVALID_DATA_TYPE_FOR_POLAR_RECT, ERR_REGISTER_LINE,
            REGISTER_X); // Invalid input data type for this operation
      } else {
        displayCalcErrorMessage(
            ERROR_INVALID_DATA_TYPE_FOR_OP, ERR_REGISTER_LINE,
            REGISTER_X); // Invalid input data type for this operation
      }
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
      if (!polarOk && getSystemFlag(FLAG_POLAR)) {
        sprintf(errorMessage,
                "You cannot use CC or COMPLEX to create a Polar "
                "complex number with %s(%s) in X and %s(%s) in Y!",
                getDataTypeName(getRegisterDataType(REGISTER_X), true, false),
                getRegisterTagName((REGISTER_X), 0),
                getDataTypeName(getRegisterDataType(REGISTER_Y), true, false),
                getRegisterTagName((REGISTER_Y), 0));
      } else if (!rectOk && !getSystemFlag(FLAG_POLAR)) {
        sprintf(errorMessage,
                "You cannot use CC or COMPLEX to create a Rectangular "
                "complex number with %s(%s) in X and %s(%s) in Y!",
                getDataTypeName(getRegisterDataType(REGISTER_X), true, false),
                getRegisterTagName((REGISTER_X), 0),
                getDataTypeName(getRegisterDataType(REGISTER_Y), true, false),
        ...
        sprintf(errorMessage,
                "You cannot use CC or COMPLEX with %s in X and %s in Y!",
                getDataTypeName(getRegisterDataType(REGISTER_X), true, false),
                getDataTypeName(getRegisterDataType(REGISTER_Y), true, false));
  ...
  case CM_AIM: {
    if (catalog && catalog != CATALOG_MVAR) {
      if (stringByteLength(aimBuffer) > 0) {
        ...
        aimBuffer[lg] = 0;
        xCursor = showString(aimBuffer, &standardFont, 1,
                             Y_POSITION_OF_AIM_LINE + 6, vmNormal, true, true);
      }
      ...
      lg =
          stringLastGlyph(aimBuffer); // find beginning of last glyoh, to delete
      aimBuffer[lg] = 0;              // delete it
      aimBuffer[T_cursorPos] = T_cursorPos_tmp; // Restore broken glyph in
                                                // middle at break point
      uint16_t ix = 0;
      while (aimBuffer[ix + T_cursorPos] !=
             0) { // copy second part to append to first part
        aimBuffer[ix + lg] = aimBuffer[ix + T_cursorPos];
        ix++;
      }
      aimBuffer[ix + lg] = 0; // end new buffer
      // printf("newXCursor=%d  T_cursorPos=%d
      // stringLastGlyph(aimBuffer)=%d\n",newXCursor,T_cursorPos,stringLastGlyph(aimBuffer));
...
// JMCURSOR^^
  ...
  case CM_EIM: {
    if (xCursor > 0) {
      ...
      char *lstPos =
          aimBuffer + stringNextGlyph(aimBuffer, stringLastGlyph(aimBuffer));
      --xCursor;
  ...
  case CM_PEM: {
    ...
    if (getSystemFlag(FLAG_ALPHA)) {
      ...
      if (!pemCursorIsZerothStep) {
        ...
        if (*currentStep != 255 ||
            *(currentStep + 1) != 255) { // Not the last END
  ...
  default: {
    sprintf(errorMessage, commonBugScreenMessages[bugMsgCalcModeWhileProcKey],
            "fnKeyBackspace", calcMode, "BACKSPACE");
    displayBugScreen(errorMessage);
  }
  }
#endif // !TESTSUITE_BUILD
...
void fnKeyUp(uint16_t unusedButMandatoryParameter) {
  ...
  if (softmenu[menuId].menuItem != -MNU_REG &&
      softmenu[menuId].menuItem != -MNU_FLG && tam.mode && !catalog) {
    if (tam.alpha) {
      resetAlphaSelectionBuffer();
  ...
  case CM_GRAPH: {
    ...
    // make this keyActionProcessed = false; to have arrows up and down
    // placed in bufferize make arrowCasechnage true
    // JM^^
    else
      ...
      menuUp();
                ...
                calcMode == CM_NIM) &&
               (numberOfFormulae < 2 || currentMenu() != -MNU_EQN)) {
      screenUpdatingMode = SCRUPD_AUTO;
      screenUpdatingMode |= SCRUPD_SKIP_STATUSBAR_ONE_TIME;
      if (calcMode == CM_NIM) {
        closeNim();
  ...
  case CM_REGISTER_BROWSER: {
    ...
    if (rbrMode == RBR_GLOBAL) {
      currentRegisterBrowserScreen =
          modulo(currentRegisterBrowserScreen + RBR_INCDEC1,
                 LAST_GLOBAL_REGISTER_SCREEN + RBR_INCDEC1);
    } else if (rbrMode == RBR_LOCAL) {
      currentRegisterBrowserScreen =
          modulo(currentRegisterBrowserScreen - FIRST_LOCAL_REGISTER + 1,
                 currentNumberOfLocalRegisters) +
          FIRST_LOCAL_REGISTER;
    } else if (rbrMode == RBR_NAMED) {
      currentRegisterBrowserScreen =
          modulo(currentRegisterBrowserScreen - FIRST_NAMED_VARIABLE + 1,
                 numberOfNamedVariables + LAST_RESERVED_VARIABLE -
                     FIRST_RESERVED_VARIABLE -
                     (NUMBER_OF_LETTERED_VARIABLES - 1)) +
          FIRST_NAMED_VARIABLE;
    } else {
      sprintf(errorMessage, commonBugScreenMessages[bugMsgRbrMode], "fnKeyUp",
              "UP", rbrMode);
      displayBugScreen(errorMessage);
    }
    break;
  ...
  case CM_ASN_BROWSER: {
    ...
    if (currentAsnScr == 0 || currentAsnScr >= 7) {
      currentAsnScr = (previousCalcMode == CM_AIM ||
                       previousCalcMode == CM_EIM || tam.alpha)
                          ? 4
                          : 1;
    }
    break;
  ...
  case CM_MIM: {
    ...
    if (currentSoftmenuScrolls() &&
        (catalog ||
         (currentMenu() != -MNU_TAMSTO &&
          currentMenu() != -MNU_TAMRCL))) { // JM remove to allow normal
                                            // arrows to work as cursors
      menuUp();
    }
...
void fnKeyDown(uint16_t unusedButMandatoryParameter) {
  ...
  //--     if(SHOWMODE && currentMenu() != -MNU_EQN && !tam.mode) {
  ////JMSHOW vv
  ...
  case CM_REGISTER_BROWSER: {
    ...
    if (rbrMode == RBR_GLOBAL) {
      ...
      sprintf(errorMessage, commonBugScreenMessages[bugMsgRbrMode], "fnKeyDown",
              "DOWN", rbrMode);
      displayBugScreen(errorMessage);
    }
    break;
  ...
  case CM_FONT_BROWSER: {
    if (currentFntScr <
        numScreensNumericFont + numScreensStandardFont + numScreensTinyFont) {
      currentFntScr++;
    }
    break;
  }
  ...
  case CM_ASSIGN: {
    if (currentSoftmenuScrolls()) {
      menuDown();
    } else if (tam.alpha && (itemToBeAssigned == 0 || tam.mode == TM_NEWMENU) &&
               alphaCase == AC_UPPER) {
      alphaCase = AC_LOWER;
    } else if (tam.alpha && itemToBeAssigned == 0 && aimBuffer[0] == 0) {
      assignLeaveAlpha();
...
void setLastKeyCode(int key) { // key = 10      setLastKeyCode = 10-6
                               // +30 = 34 row 3, col 4
  if (1 <= key && key <= 43) { // key = 27      setLastKeyCode =
                               // 27-22+60 = 65 row 6, col 5
    if (key <= 6) { // key = 28 EXIT setLastKeyCode = 28-27+70 = 71 row
                    // 7, col 1
      lastKeyCode = key + 20;
    } else if (key <= 12) {
