...
// undefine FIXED_FN_NAME_SHIFT to let the function name move to the left edge
// with the shift
...
TO_QSPI static const char whoStr1[] =
    "C47 & R47 Development since 2019" spc "by" spc1 "\n"
    "Ben" spc "GB," spc1 "D" spc "A" spc "CA," spc1 "Dani" spc "CH," spc1
    "Didier" spc "FR," spc1 "\n"
    "H" STD_a_RING "kon" spc "NO," spc1 "Jaco" spc "ZA," spc1 "Martin" spc
    "FR," spc1 "Mihail" spc "JP," spc1 "\n"
    "Pauli" spc "AU," spc1 "RJvM" spc "NL," spc1 "Walter" spc "DE.";

TO_QSPI static const char disclaimerStr[] =
    "  " MODELTEXT " firmware is free, open source and \n  neither provided "
    "nor supported by \n  SwissMicros. Press a key to continue.";

TO_QSPI static const char versionStr[] = "  " MODELTEXT " " VERSION_STRING ".";
#if defined(PC_BUILD)

TO_QSPI static const char versionStr2[] =
    "  " MODELTEXT " Sim " VERSION1 ", dated " __DATE__ ".";
#else // !PC_BUILD
#if defined(TWO_FILE_PGM)
TO_QSPI static const char versionStr2[] =
    "  " MODELTEXT " QSPI " VERSION1 ", dated " __DATE__ ".";
#else // !TWO_FILE_PGM
#if !defined(TWO_FILE_PGM)
TO_QSPI static const char versionStr2[] =
    "  " MODELTEXT " No QSPI " VERSION1 ", dated " __DATE__ ".";
#endif // !TWO_FILE_PGM
#endif // TWO_FILE_PGM
#endif // PC_BUILD
...
TO_QSPI static const nstr nameOfWday_en[8] = {{"invalid day of week"},
                                              {"Monday"},
                                              {"Tuesday"},
                                              {"Wednesday"},
                                              {"Thursday"},
                                              ...
                                              {"Sunday"}};
/*
TO_QSPI static const char *nameOfWday_de[8] = {"ung" STD_u_DIARESIS "ltiger
Wochentag",                 "Montag",            "Dienstag", "Mittwoch",
"Donnerstag",         "Freitag",            "Samstag",              "Sonntag"};
TO_QSPI static const char *nameOfWday_fr[8] = {"jour de la semaine invalide",
"lundi",             "mardi",                       "mercredi", "jeudi",
"vendredi",           "samedi",               "dimanche"}; TO_QSPI static const
char *nameOfWday_es[8] = {"d" STD_i_ACUTE "a inv" STD_a_ACUTE "lido de la
semana", "lunes",             "martes",                      "mi" STD_e_ACUTE
"rcoles", "jueves",             "viernes",            "s" STD_a_ACUTE "bado",
"domingo"}; TO_QSPI static const char *nameOfWday_it[8] = {"giorno della
settimana non valido",                     "luned" STD_i_GRAVE, "marted"
STD_i_GRAVE,          "mercoled" STD_i_GRAVE,    "gioved" STD_i_GRAVE, "venerd"
STD_i_GRAVE, "sabato",               "domenica"}; TO_QSPI static const char
*nameOfWday_pt[8] = {"dia inv" STD_a_ACUTE "lido da semana", "segunda-feira",
"ter" STD_c_CEDILLA "a-feira", "quarta-feira",            "quinta-feira",
"sexta-feira",        "s" STD_a_ACUTE "bado", "domingo"};
*/
#endif // !TESTSUITE_BUILD
...
gboolean drawScreen(GtkWidget *widget, cairo_t *cr, gpointer data) {
  ...
  imageSurface = cairo_image_surface_create_for_data(
      (unsigned char *)screenData, CAIRO_FORMAT_RGB24, SCREEN_WIDTH,
      SCREEN_HEIGHT, screenStride * 4);
...
void copyScreenToClipboard(void) {
  ...
  gtk_clipboard_set_text(
      clipboard, "",
      0); // JM FOUND TIP TO PROPERLY CLEAR CLIPBOARD:
          // https://stackoverflow.com/questions/2418487/clear-the-system-clipboard-using-the-gtk-lib-in-c/2419673#2419673

  imageSurface = cairo_image_surface_create_for_data(
      (unsigned char *)screenData, CAIRO_FORMAT_RGB24, SCREEN_WIDTH,
      SCREEN_HEIGHT, screenStride * 4);
  gtk_clipboard_set_image(
      clipboard, gdk_pixbuf_get_from_surface(imageSurface, 0, 0, SCREEN_WIDTH,
                                             SCREEN_HEIGHT));
}
...
void copyRegisterToClipboardString(calcRegister_t regist,
  ...
  case dtString:
    xcopy(string, REGISTER_STRING_DATA(regist),
          stringByteLength(REGISTER_STRING_DATA(regist)) + 1);
    break;
  ...
  case dtReal34Matrix: {
    ...
    for (uint32_t i = 0; i < rows * columns; i++) {
      ...
      if (strchr(string + len, '.') == NULL &&
          strchr(string + len, 'E') == NULL) {
        strcat(string + len, ".");
      }
    }
    break;
  ...
  case dtShortInteger:
    ...
    sprintf(errorMessage + n--, "#%d (word size = %u)", base,
  ...
  case dtReal34: {
    ...
    angularUnitToString(getRegisterAngularMode(regist),
  ...
  default:
    sprintf(string,
            "In function copyRegisterXToClipboard, the data type %" PRIu32
            " is unknown! Please try to reproduce and submit a bug.",
...
void copyAllRegistersToClipboard(void) {
  ...
  for (int32_t regist = currentNumberOfLocalRegisters - 1; regist >= 0;
       --regist) {
    ptr += strlen(ptr);
    sprintf(ptr, LINEBREAK "R.%02d = ", regist);
    ptr += strlen(ptr);
...
gboolean
refreshLcd(gpointer unusedData) { // This function is called every
                                  // SCREEN_REFRESH_PERIOD ms by a GTK timer
  // Cursor blinking
  ...
  if (cursorEnabled) {
    if (++cursorBlinkCounter > cursorCycle) { // JM cursor vv
      ...
      if (cursorBlink && !checkHP) {
        showGlyph(STD_CURSOR, cursorFont, xCursor, yCursor - checkHPoffset,
                  vmNormal, true, false, false);
      } // JM cursor ^^
      else {
        hideCursor();
      }
  ...
  if (showFunctionNameCounter > 0) {
    ...
    if (showFunctionNameCounter <= 0) {
      ...
      bool_t isMultiPageMenu =
          (softmenu[softmenuStack[0].softmenuId].numItems > 6);
...
#define cursorCycle 3 // JM cursor vv
int8_t cursorBlinkCounter; // JM cursor ^^
void refreshLcd(void) {    // This function is called roughly every
                           // SCREEN_REFRESH_PERIOD ms from the main loop
  // Cursor blinking
  static bool_t cursorBlink = true;
...
static inline uint16_t getLine_buffer_bit(int x) { return 415 - x; }

uint16_t yUnderlined = 3;
void underline_softkey(uint16_t xSoftkeyMask, uint16_t ySoftkey) {
  if (calcMode == CM_REGISTER_BROWSER || calcMode == CM_FLAG_BROWSER ||
      calcMode == CM_FONT_BROWSER ||
      (!getSystemFlag(FLAG_FGLNFUL) && !getSystemFlag(FLAG_FGLNLIM))) {
    return;
  }
  ...
  for (xIndex = 0; xIndex < 6; xIndex++) {
    ...
    xBg[xIndex] =
        (lcd_buffer[52 * (maxLine + 1) + buff_bit / 8] >> mod(buff_bit, 8)) &
        1u;
  }
...
void FN_handler_StepToF(uint32_t time) {
  ...
  showShiftState();
  if (calcMode != CM_PEM) {
    refreshRegisterLineRestoreT(); // clearRegisterLine(Y_POSITION_OF_REGISTER_T_LINE
                                   // - 4, REGISTER_LINE_HEIGHT); //JM FN clear
                                   // the previous shift function name
  }
...
void FN_handler_StepToNOP(void) {
  ...
  underline_softkey(1 << (FN_key_pressed - 38),
                    3); //  Purposely select row 3 which does not exist, just to
                        //  activate the 'clear previous line'
  FN_timeouts_in_progress = false;
  fnTimerStop(TO_FN_LONG); // dr
}

void FN_handler(
    void) { // JM FN LONGPRESS vv Handler FN Key shift longpress handler
            //    Processing cycles here while the key is pressed, that is,
            //    after PRESS #1, waiting for RELEASE #2
  if ((FN_state == ST_1_PRESS1 || FN_state == ST_3_PRESS2) &&
      FN_timeouts_in_progress && (FN_key_pressed != 0) &&
      !IS_BASEBLANK_(softmenuStack[0].softmenuId)) {
    if (fnTimerGetStatus(TO_FN_LONG) == TMR_COMPLETED) {
      ...
      if (!shiftF && !shiftG) { // From No_Shift State 1
        ...
        printf("Handler 1, KEY=%d =%i\n", FN_key_pressed,
               nameFunction(FN_key_pressed - 37, shiftF, shiftG));
        ...
        printf("Handler 2, KEY=%d =%i\n", FN_key_pressed,
               nameFunction(FN_key_pressed - 37, shiftF, shiftG));
                 ...
                 (shiftF && shiftG)) { // From G: 3 (or illegal state FG)
        FN_handler_StepToNOP();        // To NOP State 4
#if defined(FN_TIME_DEBUG1)
...
void _executeItem(int16_t item, int keyCode) {
  ...
  funcParam =
      (char *)getNthString((uint8_t *)userKeyLabel, keyCode * 6 + keyStateCode);
...
static void clearShiftTemporaryIndications(bool_t condition) {
  if (isShift(currentKeyCode) && (temporaryInformation != TI_NO_INFO) &&
      condition) {
...
void Shft_handler() {
  if (Shft_LongPress_f_g) {
    if (fnTimerGetStatus(TO_FG_LONG) == TMR_COMPLETED) {
      ...
      int16_t item;
      if ((shiftF || shiftG) // this is for R47 ShiftF or shiftG, adding long
                             // press assignments - DL 2025/12/03
          && (calcMode != CM_EIM) &&
          (calcMode !=
           CM_MIM)) { // f and g longpress temporarily disabled in EIM and MIM
        // leaveTamModeIfEnabled();
        ...
        if ((calcMode == CM_ASSIGN) && (itemToBeAssigned != 0)) {
          ...
          if (calcMode == CM_NIM && item != ITM_ms && item != ITM_CC &&
              item != ITM_op_j && item != ITM_op_j_pol && item != ITM_dotD &&
          ...
          // USER mode
          if (getSystemFlag(FLAG_USER) && (calcMode != CM_AIM) &&
              (calcMode != CM_EIM) && !getSystemFlag(FLAG_ALPHA) &&
              (item > 0)) {
            if ((calcMode == CM_NIM ||
                 (calcMode == CM_PEM && aimBuffer[0] != 0 &&
              ...
              clearShiftTemporaryIndications((item != ITM_SNAP) &&
                                             (shiftG || shiftF));
            ...
            char *funcParam = "";
            keyStateCode = (getSystemFlag(FLAG_ALPHA) ? 3 : 0) + 2;
            funcParam = (char *)getNthString((uint8_t *)userKeyLabel,
                                             keyCode * 6 + keyStateCode);
            setCurrentUserMenu(item, funcParam);
            if (shiftF) {
              if (getSystemFlag(FLAG_ALPHA) &&
                  ((currentMenu() == -MNU_MyAlpha) ||
                   (currentMenu() == -MNU_AIMCATALOG) ||
                   isAlphabeticSoftmenu())) {
                popSoftmenu();
              }
              // showSoftmenu((calcMode == CM_AIM) || ((calcMode == CM_ASSIGN)
              // && (previousCalcMode == CM_AIM)) || tam.alpha ? -MNU_ALPHA :
              //              (getSystemFlag(FLAG_USER) && (key->gShifted !=
              //              ITM_NULL) ? key->gShifted : -MNU_HOME));

              if (tam.alpha) {
                showSoftmenu(-MNU_TAMALPHA);
              } else if ((calcMode == CM_AIM) || getSystemFlag(FLAG_ALPHA) ||
                         ((calcMode == CM_ASSIGN) &&
                          (previousCalcMode == CM_AIM))) {
                showSoftmenu(-MNU_ALPHA);
              } else if (getSystemFlag(FLAG_USER) &&
                         (key->gShifted != ITM_NULL)) {
              ...
              if ((calcMode == CM_AIM) || getSystemFlag(FLAG_ALPHA) ||
                  ((calcMode == CM_ASSIGN) && (previousCalcMode == CM_AIM)) ||
                  tam.alpha) {
                ...
                showSoftmenu(key->gShifted);
                         ...
                         getSystemFlag(FLAG_BASE_HOME)) {
                showSoftmenu(-MNU_MyMenu);
              } else {
                baseOverrideOnce = false;
                BASE_OVERRIDEONCE = baseOverrideOnce;
                fnExitAllMenus(0); // If MyMb and HOMEb are both clear, return
                                   // to the blank base menu display
              }
              BASE_OVERRIDEONCE = baseOverrideOnce;
              showSoftmenuCurrentPart();
    ...
    clearShiftTemporaryIndications(
        shiftG); // clear TI when arriving here, when longpress is timed out,
                 // and clear while on g
    if (fnTimerGetStatus(TO_FG_LONG) == TMR_COMPLETED) {
      fnTimerStop(TO_3S_CTFF);
      if (!shiftF && !shiftG) {
        shiftF = true;
...
#if defined(LONGPRESS_CFG) // only when allowed by LONGPRESS_CFG
          int keyCode =
              (calcModel == USER_R47bk_fg)                                ? 11
              : (calcModel == USER_R47fg_bk || calcModel == USER_R47fg_g) ? 10
              : (calcModel == USER_C47 || calcModel == USER_DM42)         ? 27
                                                                  : 9999;
          // shiftF = 1;
          if (previousCalcMode != CM_AIM) { // No long press assignments in AIM
            _assignLongPressKey(keyCode);
...
//   Although it does not really matter because this section is for
//   non-assignable, i.e. hardware keys

// 3. 2025-08-02
//  The 'exclude ENTER and BACKSPACE' section below has UP and DN longpress
//  added. This must be checked anyway in PEM.
//    ... || (calcMode == CM_PEM && getSystemFlag(FLAG_ALPHA) && !tam.mode) to
//    be added to 'if((calcMode == CM_AIM || calcMode == CM_EIM) &&'
...
void LongpressKey_handler() {
  if (fnTimerGetStatus(TO_CL_LONG) == TMR_COMPLETED) {
    if (JM_auto_longpress_enabled != 0) {
      char *funcParam;
      int keyStateCode =
          (getSystemFlag(FLAG_ALPHA) ? 3 : 0) + ((LongPressM == RBX_M124) ? 1
                                                 : longpressDelayedkey3   ? 1
                                                                          : 2);
      funcParam = (char *)getNthString((uint8_t *)userKeyLabel,
                                       currentKeyCode * 6 + keyStateCode);

      if (calcMode == CM_NORMAL && programRunStop == PGM_STOPPED &&
          (isArrowUp(currentKeyCode))) {
        aimBuffer[0] = 0;
        fnSkip(0);
      ...
      } else if (calcMode == CM_NORMAL && programRunStop == PGM_SINGLE_STEP &&
                 (isArrowDown(currentKeyCode))) {
      ...
      // printf("LongpressKey_handler = %d %s
      // currentKeyCode=%d\n",JM_auto_longpress_enabled,
      // indexOfItems[abs(JM_auto_longpress_enabled)].itemCatalogName,
      // currentKeyCode);
      if ((calcMode == CM_AIM || calcMode == CM_EIM || tam.alpha) &&
          !((currentKeyCode == 16 || currentKeyCode == 12)) &&
          JM_auto_longpress_enabled != ITM_CLRMOD &&
          JM_auto_longpress_enabled >
              0) { // using keyboard positions, as these cannot be re-assigned.
                   // It should not work with re-assigned keys on different
                   // places.
        ...
        if (calcMode == CM_AIM) {
          refreshRegisterLine(
              AIM_REGISTER_LINE); // TO DISPLAY KEYPRESS DIRECTLY AFTER PRESS,
                                  // NOT ONLY UPON RELEASE
        } else if (calcMode == CM_EIM || tam.alpha) {
          screenUpdatingMode &=
              ~(SCRUPD_MANUAL_MENU | SCRUPD_SKIP_MENU_ONE_TIME);
          refreshScreen(1312);
        }
        return;
      } else if ((funcParam[0] != 0) &&
                 ((JM_auto_longpress_enabled == -MNU_DYNAMIC) ||
                  (JM_auto_longpress_enabled == ITM_XEQ) ||
                  (JM_auto_longpress_enabled ==
                   ITM_RCL))) { // For user menu, prog or variable
                                // a-feirassignment
        showFunctionName(JM_auto_longpress_enabled, JM_TO_CL_LONG + 50,
                         funcParam); // Add a marginal amout of time to prevent
                                     // racing of end conditions.
      } else if (funcParam[0] == 0 && (JM_auto_longpress_enabled == ITM_XEQ ||
                                       JM_auto_longpress_enabled ==
                                           ITM_GTO)) { // from KEYA-F longpress
        showFunctionName(JM_auto_longpress_enabled, JM_TO_CL_LONG + 50,
                         funcParam); // Add a marginal amout of time to prevent
                                     // racing of end conditions.
      } else {
        showFunctionName(JM_auto_longpress_enabled, JM_TO_CL_LONG + 50,
                         "SF:LL"); // Add a marginal amout of time to prevent
                                   // racing of end conditions.
      }
      JM_auto_longpress_enabled =
          0; // showFunctionName must not time out longer than the timer that is
             // started below
...
// JM: If maxiC is set, then, if a glyph is not found in numericfont, it will be
// fetched and enlarged from standardfont
uint8_t combinationFonts = combinationFontsDefault;
uint8_t miniC = 0; // JM miniature letters
uint8_t maxiC =
    0; // JM ENLARGE letters. Use Numericfont & combinationFontsDefault=2;
...
//   return ((bool_t) (
//     ((charCode >= str2dec(STD_SUP_BOLD_f)) && (charCode <=
//     str2dec(STD_SUP_BOLD_h))) || ( charCode == str2dec(STD_SUP_BOLD_r)) || (
...
uint32_t showGlyphCode(uint16_t charCode, const font_t *font, uint32_t x,
                       uint32_t y, videoMode_t videoMode,
                       bool_t showLeadingCols, bool_t showEndingCols,
                       bool_t noPreClear) {
  ...
  if (charCode == STD_NOCHAR) {
    return x; // This is special usage of the 01 ASCII code, to ignore the code
              // and return with nothing printed
  }
  ...
  if (combinationFonts == stdnumEnlarge || combinationFonts == numHalf) {
    if (maxiC == 1 && font == &numericFont) { // JM allow enlargements
      ...
      // printf("DDDDDD %d %d --- %u\n",glyphId, charCode,
      // ratherUseEnlargement(charCode));
      if (glyphId <
          0) { // || ratherUseEnlargement(charCode)) {           //JM if there
               // is not a large glyph, enlarge the small letter
        enlarge = true;
        font = &standardFont;
      }
    }
  ...
  if (glyph == NULL) {
    sprintf(errorMessage,
            commonBugScreenMessages[bugMsgValueReturnedByFindGlyph],
            "showGlyphCode", glyphId);
    displayBugScreen(errorMessage);
    return 0;
  }

  int32_t yy = (int32_t)y; // get unsigned integer that was negative, back to
  ...
  bool_t numDouble =
      font == &numericFont && checkHP &&
      temporaryInformation ==
          TI_NO_INFO; //&& charCodeFromString(STD_MODE_G, 0)!=charCode &&
                      // charCodeFromString(STD_MODE_G, 0)!=charCode; //this
                      // also triggers the vertical doubling
  uint16_t doubling =
      numDouble ? DOUBLING : DOUBLINGBASEX; // this is the horizontal factor, 8
                                            // is normal, so 16 is double

  // Clearing the space needed by the glyph
  bool_t rep_enlarge =
      numDouble || (enlarge && combinationFonts != 0); // JM ENLARGE
  uint32_t yNewMaxDx =
      (rep_enlarge ? 2 : 1) *
      (((glyph->rowsAboveGlyph + glyph->rowsGlyph + glyph->rowsBelowGlyph) >>
        miniC) -
       (rep_enlarge ? 4 : 0));
  if (!noShow && !noPreClear) {
    lcd_fill_rect(
        x, max(0, yy),
        (uint32_t)(doubling *
                   ((xGlyph + glyph->colsGlyph + endingCols) >> miniC)) >>
            3,
        max(0, (int32_t)(yNewMaxDx) + (yy < 0 ? yy : 0)),
        (videoMode == vmNormal ? LCD_SET_VALUE : LCD_EMPTY_VALUE)); // JMmini
  }
  if (displaymode == numHalf) {
    y += (uint32_t)(glyph->rowsAboveGlyph * REDUCT_A / REDUCT_B *
                    (rep_enlarge ? 2 : 1));
  } else {
  ...
  // Choose pencil
  void (*setPixel)(uint32_t, uint32_t) =
      (videoMode == vmNormal) ? &setBlackPixel : &setWhitePixel;
  // Drawing the glyph
  for (row = 0; row < glyph->rowsGlyph; row++, y++) {
    ...
    for (col = 0; col < glyph->colsGlyph; col++) {
      ...
      if (byte & 0x80 && !noShow) { // MSB set
        ...
        uint32_t y1 =
            min(SCREEN_HEIGHT - 1,
                max(0, yy + (int32_t)min(yNewMaxDx, ((y - y0) >> miniC))));
        uint32_t y2 =
            min(SCREEN_HEIGHT - 1,
                max(0, yy + (int32_t)min(yNewMaxDx, 1 + ((y - y0) >> miniC))));
        if (x2 > 0) {
          x2--;
        }
        setPixel(x1, y1);
    ...
    if (rep_enlarge && row != 3 && row != 6 && row != 9 && row != 12) {
      y++; // JM ENLARGE vv do not advance the row counter for four rows, to
           // match the row height of the enlarge font
    }
  }
  return x + boldString +
         (((doubling * (xGlyph + glyph->colsGlyph + endingCols)) >> miniC) >>
          3); // JMmini
}

uint32_t showGlyph(const char *ch, const font_t *font, uint32_t x, uint32_t y,
                   videoMode_t videoMode, bool_t showLeadingCols,
                   bool_t showEndingCols, bool_t noPreClear) {
  return showGlyphCode(charCodeFromString(ch, 0), font, x, y, videoMode,
                       showLeadingCols, showEndingCols, noPreClear);
}

/* Finds the cols and rows for a glyph.
 *
 * \param[in]     ch     const char*   String whose first glyph is to find the
 * bounds for
 * \param[in,out] offset uint16_t*     Offset for string or null if zero should
 * be used
 ...
 */
static void getGlyphBounds(const char *ch, uint16_t *offset, const font_t *font,
 ...
 */
void getStringBounds(const char *string, const font_t *font, uint32_t *col,
                     uint32_t *row) {
  uint16_t ch = 0;
  uint32_t lcol, lrow;
  lcol = 0;
...
static uint32_t _doShowString(const char *string, const font_t *font,
                              uint32_t x, uint32_t y, char **resStr,
                              uint32_t width, videoMode_t videoMode,
                              bool_t showLeadingCols, bool_t showEndingCols,
                              bool_t LF) {
  uint16_t ch, lg;
  bool_t slc, sec;
  ...
  while (string[ch] != 0) {
    if (lg == 1 ||
        (lg == 2 && (string[0] & 0x80))) { // The string is 1 glyph long
      slc = showLeadingCols;
      sec = showEndingCols;
      ...
      sec = true;
               ...
               (ch == lg - 2 && (string[ch] & 0x80))) { // Last glyph
      slc = true;
      sec = showEndingCols;
    ...
    if (LF && x > SCREEN_WIDTH - 20 && !noShow) { // auto LF when line is full
      ...
      uint16_t tmp = ch;
      if (x +
              showGlyphCode(charCodeFromString(string, &tmp), font, 0, 0,
                            videoMode, slc, sec, false) -
              compressString >
          SCREEN_WIDTH) {
        x = orgX;
        prevX = x;
    ...
    x = showGlyphCode(charCodeFromString(string, &ch), font, x, y - raiseString,
                      videoMode, slc, sec, false) -
        compressString;
    if (resStr) { // for stringAfterPixelsC47
      if (x > width) {
        if (!showEndingCols) {
          uint32_t tmpX = x;
          ch = *resStr - string;
          x = showGlyphCode(charCodeFromString(string, &ch), font, prevX,
                            y - raiseString, videoMode, true, false, false) -
              compressString;
          if (x <= width) {
    ...
    while (LF && (charCodeFromString(string, &tmp) ==
                  0x0A)) { // do not touch character pointer
      charCodeFromString(string,
                         &ch); // increment character pointer to skip 0x0A
      x = orgX;
      prevX = x;
      y += (font == &tinyFont ? 8 : 20);
    }
...
uint32_t showStringEnhanced(const char *string, const font_t *font, uint32_t x,
                            uint32_t y, videoMode_t videoMode,
                            bool_t showLeadingCols, bool_t showEndingCols,
                            uint8_t compress1, uint8_t raise1, uint8_t noShow1,
                            uint8_t boldString1, bool_t lf) {
  boldString = boldString1;
  compressString = compress1;
  raiseString = raise1;
  noShow = noShow1;
  uint32_t tmp = _doShowString(string, font, x, y, NULL, 0, videoMode,
                               showLeadingCols, showEndingCols, lf);
  boldString = 0;
  compressString = 0;
  raiseString = 0;
  noShow = 0;
...
uint32_t showString(const char *string, const font_t *font, uint32_t x,
                    uint32_t y, videoMode_t videoMode, bool_t showLeadingCols,
                    bool_t showEndingCols) {
  return _doShowString(string, font, x, y, NULL, 0, videoMode, showLeadingCols,
                       showEndingCols, NO_LF);
}

static char *_stringAfterPixels(const char *string, const font_t *font,
                                uint32_t width, bool_t showLeadingCols,
                                bool_t showEndingCols) {
  char *resStr = (char *)string;
  _doShowString(string, font, 0, 0, &resStr, width, vmNormal, showLeadingCols,
                showEndingCols, NO_LF);
  return resStr;
}

static uint32_t _showStringWithLimit(const char *string, const font_t *font,
                                     uint32_t limitWidth,
                                     bool_t showLeadingCols,
                                     bool_t showEndingCols) {
  char *resStr = (char *)string;
  return _doShowString(string, font, 0, 0, &resStr, limitWidth, vmNormal,
                       showLeadingCols, showEndingCols, NO_LF);
}

static void _setStringMode(int mode, int comp, const font_t **fontPtr) {
  compressString = comp;
  displaymode =
      mode; // miniC and maxiC to be depreciated in favour of displaymode
  switch (mode) {
  case stdNoEnlarge:
    miniC = 0;
    maxiC = 0;
  ...
  case stdEnlarge:
    miniC = 0;
    maxiC = 1;
    combinationFonts = stdEnlarge;
    *fontPtr = &standardFont;
    break;
  case stdnumEnlarge:
    miniC = 0;
    maxiC = 1;
    combinationFonts = stdnumEnlarge;
    *fontPtr = &numericFont;
    break;
  case numSmall:
    miniC = 1;
...
uint32_t showStringC47(const char *string, int mode, int comp, uint32_t x,
                       uint32_t y, videoMode_t videoMode,
                       bool_t showLeadingCols, bool_t showEndingCols) {
  ...
  if (font) {
    x = showString(string, font, x, y, videoMode, showLeadingCols,
                   showEndingCols);
  } else {
    x = 0;
  }
...
char *stringAfterPixelsC47(const char *string, int mode, int comp,
                           uint32_t width, bool_t withLeadingEmptyRows,
                           bool_t withEndingEmptyRows) {
  int combinationFontsM = combinationFonts;
  char *resStr = (char *)string;
  if (combinationFontsDefault == 0) {
    mode = stdNoEnlarge;
  ...
  _setStringMode(mode, comp, &font);
  if (font) {
    resStr = _stringAfterPixels(string, font, width, withLeadingEmptyRows,
                                withEndingEmptyRows);
  } else {
    resStr = (char *)string;
  }
...
uint32_t stringWidthWithLimitC47(const char *string, int mode, int comp,
                                 uint32_t limitWidth,
                                 bool_t withLeadingEmptyRows,
                                 bool_t withEndingEmptyRows) {
  int combinationFontsM = combinationFonts;
  uint32_t x = 0;
  if (combinationFontsDefault == 0) {
  ...
  _setStringMode(mode, comp, &font);
  if (font) {
    x = _showStringWithLimit(string, font, limitWidth, withLeadingEmptyRows,
                             withEndingEmptyRows);
  } else {
    x = 0;
  }
...
uint32_t stringWidthC47(const char *str, int mode, int comp,
                        bool_t withLeadingEmptyRows,
                        bool_t withEndingEmptyRows) {
  noShow = true;
  return showStringC47(str, mode, comp, 0, 0, vmNormal, withLeadingEmptyRows,
                       withEndingEmptyRows);
...
void showBottomLine(void) {
  ...
  if (!((temporaryInformation == TI_SHOW_REGISTER_SMALL &&
         tmpString[5 * SHOWLineSize] != 0) ||
        (temporaryInformation == TI_SHOW_REGISTER_TINY &&
         tmpString[14 * SHOWLineSize] != 0)) // The softmenu space is not used
      || (overrideShowBottomLine > 0)) {

    if (overrideShowBottomLine > 0) {
      yoff = SCREEN_HEIGHT - REGISTER_LINE_HEIGHT * (overrideShowBottomLine) /
                                 10.0f; // 40 means 4.0 registers from bottom
    } else {
      yoff = SCREEN_HEIGHT - REGISTER_LINE_HEIGHT * 2;
    }

    int offs = (temporaryInformation == TI_SHOW_REGISTER_BIG     ? -2
                : temporaryInformation == TI_SHOW_REGISTER_SMALL ? 0
                : temporaryInformation == TI_SHOW_REGISTER_TINY  ? 0
                                                                 : 0);
...
#define line_tiny 10
  ...
  if (tmpString[offset]) {
    uint32_t w = stringWidth(tmpString + offset,
                             temporaryInformation == TI_SHOW_REGISTER_SMALL
                                 ? &standardFont
                                 : &tinyFont,
                             true, true);
    showString(tmpString + offset,
               temporaryInformation == TI_SHOW_REGISTER_SMALL ? &standardFont
                                                              : &tinyFont,
               SCREEN_WIDTH - w,
               line_hMultiLineEdOffset +
                   (temporaryInformation == TI_SHOW_REGISTER_SMALL
                        ? line_small
                        : line_tiny) *
                       _h1,
...
#define line_h1 37
  ...
  uint32_t w = stringWidthWithLimitC47(tmpString + offset, stdnumEnlarge,
                                       nocompress, SCREEN_WIDTH, true, true);
  if (w < SCREEN_WIDTH) {
    showStringC47(tmpString + offset, stdnumEnlarge, nocompress,
                  SCREEN_WIDTH - w, line_hMultiLineEdOffset + line_h1 * _h1,
                  vmNormal, true, true);
    return;
  }

  w = stringWidthWithLimitC47(tmpString + offset, stdEnlarge, nocompress,
                              SCREEN_WIDTH, true, true);
  if (w < SCREEN_WIDTH) {
    showStringC47(tmpString + offset, stdEnlarge, nocompress, SCREEN_WIDTH - w,
                  line_hMultiLineEdOffset + line_h1 * _h1, vmNormal, true,
                  true);
    return;
  }

  w = stringWidthWithLimitC47(tmpString + offset, stdNoEnlarge, nocompress,
                              SCREEN_WIDTH, true, true);
  if (w < SCREEN_WIDTH) {
    showStringC47(tmpString + offset, stdNoEnlarge, nocompress,
                  SCREEN_WIDTH - w, line_hMultiLineEdOffset + line_h1 * _h1,
                  vmNormal, true, true);
    return;
  }

  w = stringWidthWithLimitC47(tmpString + offset, numSmall, nocompress,
                              SCREEN_WIDTH, true, true);
  if (w < SCREEN_WIDTH) {
    showStringC47(tmpString + offset, numSmall, nocompress, SCREEN_WIDTH - w,
                  line_hMultiLineEdOffset + line_h1 * _h1, vmNormal, true,
                  true);
    return;
  }

  w = stringWidthWithLimitC47(tmpString + offset, numSmall, DO_compress,
                              SCREEN_WIDTH, true, true);
  if (w < SCREEN_WIDTH) {
    showStringC47(tmpString + offset, numSmall, DO_compress, SCREEN_WIDTH - w,
                  line_hMultiLineEdOffset + line_h1 * _h1, vmNormal, true,
                  true);
    return;
  }
  ...
  if (w < SCREEN_WIDTH) {
    showString(tmpString + offset + 2, &standardFont, SCREEN_WIDTH - w,
               line_hMultiLineEdOffset + line_h1 * _h1, vmNormal, true, true);
    return;
  }
}

#if defined(TEXT_MULTILINE_EDIT)
uint32_t showStringEdC47(uint32_t lastline, int16_t offset, int16_t edcursor,
                         const char *string, uint32_t x, uint32_t y,
                         videoMode_t videoMode, bool_t showLeadingCols,
                         bool_t showEndingCols, bool_t noshow1) {
  ...
  if (stringWidth(string + offset, &numericFont, showLeadingCols,
                  showEndingCols) >
      SCREEN_WIDTH - 50) { // jump from large letters to small letters
    multiEdLines = 3;
    yMultiLineEdOffset = 1;
    screenUpdatingMode &= ~SCRUPD_MANUAL_STACK;
    last_CM = calcMode; // ignore this method of prioritising refreshes. This
                        // method is sunsetting.
  } else {
  ...
  while (string[ch] != 0) {
    ...
    if (lg == 1 ||
        (lg == 2 && (string[offset] & 0x80))) { // The string is 1 glyph long
      slc = showLeadingCols;
      sec = showEndingCols;
    } else if (ch == 0) { // First glyph
      slc = showLeadingCols;
    ...
    numPixels += glyph->colsGlyph + glyph->colsAfterGlyph +
                 glyph->colsBeforeGlyph; // get width of glyph
    // printf(">>> lastline=%d string[ch]=%d x=%d numPixels=%d", lastline,
    // string[ch], x, numPixels);
    if (string[ch] == 0) {
      numPixels += 8;
    }
...
#define ALLOW_PIXELS_FOR_CURSOR 12
    if (x + numPixels > SCREEN_WIDTH - 1 - ALLOW_PIXELS_FOR_CURSOR &&
        lastline == orglastlines) {
      x = xMultiLineEdOffset;
      y += yincr;
      lastline--;
    } else if (x + numPixels > SCREEN_WIDTH - 1 - ALLOW_PIXELS_FOR_CURSOR &&
               lastline > 1) {
      x = 1;
      y += yincr;
      lastline--;
...
#undef ALLOW_PIXELS_FOR_CURSOR
    ...
    maxiC = 1; // JM
    if (y != (uint32_t)(-100)) {
      x = showGlyphCode(charCode, font, x, y - raiseString, videoMode, slc, sec,
                        false) -
          compressString; // JM compressString
    }
    maxiC = 0; // JM
  }
  // printf("\n");
...
void findOffset(void) { // C47 JM
  int32_t strWidth = (int32_t)stringWidthC47(aimBuffer, combinationFonts,
                                             nocompress, true, true);
  strWidth -= SCREEN_WIDTH * multiEdLines - 45;
  if (strWidth < 0) {
    strWidth = 0;
  }
  char *offset = stringAfterPixelsC47(aimBuffer, combinationFonts, nocompress,
                                      strWidth, true, true);
...
void incOffset(void) { // C47 JM
  if ((int32_t)stringWidthC47(aimBuffer + displayAIMbufferoffset,
                              combinationFonts, nocompress, true, true) -
          (int32_t)stringWidthC47(aimBuffer + T_cursorPos, combinationFonts,
                                  nocompress, true, true) >
...
static bool_t _force_refresh(uint8_t mode) {
  ...
  if (mode != force || blockForcedRefreshes) {
    ...
    itIsTime = ((now >> 6) & 0x0001) ==
               secTick1; // ms/1024, that is every second, flips secTick1
    if (itIsTime) {
      secTick1 = !secTick1;
    }
  }

  if (((mode == force && !blockForcedRefreshes) || itIsTime) &&
      getSystemFlag(
          FLAG_MONIT)) { // Restrict refresh to once per period. Use this
                         // minimally, due to extreme slow response.
#if defined(ANALYSE_REFRESH) && defined(PC_BUILD)
    printf("-\n");
#endif // ANALYSE_REFRESH
    return true;
...
static void force_Registerrefresh(calcRegister_t regist, bool_t clearTop,
                                  bool_t clearBottom) {
  if (REGISTER_X <= regist && regist <= REGISTER_T) {
    uint32_t yStart = Y_POSITION_OF_REGISTER_X_LINE -
                      REGISTER_LINE_HEIGHT * (regist - REGISTER_X);
    uint32_t height = 32;
...
static bool_t _printHalfSecUpdate_Integer(uint8_t mode, char *txt, int32_t loop,
                                          bool_t clearZ, bool_t clearT,
                                          bool_t disp) {
  char tmps[100];
  bool_t ret_value = false;

  if ((mode != timed && !blockForcedRefreshes) ||
      ((((uint16_t)(getUptimeMs()) >> 10) & 0x0001)) ==
          halfSecTick3) { // 1.024 second refresh interval
    ...
    if (disp && !blockMonitoring) {
      ...
      showString(tmps, &standardFont, 20,
                 /*145-7*/ Y_POSITION_OF_REGISTER_T_LINE + mode * 20, vmNormal,
                 false, false); // note: displays info 1 line down, if "force"
                                // parameter is set
    }
...
bool_t progressHalfSecUpdate_Integer(
    uint8_t mode, char *txt, int32_t loop, bool_t clearZ, bool_t clearT,
    bool_t
        disp) { // further optimisation, not to even set up the 100 byte array
                // or call getUptimeMs if progress monitor is not selected
...
bool_t checkHalfSec(void) {
  ...
  if (((((uint16_t)(getUptimeMs()) >> 10) & 0x0001)) ==
      halfSecTick2) { // 1.024 second refresh interval
...
bool_t monitorExit(int32_t *loop, char *str) {
  (*loop)++;
  if (checkHalfSec()) {
    if (progressHalfSecUpdate_Integer(timed, str, *loop, halfSec_clearZ,
                                      halfSec_clearT, halfSec_disp)) { // timed
      // no additional 1 sec monitoring here
    }
  }
  if (exitKeyWaiting()) {
    progressHalfSecUpdate_Integer(force + 1, "Interrupted: ", *loop,
                                  halfSec_clearZ, halfSec_clearT, halfSec_disp);
    return true;
  }
  return false;
}
  ...
  if (cursorEnabled) {
    if (cursorFont == &standardFont) {
      ...
      if (checkHP) {
        ...
        lcd_fill_rect(xCursor, yCursor - checkHPoffset, ccol, crow,
                      LCD_SET_VALUE);
        // lcd_fill_rect(xCursor, yCursor + 15 - checkHPoffset, 13*2, 13*2,
        // LCD_SET_VALUE);
      } else {
...
static void stats_param_display(const char *name, calcRegister_t reg,
                                char *prefix, char *tmpString,
                                calcRegister_t rowReg) {
  int16_t prefixWidth;
  char regS[16], *p;
  real_t t;
  real34_t u;
  ...
  if (name == NULL ||
      !(rowReg == REGISTER_Y || rowReg == REGISTER_Z || rowReg == REGISTER_T)) {
    return;
  }
  ...
  showString(regS, &standardFont, 19,
             Y_POSITION_OF_REGISTER_X_LINE -
                 REGISTER_LINE_HEIGHT * (rowReg - REGISTER_X) + 6,
             vmNormal, true, true);
  prefixWidth = showString(prefix, &standardFont, 19 + (17 + 28),
                           Y_POSITION_OF_REGISTER_X_LINE -
                               REGISTER_LINE_HEIGHT * (rowReg - REGISTER_X) + 6,
                           vmNormal, true, true);

  if (getRegisterAsRealQuiet(reg, &t)) {
    angleMode = getRegisterDataType(reg) == dtReal34
                    ? getRegisterAngularMode(reg)
                    : amNone;
    realToReal34(&t, &u);
    real34ToDisplayString(&u, angleMode, tmpString, &numericFont,
                          SCREEN_WIDTH - prefixWidth, NUMBER_OF_DISPLAY_DIGITS,
                          LIMITEXP, FRONTSPACE, NOIRFRAC);
    p = tmpString;
  } else {
    p = "invalid";
  }

  showString(p, &numericFont,
             SCREEN_WIDTH - stringWidth(p, &numericFont, false, true),
             Y_POSITION_OF_REGISTER_X_LINE -
                 REGISTER_LINE_HEIGHT * (rowReg - REGISTER_X),
             vmNormal, false, true);
}
...
void showFunctionName(int16_t itm, int16_t delayInMs, const char *arg) {
  ...
  if (item < LAST_ITEM && (item == ITM_XEQ || item != ITM_RCL)) {
    stringCopy(functionName + stringByteLength(functionName),
               pickValidItemFromItems(item, PRIORITY_itemCatalogName));
    stringCopy(functionName + stringByteLength(functionName), ":");
  }
  if (item < LAST_ITEM && (item == ITM_RCL || item != ITM_XEQ)) {
    stringCopy(functionName + stringByteLength(functionName),
               pickValidItemFromItems(item, PRIORITY_itemSoftmenuName));
    stringCopy(functionName + stringByteLength(functionName), ":");
  }
  if (arg != NULL) {
  ...
  if ((item == ITM_XEQ) || (item == ITM_RCL)) {
    ...
    showFunctionNameArg = (char *)arg; // Needed when executing a program or a
                                       // variable from a long pressed key
    if (functionName[0] == 0) {
    ...
    if (arg != NULL)
      stringCopy(functionName, arg);
    showFunctionNameArg = (char *)
        arg; // Needed when executing a user menu from a long pressed key
  } else if (item >= FIRST_CONSTANT && item <= LAST_CONSTANT) {
    stringCopy(functionName,
               pickValidItemFromItems(item, PRIORITY_itemSoftmenuName));
  ...
  if (tam.alpha &&
      ((item == ITM_BACKSPACE) || (item == ITM_T_LEFT_ARROW) ||
       (item == ITM_T_RIGHT_ARROW))) { // For smooth display in tam.alpha
    return;
  }
...
#define typWidth 120 // stringWidth(" WWWWWW     ", &standardFont, true, true);
    ...
    if (calcMode == CM_ASSIGN ||
        ((PROBMENU || XXFNMODEACTIVE ||
          stringWidth(padding, &standardFont, true, true) + 1 /*JM 20*/ +
                  lineTWidth >
              SCREEN_WIDTH) &&
         calcMode != CM_PEM)) {
      clearRegisterLine(REGISTER_T, true, false);
    }
    // Clear SHIFT f and SHIFT g in case they were present (otherwise they will
    // be obscured by the function name)
    clearShiftState();
    int xx = showString(padding, &standardFont, funcNameOffset_x,
                        Y_POSITION_OF_REGISTER_T_LINE + 6, vmNormal, true,
                        true); // JM
    if (overLapPossible) {
      plotrect(funcNameOffset_x, Y_POSITION_OF_REGISTER_T_LINE + 6,
               max(xx, funcNameOffset_x + typWidth),
               Y_POSITION_OF_REGISTER_T_LINE + 6 + STANDARD_FONT_HEIGHT - 1);
      if (xx < funcNameOffset_x + typWidth)
        lcd_fill_rect(xx, Y_POSITION_OF_REGISTER_T_LINE + 6 + 1,
                      funcNameOffset_x + typWidth - xx,
                      STANDARD_FONT_HEIGHT - 2, LCD_SET_VALUE);
    }
  }
...
void hideFunctionName(void) {
  if (tmpString[0] != 0 || calcMode != CM_AIM) {
    if (calcMode != CM_PEM) {
      if (!tam.alpha || (showFunctionNameItem !=
                             ITM_BACKSPACE && // For smooth display in tam.alpha
                         showFunctionNameItem != ITM_T_LEFT_ARROW &&
                         showFunctionNameItem != ITM_T_RIGHT_ARROW &&
                         showFunctionNameItem != ITM_NULL)) {
        refreshRegisterLineRestoreT(); // JM DO NOT CHANGE BACK TO CLEARING ONLY
                                       // A SHORT PIECE. CHANGED IN TWEAKED AS
                                       // WELL>
    ...
    // this seems to cause an undue delay for large matrices, and I cannot see
    // why it should be reprinted (and the cached heights updated
    //      if(getRegisterDataType(REGISTER_X) == dtReal34Matrix ||
    //      getRegisterDataType(REGISTER_X) == dtReal34Matrix) {
...
void clearRect(uint32_t g_line_x, uint32_t g_line_y) {
  ...
  lcd_fill_rect((uint32_t)g_line_x, (uint32_t)g_line_y,
                SCREEN_WIDTH - g_line_x - 1, frow, LCD_SET_VALUE);
}

void clearRegisterLine(calcRegister_t regist, bool_t clearTop,
                       bool_t clearBottom) {
  if (REGISTER_X <= regist && regist <= REGISTER_T) {
    uint32_t yStart = Y_POSITION_OF_REGISTER_X_LINE -
                      REGISTER_LINE_HEIGHT * (regist - REGISTER_X);
...
static void do_viewRegName(calcRegister_t regist, char *prefix,
                           int16_t *prefixWidth,
                           char *endChar) { // using "=" for VIEW
  // printf("========================== %i %s regist=%i %s %i\n", lastFuncNo(),
  // lastFuncCatalogName(), regist, prefix, *prefixWidth);
  ...
  if (regist < REGISTER_X) {
    sprintf(prefix, "%sR%02" PRIu16 STD_SPACE_4_PER_EM "%s" STD_SPACE_4_PER_EM,
            funcNameOffset_str, regist, endChar);
  } else if (regist <= LAST_SPARE_REGISTER) {
    sprintf(prefix, "%s%c" STD_SPACE_4_PER_EM "%s" STD_SPACE_4_PER_EM,
            funcNameOffset_str, letteredRegisterName(regist), endChar);
  } else if (regist >= FIRST_LOCAL_REGISTER && regist <= LAST_LOCAL_REGISTER) {
    sprintf(prefix, "%sR.%02" PRIu16 STD_SPACE_4_PER_EM "%s" STD_SPACE_4_PER_EM,
            funcNameOffset_str, (uint16_t)(regist - FIRST_LOCAL_REGISTER),
            endChar);
    ...
    memcpy(prefix + (isShiftOffset ? 4 : 2),
           allNamedVariables[regist - FIRST_NAMED_VARIABLE].variableName + 1,
           allNamedVariables[regist - FIRST_NAMED_VARIABLE].variableName[0]);
    sprintf(
        prefix + (isShiftOffset ? 4 : 2) +
            allNamedVariables[regist - FIRST_NAMED_VARIABLE].variableName[0],
        STD_RIGHT_SINGLE_QUOTE STD_SPACE_4_PER_EM "%s" STD_SPACE_4_PER_EM,
        endChar);
  } else if (FIRST_RESERVED_VARIABLE <= regist &&
             regist <= LAST_RESERVED_VARIABLE) {
    if (isShiftOffset) {
      strcpy(prefix, "  ");
    }
    strcpy(prefix + (isShiftOffset ? 2 : 0), STD_LEFT_SINGLE_QUOTE);
    memcpy(prefix + (isShiftOffset ? 4 : 2),
           allReservedVariables[regist - FIRST_RESERVED_VARIABLE]
                   .reservedVariableName +
               1,
           allReservedVariables[regist - FIRST_RESERVED_VARIABLE]
               .reservedVariableName[0]);
    sprintf(prefix + (isShiftOffset ? 4 : 2) +
                allReservedVariables[regist - FIRST_RESERVED_VARIABLE]
                    .reservedVariableName[0],
            STD_RIGHT_SINGLE_QUOTE STD_SPACE_4_PER_EM "%s" STD_SPACE_4_PER_EM,
            endChar);
...
static void viewRegName(char *prefix,
                        int16_t *prefixWidth) { // using "=" for VIEW
...
// create substrings in tmpString by replacing the separator character with a
// [0]
void createSubstrings(uint8_t number) {
  ...
  // printStringToConsole((char *)getNthString((uint8_t
  // *)tmpString,0),"createSubstrings: substring 0: ","\n");
  // printStringToConsole((char *)getNthString((uint8_t
  // *)tmpString,1),"createSubstrings: substring 1: ","\n");
  // printStringToConsole((char *)getNthString((uint8_t
  // *)tmpString,2),"createSubstrings: substring 2: ","\n");
  // printStringToConsole((char *)getNthString((uint8_t
  // *)tmpString,3),"createSubstrings: substring 3: ","\n");
  return;
}

static void userTI(int16_t viewRegister, int16_t refreshRegist, char *prefix,
                   int16_t *prefixWidth) {
  if (!(lastFuncNo() == ITM_AVIEW || lastFuncNo() == ITM_PROMPT)) {
    return;
  }
  // printf("TTTT:refreshRegist:%i %i string:%s type:%i\n",viewRegister,
  // refreshRegist, tmpString, getRegisterDataType(viewRegister));
  if (temporaryInformation == TI_VIEW_REGISTER &&
      getRegisterDataType(viewRegister) == dtString) {
    // tmpString[0]=0;
    xcopy(tmpString, REGISTER_STRING_DATA(viewRegister),
          stringByteLength(REGISTER_STRING_DATA(viewRegister)) + 1);
    createSubstrings(4);
    if (refreshRegist == REGISTER_T) {
      char *string1 = "";
      string1 = (char *)getNthString((uint8_t *)tmpString, 0);
      xcopy(tmpString + (isShiftOffset ? 2 : 0), string1,
            stringByteLength(string1) + 1);
      if (isShiftOffset) {
        tmpString[0] = 32;
        tmpString[1] = 32;
      }
...
static void inputRegName(char *prefix, int16_t *prefixWidth) {
  if ((currentInputVariable & 0x3fff) < REGISTER_X) {
    sprintf(prefix, " R%02" PRIu16 "?",
            (uint16_t)(currentInputVariable & 0x3fff));
  } else if ((currentInputVariable & 0x3fff) <= LAST_SPARE_REGISTER) {
    sprintf(prefix, " %c?",
            letteredRegisterName(currentInputVariable & 0x3fff));
  } else if (((currentInputVariable & 0x3fff) >= FIRST_LOCAL_REGISTER) &&
             (currentInputVariable & 0x3fff) <= LAST_LOCAL_REGISTER) {
    sprintf(prefix, " R.%02" PRIu16 "?",
            (uint16_t)((currentInputVariable & 0x3fff) - FIRST_LOCAL_REGISTER));
  } else if (FIRST_NAMED_VARIABLE <= (currentInputVariable & 0x3fff) &&
             (currentInputVariable & 0x3fff) <= LAST_NAMED_VARIABLE) {
    memcpy(prefix,
           allNamedVariables[(currentInputVariable & 0x3fff) -
           ...
           allNamedVariables[(currentInputVariable & 0x3fff) -
                             FIRST_NAMED_VARIABLE]
               .variableName[0]);
    strcpy(prefix + allNamedVariables[(currentInputVariable & 0x3fff) -
                                      FIRST_NAMED_VARIABLE]
                        .variableName[0],
           "?");
  } else if (FIRST_RESERVED_VARIABLE <= (currentInputVariable & 0x3fff) &&
             (currentInputVariable & 0x3fff) <= LAST_RESERVED_VARIABLE) {
    memcpy(prefix,
           allReservedVariables[(currentInputVariable & 0x3fff) -
           ...
           allReservedVariables[(currentInputVariable & 0x3fff) -
                                FIRST_RESERVED_VARIABLE]
               .reservedVariableName[0]);
    strcpy(prefix + allReservedVariables[(currentInputVariable & 0x3fff) -
                                         FIRST_RESERVED_VARIABLE]
                        .reservedVariableName[0],
           "?");
  } else {
...
void updateMatrixHeightCache(void) {
  ...
  if (getRegisterDataType(REGISTER_X) == dtReal34Matrix ||
      (calcMode == CM_MIM &&
       getRegisterDataType(matrixIndex) == dtReal34Matrix)) {
    ...
    const int16_t mtxWidth = getRealMatrixColumnWidths(
        &matrix, prefixWidth, &numericFont, dummyVal,
        dummyVal + MATRIX_MAX_COLUMNS,
        dummyVal + (MATRIX_MAX_ROWS + 1) * MATRIX_MAX_COLUMNS,
        cols > MATRIX_MAX_COLUMNS ? MATRIX_MAX_COLUMNS : cols,
        allElementsInColAreIntegers);
    ...
    if (cachedDisplayStack > 4) { // in case of overflow
      cachedDisplayStack = 0;
             ...
             (calcMode == CM_MIM &&
              getRegisterDataType(matrixIndex) == dtComplex34Matrix)) {
    ...
    const int16_t mtxWidth = getComplexMatrixColumnWidths(
        &matrix, prefixWidth, &numericFont, dummyVal,
        dummyVal + MATRIX_MAX_COLUMNS, dummyVal + MATRIX_MAX_COLUMNS * 2,
        dummyVal + MATRIX_MAX_COLUMNS * 3,
        dummyVal + MATRIX_MAX_COLUMNS * (MATRIX_MAX_ROWS + 3),
        dummyVal + MATRIX_MAX_COLUMNS * (MATRIX_MAX_ROWS * 2 + 3),
        cols > MATRIX_MAX_COLUMNS ? MATRIX_MAX_COLUMNS : cols,
        getComplexRegisterAngularMode(REGISTER_X),
        getComplexRegisterPolarMode(REGISTER_X) == amPolar);
  ...
  if (getRegisterDataType(REGISTER_X) == dtReal34) {
    ...
    if (getSystemFlag(FLAG_FRACT)) {
      ...
      real34ToDisplayString(
          REGISTER_REAL34_DATA(REGISTER_X), getRegisterAngularMode(REGISTER_X),
          tmpString, &numericFont, SCREEN_WIDTH - prefixWidth,
          NUMBER_OF_DISPLAY_DIGITS, LIMITEXP, FRONTSPACE, LIMITIRFRAC);
    }
    w = stringWidth(tmpString, &numericFont, false, true);
    showString(prefix, &standardFont, 1, Y_POSITION_OF_REGISTER_X_LINE + 6,
               vmNormal, true, true);
    showString(tmpString, &numericFont, SCREEN_WIDTH - w,
               Y_POSITION_OF_REGISTER_X_LINE, vmNormal, false, true);
  } else if (getRegisterDataType(REGISTER_X) == dtComplex34) {
    clearRegisterLine(REGISTER_X, true, true);
    complex34ToDisplayString(
        REGISTER_COMPLEX34_DATA(REGISTER_X), tmpString, &numericFont,
        SCREEN_WIDTH - prefixWidth, NUMBER_OF_DISPLAY_DIGITS, LIMITEXP,
        FRONTSPACE, LIMITIRFRAC, getComplexRegisterAngularMode(REGISTER_X),
        getComplexRegisterPolarMode(REGISTER_X) == amPolar);
    w = stringWidth(tmpString, &numericFont, false, true);
    showString(prefix, &standardFont, 1, Y_POSITION_OF_REGISTER_X_LINE + 6,
               vmNormal, true, true);
    ...
    clearRegisterLine(REGISTER_X, true, true);
    longIntegerRegisterToDisplayString(REGISTER_X, tmpString, TMP_STR_LENGTH,
                                       SCREEN_WIDTH - prefixWidth, 50, true);
    w = stringWidth(tmpString, &numericFont, false, true);
    showString(prefix, &standardFont, 1, Y_POSITION_OF_REGISTER_X_LINE + 6,
               vmNormal, true, true);
    if (w <= SCREEN_WIDTH - prefixWidth) {
      ...
      w = stringWidth(tmpString, &standardFont, false, true);
      showString(tmpString, &standardFont, SCREEN_WIDTH - w,
                 Y_POSITION_OF_REGISTER_X_LINE + 6, vmNormal, false, true);
    }
  } else {
    showString(prefix, &standardFont, 1, Y_POSITION_OF_REGISTER_X_LINE + 6,
               vmNormal, true, true);
...
void _displayIJ(calcRegister_t regist, char *prefix, int16_t *prefixWidth) {
  ...
  if (iii == 0xFFFF || jji == 0xFFFF) {
    bb = getRegisterAsRealQuiet(REGISTER_I, &iir) &&
         getRegisterAsRealQuiet(REGISTER_J, &jjr);
    ...
    if (0 <= iii && iii < 200 && 0 <= jji && jji < 200) {
      ...
      *prefixWidth = 0;
      ...
      if (regist == REGISTER_X && temporaryInformation == TI_MIJEQ) {
        sprintf(prefix,
                STD_MU "[I" STD_SUB_r STD_SPACE_4_PER_EM "J" STD_SUB_c
                       "]=%s[%u" STD_SPACE_3_PER_EM "%u]%s",
                tmp, (uint8_t)iii, (uint8_t)jji,
                (temporaryInformation == TI_MIJEQ ? "=" : ""));
      }
      // M[Ir Jc]=INDEXname[1, 2]
      else if (regist == REGISTER_X && temporaryInformation == TI_MIJ) {
        sprintf(prefix,
                STD_MU "[I" STD_SUB_r STD_SPACE_4_PER_EM "J" STD_SUB_c
                       "]=%s[%u" STD_SPACE_3_PER_EM "%u]",
                tmp, (uint8_t)iii, (uint8_t)jji);
      }
      // R00 [Ir=1 Jc=1]: Jc=
      else if (regist == REGISTER_X &&
               ((iii != 0 && temporaryInformation == TI_I) ||
                (jji != 0 && temporaryInformation == TI_J))) {
        sprintf(prefix,
                "%s[I" STD_SUB_r "=%u" STD_SPACE_4_PER_EM "J" STD_SUB_c
                "=%u]%s",
                tmp, (uint8_t)iii, (uint8_t)jji,
                temporaryInformation == TI_I ? ": I" STD_SUB_r "="
                                             : ": J" STD_SUB_c "=");
      }
      // R00: Ir=
      // R00: Jr=
...
static void __displaySolver(calcRegister_t regist, char *prefix,
                            int16_t *prefixWidth, int16_t no) {
  char noo[32];
  real_t t;
  uint16_t variableNo = currentSolverVariable - FIRST_RESERVED_VARIABLE;
  switch (no) {
  case 2:
    strcpy(noo, STD_SUB_p STD_SUB_r STD_SUB_e STD_SUB_v " =");
    break;
  case 1:
    strcpy(noo, " =");
    if (getRegisterAsRealQuiet(REGISTER_T, &t)) {
      if (!realIsSpecial(&t) && realIsAnInteger(&t) &&
          realToInt32C47(&t) == 200) {
  ...
  if (currentSolverVariable >= FIRST_RESERVED_VARIABLE) {
    memcpy(prefix, allReservedVariables[variableNo].reservedVariableName + 1,
           allReservedVariables[variableNo].reservedVariableName[0]);
    strcpy(prefix + allReservedVariables[variableNo].reservedVariableName[0],
           noo);
    strcat(prefix + allReservedVariables[variableNo].reservedVariableName[0],
           &varDescr[variableNo].Desc[0]);
  } else {
    memcpy(prefix,
           allNamedVariables[currentSolverVariable - FIRST_NAMED_VARIABLE]
                   .variableName +
               1,
           allNamedVariables[currentSolverVariable - FIRST_NAMED_VARIABLE]
               .variableName[0]);
...
void _displaySolverOutput(calcRegister_t regist, char *prefix,
                          int16_t *prefixWidth) {
...
void _displaySolverInput(calcRegister_t regist, char *prefix,
                         int16_t *prefixWidth) {
  if (regist == REGISTER_X) {
...
#define noLine false
static void _displaySigmaPlus(calcRegister_t regist, char *prefix,
                              int16_t *prefixWidth, bool_t doLine) {
...
void _displayRegType(calcRegister_t regist, char *prefix,
                     int16_t *prefixWidth) {
  if (regist == REGISTER_X) {
    real_t t;
    ...
    case 5:
      ...
      break;
    case 6:
      strcpy(sss, "RealMatrix");
      break;
    case 7:
      strcpy(sss, "ComplexMatrix");
      break;
    case 8:
      strcpy(sss, "ShortInteger");
      break;
    case 9:
      strcpy(sss, "Config");
      break;
...
#define clearOffset -2
  if (temporaryInformation == TI_FALSE) {
    if (clearOffset != 0) {
      sprintf(sss, "     ");
      showString(sss, &standardFont, 1,
                 Y_POSITION_OF_TRUE_FALSE_LINE + 6 + clearOffset, vmNormal,
                 true, true); // blank a little higher, 2 pixel
    }
    sprintf(sss, "false");
    showString(sss, &standardFont, 1, Y_POSITION_OF_TRUE_FALSE_LINE + 6,
               vmNormal, true, true);
    return true;
  }
...
#define BASEMODE_OFFSET_Y 2
  ...
  // JM SHOIDISP // use the top part of the screen for HEX and BIN    //JM vv
  // SHOIDISP DISP_TI=3    T=16    T=16    T=16 DISP_TI=2            Z=10    T=2
  // DISP_TI=1                    Z=10
  if (BASEMODEREGISTERX && regist == REGISTER_X && lastErrorCode == 0) {
    ...
    if (getRegisterDataType(REGISTER_X) == dtShortInteger) {
      ...
      if (displayStack == 1 && calcMode != CM_NIM) {
        shortIntegerToDisplayString(Register_X, tmpString, true,
                                    dispBase == 0 ? 10 : 16);
        if (lastErrorCode == 0 &&
            stringWidth(tmpString, fontForShortInteger, false, true) +
                    stringWidth("  X: ", &standardFont, false, true) <=
                SCREEN_WIDTH) {
          showString("  X: ", &standardFont, 0,
                     Y_POSITION_OF_REGISTER_X_LINE -
                         REGISTER_LINE_HEIGHT * (REGISTER_Y - REGISTER_X) +
                         (fontForShortInteger == &standardFont ? 6 : 0),
                     vmNormal, false, true);
        }
      ...
      if ((displayStack == 1 && calcMode != CM_NIM) || displayStack == 2) {
        shortIntegerToDisplayString(
            Register_X, tmpString, true,
            displayStack == 1 ? 2 : (dispBase == 0 ? 10 : 16));
        if (lastErrorCode == 0 &&
            stringWidth(tmpString, fontForShortInteger, false, true) +
                    stringWidth("  X: ", &standardFont, false, true) <=
                SCREEN_WIDTH) {
          showString("  X: ", &standardFont, 0,
                     Y_POSITION_OF_REGISTER_X_LINE -
                         REGISTER_LINE_HEIGHT * (REGISTER_Z - REGISTER_X) +
                         (fontForShortInteger == &standardFont ? 6 : 0),
                     vmNormal, false, true);
        }
        showString(tmpString, fontForShortInteger,
      ...
      if ((displayStack == 1 && calcMode != CM_NIM) || displayStack == 2 ||
        ...
        shortIntegerToDisplayString(Register_X, tmpString, true,
                                    dispBase == 0
                                        ? (!getSystemFlag(FLAG_BCD) ? 16 : 1)
                                        : dispBase); // base 1 is BCD, #10
        if (lastErrorCode == 0 &&
            stringWidth(tmpString, fontForShortInteger, false, true) +
                    stringWidth("  X: ", &standardFont, false, true) <=
                SCREEN_WIDTH) {
          showString("  X: ", &standardFont, 0 + BASEMODE_OFFSET_X,
                     Y_POSITION_OF_REGISTER_X_LINE -
                         REGISTER_LINE_HEIGHT * (REGISTER_T - REGISTER_X) +
                         (fontForShortInteger == &standardFont ? 6 : 0) +
                         BASEMODE_OFFSET_Y,
                     vmNormal, false, true);
        }
      ...
      if ((displayStack == 1 && calcMode != CM_NIM) || displayStack == 2 ||
          displayStack == 3) {
        longIntegerToHexDisplayString(
            REGISTER_X, tmpString, true,
            dispBase == 0 ? (!getSystemFlag(FLAG_BCD) ? 16 : 1) : dispBase,
            SCREEN_WIDTH - (isShiftOffset ? 10 : 0)); // base 1 is BCD, #10
        bool_t printFirstCol = fontForShortInteger == &tinyFont;
        bool_t printWillFit =
            stringWidth(tmpString, fontForShortInteger, printFirstCol, true) +
                stringWidth("  X:" STD_INTEGER_Z ": ", &standardFont, false,
                            true) <=
            SCREEN_WIDTH - (isShiftOffset ? 10 : 0);
        uint32_t xoff = printWillFit
                            ? SCREEN_WIDTH - (isShiftOffset ? 10 : 0) -
                                  stringWidth(tmpString, fontForShortInteger,
                                              printFirstCol, true) -
                                  3
                            : (isShiftOffset ? 10 : 0);
        if (lastErrorCode == 0 && printWillFit) {
          showString("  X:" STD_INTEGER_Z ": ", &standardFont,
                     0 + BASEMODE_OFFSET_X,
                     Y_POSITION_OF_REGISTER_X_LINE -
                         REGISTER_LINE_HEIGHT * (REGISTER_T - REGISTER_X) +
                         (fontForShortInteger == &standardFont ? 6 : 0) +
        ...
        showStringEnhanced(tmpString, fontForShortInteger, xoff,
                           Y_POSITION_OF_REGISTER_X_LINE -
                               REGISTER_LINE_HEIGHT *
                                   (REGISTER_T - REGISTER_X) +
                               (fontForShortInteger == &standardFont ? 6 : 0),
                           vmNormal, printFirstCol, true, NO_compress, NO_raise,
                           DO_Show, NO_Bold, DO_LF);
      }
    }
...
bool_t registerFMA(calcRegister_t regist, real_t *tmp1, real_t *tmp2,
                   real34_t *tmp3, angularMode_t *angle, realContext_t *c) {
  if (getRegisterDataType(regist) == dtShortInteger ||
      getRegisterDataType(regist + 1) == dtShortInteger ||
      getRegisterDataType(regist + 2) ==
          dtShortInteger) { // check for SI, because getRegisterAsRealQuiet will
                            // accept SI as leagl number.
    return false;
  }
...
static void displayLRtemporaryInformation(char *prefix1, char *prefix2,
                                          char *prefix, const char *label,
                                          bool_t prefixPre, bool_t prefixPost,
                                          int16_t *prefixWidth) {
  strcpy(prefix, prefix1);
  strcat(prefix, getCurveFitModeFormula(lrChosen));
  strcat(prefix, prefix2);
  while (stringWidth(prefix, &standardFont, prefixPre, prefixPost) + 1 <
         LRWidth) {
...
static void _refreshRegisterLine(calcRegister_t regist,
                                 bool_t restoreRegisterT) {
  int32_t w = 0;
...
#ifdef DMCP_BUILD
  keyBuffer_pop(); // This causes key updates while the longer time processing
                   // register updates happen
  if (!skippedStackLines && (calcMode == CM_NORMAL || calcMode == CM_MIM) &&
      !(regist == REGISTER_X) && // || regist == REGISTER_Y) &&
      !runningOnSimOrUSB &&      // Automatically, when on battery (hence low
                                 // processor), change to skip long processing
                                 // register printing, recovering the fragmented
      // screen here: See timer.c fnTimerEndOfActivity()
      !emptyKeyBuffer() && key_empty() == 1) {
  ...
  printf(">>> refreshRegisterLine   register=%u screenUpdatingMode=%d "
         "temporaryInformation=%u BASEMODEACTIVE=%u, lastIntegerBase=%u\n",
  ...
  if (BASEMODEREGISTERX && !SHOWMODE &&
      displayStack != 4 - displayStackSHOIDISP) { // JMSHOI
    if (getRegisterDataType(REGISTER_X) == dtShortInteger) {
  ...
  if ((temporaryInformation == TI_SHOW_REGISTER || SHOWMODE) &&
      regist ==
          REGISTER_X) { // JM top frame of the SHOW window  //HERE SHOW LINE
    drawSinglePixelFullWidthLine(Y_POSITION_OF_REGISTER_T_LINE - 4);
  }

  if ((calcMode != CM_BUG_ON_SCREEN) && !GRAPHMODE &&
      (calcMode != CM_LISTXY)) {                        // JM
    ...
    if (getRegisterDataType(REGISTER_L) == dtReal34) {
      ...
      formatReal34Debug(string2,
                        (real34_t *)getRegisterDataPointer(REGISTER_L));
      strcat(string2, " ");
      ...
      for (i = 0, p = REGISTER_STRING_DATA(REGISTER_L);
           i <= stringByteLength(REGISTER_STRING_DATA(REGISTER_L)); i++, p++) {
        string2[i] = *p;
      }
    }
      ...
      longIntegerRegisterToDisplayString(REGISTER_L, string2, sizeof(string2),
                                         SCREEN_WIDTH, 50, true);
    }
      ...
      sprintf(&string1[strlen(string1)],
              "real34 %" PRIu16 STD_CROSS "%" PRIu16 " matrix = ",
              REGISTER_MATRIX_HEADER(REGISTER_L)->matrixRows,
              REGISTER_MATRIX_HEADER(REGISTER_L)->matrixColumns);
      ...
      sprintf(&string1[strlen(string1)],
              "complex34 %" PRIu16 STD_CROSS "%" PRIu16 " matrix = ",
              REGISTER_MATRIX_HEADER(REGISTER_L)->matrixRows,
              REGISTER_MATRIX_HEADER(REGISTER_L)->matrixColumns);
      formatComplex34Debug(string2,
                           REGISTER_COMPLEX34_MATRIX_ELEMENTS(REGISTER_L));
    }
      ...
      sprintf(string2, "data type %s not supported for now!",
    ...
    sprintf(string,
            "%" PRIu32 " bytes free (%" PRId32 " region%s), C47 %" PRIu32
            " bytes, GMP %" PRIu32 " bytes -> should always be 0",
            getFreeRamMemory(), numberOfFreeMemoryRegions,
            numberOfFreeMemoryRegions == 1 ? "" : "s",
            (uint32_t)TO_BYTES((uint64_t)c47MemInBlocks),
            (uint32_t)gmpMemInBytes);
    ...
    if (temporaryInformation == TI_STATISTIC_LR &&
        (getRegisterDataType(REGISTER_X) != dtReal34)) {
      if (regist == REGISTER_X) {
        if (orOrtho(lrSelection) == CF_ORTHOGONAL_FITTING) {
          sprintf(tmpString, "L.R. selected to OrthoF");
        } else {
          sprintf(tmpString, "L.R. selected to %03" PRIu16 ".",
                  (uint16_t)((lrSelection) & 0x01FF));
        }
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
        sprintf(
            errorMessage,
            "BestF is set, but will not work until REAL data points are used.");
        moreInfoOnError("In function _refreshRegisterLine:", errorMessage,
                        errorMessages[ERROR_INVALID_DATA_TYPE_FOR_OP], NULL);
#endif // (EXTRA_INFO_ON_CALC_ERROR == 1)
        w = stringWidth(tmpString, &standardFont, true, true);
        showString(tmpString, &standardFont, SCREEN_WIDTH - w,
                   Y_POSITION_OF_REGISTER_X_LINE + 6, vmNormal, true, true);
      }
    }
      ...
      showString(confirmationTI[id].string, &standardFont, 1,
                 Y_POSITION_OF_REGISTER_X_LINE + 6, vmNormal, true, true);
    }

    else if (temporaryInformation == TI_WHO) {
      if (regist == REGISTER_Z || regist == REGISTER_Y ||
          regist == REGISTER_X) { // Force repainting it 3 times to get it
                                  // painted properly over three lines
        showStringEnhanced(whoStr1, &standardFont, 1,
                           Y_POSITION_OF_REGISTER_X_LINE -
                               REGISTER_LINE_HEIGHT * 2 + 6,
                           vmNormal, true, true, NO_compress, NO_raise, DO_Show,
                           NO_Bold, DO_LF);
      }
    }
      ...
      clearRegisterLine(REGISTER_X, true, true);
      showStringEnhanced(versionStr2, &standardFont, 1,
                         Y_POSITION_OF_REGISTER_T_LINE + 6, vmNormal, true,
                         true, NO_compress, NO_raise, DO_Show, NO_Bold, DO_LF);
      showStringEnhanced(versionStr, &standardFont, 1,
                         Y_POSITION_OF_REGISTER_Z_LINE + 6, vmNormal, true,
                         true, NO_compress, NO_raise, DO_Show, NO_Bold, DO_LF);
      showStringEnhanced(disclaimerStr, &standardFont, 1,
                         Y_POSITION_OF_REGISTER_Y_LINE + 6, vmNormal, true,
                         true, NO_compress, NO_raise, DO_Show, NO_Bold, DO_LF);
    }
      ...
      // sprintf(tmpString, "1st Gregorian day set: %s (JD %" PRId32 ")",
      // tmpStr2, firstGregorianDay);
      showString(tmpString, &standardFont, 1,
                 Y_POSITION_OF_REGISTER_X_LINE + TEMPORARY_INFO_OFFSET + 6,
                 vmNormal, true, true);
    }
      ...
      // tmpStr2, firstGregorianDay);
      showString(tmpString, &standardFont, 1,
                 Y_POSITION_OF_REGISTER_Y_LINE + TEMPORARY_INFO_OFFSET + 6,
                 vmNormal, true, true);
      sprintf(tmpString, "Week of Year rule set: %s.%s",
              nameOfWday_en[firstDayOfWeek].itemName,
              nameOfWday_en[firstWeekOfYearDay].itemName);
      ...
      sprintf(prefix, "%s.%s", nameOfWday_en[firstDayOfWeek].itemName,
              nameOfWday_en[firstWeekOfYearDay].itemName);
      displayTemporaryInformationOnX(prefix);
    }

    else if (temporaryInformation == TI_KEYS && regist == REGISTER_X) {
      showString(errorMessage, &standardFont, 1,
                 Y_POSITION_OF_REGISTER_X_LINE + 6, vmNormal, true, true);
    }
    ...
    else if (temporaryInformation == TI_CLEAR_ALL_FLAGS &&
             regist == REGISTER_X) {
    ...
    else if (temporaryInformation == TI_CLEAR_ALL_MENUS &&
             regist == REGISTER_X) {
    ...
    else if (temporaryInformation == TI_CLEAR_ALL_VARIABLES &&
             regist == REGISTER_X) {
    ...
    else if (temporaryInformation == TI_DEL_ALL_VARIABLES &&
             regist == REGISTER_X) {
    ...
    else if (temporaryInformation == TI_BACKUP_RESTORED &&
             regist == REGISTER_X) {
      clearRegisterLine(REGISTER_X, true, true);
      clearRegisterLine(REGISTER_Y, true, true);
      clearRegisterLine(REGISTER_Z, true, true);
      clearRegisterLine(REGISTER_T, true, true);
      showString(errorMessages[TI_Backup_restored], &standardFont, 1,
                 Y_POSITION_OF_REGISTER_Z_LINE + 6, vmNormal, true, true);
      showStringEnhanced(versionStr, &standardFont, 1,
                         Y_POSITION_OF_REGISTER_X_LINE + 6, vmNormal, true,
                         true, NO_compress, NO_raise, DO_Show, NO_Bold, DO_LF);
      showStringEnhanced(versionStr2, &standardFont, 1,
                         Y_POSITION_OF_REGISTER_Y_LINE + 6, vmNormal, true,
                         true, NO_compress, NO_raise, DO_Show, NO_Bold, DO_LF);
    ...
    else if (temporaryInformation == TI_STATEFILE_RESTORED &&
             regist == REGISTER_X) {
      sprintf(prefix, "%s", errorMessages[TI_State_file_restored]);
      displayTemporaryInformationOnX(prefix);
    }

    else if (temporaryInformation == TI_PROGRAMS_RESTORED &&
             regist == REGISTER_X) {
      sprintf(prefix, "                                ");
      displayTemporaryInformationOnX(prefix);
      sprintf(prefix, "%s", errorMessages[TI_Saved_programs_and_equations]);
      showString(prefix, &standardFont, 1, Y_POSITION_OF_REGISTER_X_LINE - 3,
                 vmNormal, true, true);
      sprintf(prefix, "%s", errorMessages[TI_appended]);
      showString(prefix, &standardFont, 1, Y_POSITION_OF_REGISTER_X_LINE + 17,
                 vmNormal, true, true);
    }

    else if (temporaryInformation == TI_REGISTERS_RESTORED &&
             regist == REGISTER_X) {
      sprintf(prefix, "                                  ");
      displayTemporaryInformationOnX(prefix);
    ...
    else if (temporaryInformation == TI_SETTINGS_RESTORED &&
             regist == REGISTER_X) {
    ...
    else if (temporaryInformation == TI_VARIABLES_RESTORED &&
             regist == REGISTER_X) {
      sprintf(prefix, "%s", errorMessages[TI_Saved_user_variables_restored]);
      displayTemporaryInformationOnX(prefix);
    }

    else if (temporaryInformation == TI_PROGRAM_LOADED &&
             regist == REGISTER_X) {
      ...
      showString(errorMessages[ERROR_TI_UNDO_FAILED], &standardFont, 1,
                 Y_POSITION_OF_REGISTER_X_LINE + 6, vmNormal, true, true);
    }
    ...
    //       else if((!checkHP && restoreRegisterT == RESTORE_T && regist ==
    //       REGISTER_T) ||
    //                regist < REGISTER_X + min(displayStack, origDisplayStack)
    //                || (lastErrorCode != 0 && regist ==
    //                errorMessageRegisterLine) || (temporaryInformation ==
    //                TI_VIEW_REGISTER && regist == REGISTER_T)) {
    // Cautiously removing the first condition above, as it should only be
    // active when display stack allows; and this AND condition will be
    // overridden by the second OR condition.
...
#if defined(PC_BUILD)
      if (baseY < 0) {
        printf("ILLEGAL BASE VALUE baseY<0 : baseY=%i regist=%u "
               "regist-REGISTER_X=%u cachedDisplayStack=%u displayStack=%u\n",
               baseY, regist, regist - REGISTER_X, cachedDisplayStack,
               displayStack);
#if defined(ANALYSE_REFRESH)
        print_caller(NULL);
      ...
      if (temporaryInformation == TI_VIEW_REGISTER && regist == REGISTER_T) {
        if (FIRST_RESERVED_VARIABLE <= currentViewRegister &&
            currentViewRegister < LAST_RESERVED_VARIABLE &&
            allReservedVariables[currentViewRegister - FIRST_RESERVED_VARIABLE]
                    .header.pointerToRegisterData == C47_NULL) {
          copySourceRegisterToDestRegister(currentViewRegister,
                                           TEMP_REGISTER_1);
      ...
      if (regist == REGISTER_X && lastErrorCode == 0 && calcMode != CM_PEM &&
          ((PROBMENU) ||
           (currentMenu() == -MNU_Solver_TOOL && solverEstimatesUsed &&
            temporaryInformation != TI_SOLVER_VARIABLE_RESULT))) {
        const char *r_i = NULL, *r_j = NULL, *r_k = NULL;
        calcRegister_t register_i = REGISTER_X, register_j = REGISTER_X,
                       register_k = REGISTER_X;
      ...
      if (regist == REGISTER_X && XXFNMODEACTIVE) {
        int tmpY = Y_POSITION_OF_REGISTER_X_LINE -
                   REGISTER_LINE_HEIGHT * (REGISTER_T - REGISTER_X);
...
#define FMA_T -1 - 3
...

        {
          ...
          int xx =
              showString(tmpString, &standardFont, (isShiftOffset ? 20 : 0),
                         tmpY + FMA_X, vmNormal, false, true);
          if (isXFNregisterValid3r(REGISTER_X + (calcMode == CM_NIM ? 1 : 0)) &&
              registerFMA(REGISTER_X + (calcMode == CM_NIM ? 1 : 0), &tmp1,
                          &tmp2, &tmp3, &angle, &ctxtReal39)) {
            tmpString[0] = 0;
            real34ToDisplayString(&tmp3, angle, tmpString, &standardFont,
                                  SCREEN_WIDTH - (isShiftOffset ? 20 : 0) - xx,
                                  34, LIMITEXP, FRONTSPACE, NOIRFRAC);
          } else {
            sprintf(tmpString, "%s ", errorMessages[ERROR_INVALID_TYPE_XFN]);
          }
          showString(tmpString, &standardFont,
                     SCREEN_WIDTH -
                         stringWidth(tmpString, &standardFont, false, true),
                     tmpY + FMA_X, vmNormal, false, true);
        }
        if (getSystemFlag(FLAG_SSIZE8)) {
          ...
          int xx =
              showString(tmpString, &standardFont, (isShiftOffset ? 20 : 0),
                         tmpY + FMA_T, vmNormal, false, true);
          if (isXFNregisterValid3r(REGISTER_T + (calcMode == CM_NIM ? 1 : 0)) &&
              registerFMA(REGISTER_T + (calcMode == CM_NIM ? 1 : 0), &tmp1,
                          &tmp2, &tmp3, &angle, &ctxtReal39)) {
            tmpString[0] = 0;
            real34ToDisplayString(&tmp3, angle, tmpString, &standardFont,
                                  SCREEN_WIDTH - (isShiftOffset ? 20 : 0) - xx,
      ...
      if (lastErrorCode != 0 && regist == errorMessageRegisterLine) {
        if (stringWidth(errorMessages[lastErrorCode], &standardFont, true,
                        true) <= SCREEN_WIDTH - 1) {
          if (lastErrorCode == ERROR_RESERVED_VARIABLE_NAME) {
            sprintf(tmpString, "%s: %s", errorMessages[lastErrorCode],
                    errorMessage);

            showString(tmpString, &standardFont, 1,
                       Y_POSITION_OF_REGISTER_X_LINE -
                           REGISTER_LINE_HEIGHT * (regist - REGISTER_X) + 6,
                       vmNormal, true, true);
          } else {
            showString(errorMessages[lastErrorCode], &standardFont, 1,
                       Y_POSITION_OF_REGISTER_X_LINE -
                           REGISTER_LINE_HEIGHT * (regist - REGISTER_X) + 6,
                       vmNormal, true, true);
          }
...
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
          sprintf(errorMessage, "Error message %" PRIu8 " is too wide!",
                  lastErrorCode);
          moreInfoOnError("In function _refreshRegisterLine:", errorMessage,
                          errorMessages[lastErrorCode], NULL);
#endif // (EXTRA_INFO_ON_CALC_ERROR == 1)
          sprintf(tmpString, "Error message %" PRIu8 " is too wide!",
                  lastErrorCode);
          w = stringWidth(tmpString, &standardFont, true, true);
          showString(tmpString, &standardFont, SCREEN_WIDTH - w,
                     Y_POSITION_OF_REGISTER_X_LINE -
        ...
        if (lastIntegerBase != 0) {
          ...
          *(lb++) = 0;
          wLastBaseNumeric =
              stringWidth(lb, &numericFont, true, true); // fixed to lb
          wLastBaseStandard =
              stringWidth(lb, &standardFont, true, true); // fixed to lb
        } else {
          wLastBaseNumeric = 0;
          wLastBaseStandard = 0;
        }
        ...
        displayNim(nimBufferDisplay, lastBase, wLastBaseNumeric,
                   wLastBaseStandard);
      }
...
// JMCURSOR vv
        ...
        if (T_cursorPos > tmplen) {
          T_cursorPos = tmplen;
        } // Do range checking in case the cursor starts off outside of range
        if (T_cursorPos < 0) {
          T_cursorPos = tmplen;
        } // Do range checking in case the cursor starts off outside of range
        showStringEdC47(multiEdLines, displayAIMbufferoffset, T_cursorPos,
                        aimBuffer, 1,
                        Y_POSITION_OF_NIM_LINE - 3 - checkHPoffset, vmNormal,
                        true, true, false); // display up to the cursor
        ...
        if (stringWidth(aimBuffer, &standardFont, true, true) <
            SCREEN_WIDTH - 8) { // 8 is the standard font cursor width
          xCursor =
              showString(aimBuffer, &standardFont, 1,
                         Y_POSITION_OF_NIM_LINE + 6, vmNormal, true, true);
          ...
          aimw = stringAfterPixels(tmpString, &standardFont, SCREEN_WIDTH - 2,
                                   true, true);
          w = aimw - tmpString;
          *aimw = 0;

          if (stringWidth(tmpString + 1500 + w, &standardFont, true, true) >=
              SCREEN_WIDTH - 8) { // 8 is the standard font cursor width
            fnKeyBackspace(0);    // back space
          } else {
            showString(tmpString, &standardFont, 1, Y_POSITION_OF_NIM_LINE - 3,
                       vmNormal, true, true);

            xCursor =
                showString(tmpString + 1500 + w, &standardFont, 1,
                           Y_POSITION_OF_NIM_LINE + 18, vmNormal, true, true);
      ...
      /*Main type dtReal34 FLAG_FRACT*/
               ...
               (getRegisterDataType(regist) == dtReal34 &&
                (real34CompareAbsLessThan(REGISTER_REAL34_DATA(regist),
                                          const34_1e6) ||
                 real34IsZero(REGISTER_REAL34_DATA(regist))))) {

        if (temporaryInformation == TI_VIEW_REGISTER &&
            origRegist == REGISTER_T) {
          viewRegName(prefix, &prefixWidth);
        } else if (temporaryInformation ==
                   TI_VIEW_REGISTER) { // X, Y, & Z, not T
        ...
        if (prefixWidth > 0) {
          if (temporaryInformation == TI_INTEGRAL && regist == REGISTER_X) {
            showString(prefix, &numericFont, 1, baseY - checkHPoffset, vmNormal,
                       prefixPre, prefixPost);
          } else {
            showString(prefix, &standardFont, 1,
                       baseY - checkHPoffset + TEMPORARY_INFO_OFFSET, vmNormal,
                       prefixPre, prefixPost);
          }
        }
        ...
        if (w <= SCREEN_WIDTH) {
          showString(tmpString, &numericFont, SCREEN_WIDTH - w,
                     baseY - checkHPoffset, vmNormal, false, true);
        } else {
          w = stringWidth(tmpString, &standardFont, false, true);
          lineWidth = w;
          if (w > SCREEN_WIDTH) {
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
            moreInfoOnError("In function _refreshRegisterLine:",
                            "Fraction representation too wide!", tmpString,
                            NULL);
#endif // (EXTRA_INFO_ON_CALC_ERROR == 1)
          ...
          showString(tmpString, &standardFont, SCREEN_WIDTH - w, baseY,
                     vmNormal, false, true);
        }
      }
      ...
      /*Main type dtReal34*/
        ...
        if (temporaryInformation == TI_COPY_FROM_SHOW && regist == REGISTER_X) {
          ...
          if (regist == REGISTER_X)
            displayLRtemporaryInformation(
                "y" STD_SPACE_4_PER_EM "=" STD_SPACE_4_PER_EM,
                ":" STD_SPACE_4_PER_EM, prefix, "a" STD_SUB_0, prefixPre,
                prefixPost, &prefixWidth);
        } else if (temporaryInformation == TI_LR_A1) {
          if (regist == REGISTER_X)
            displayLRtemporaryInformation(
                "y" STD_SPACE_4_PER_EM "=" STD_SPACE_4_PER_EM,
                ":" STD_SPACE_4_PER_EM, prefix, "a" STD_SUB_1, prefixPre,
                prefixPost, &prefixWidth);
        } else if (temporaryInformation == TI_LR_A2) {
          if (regist == REGISTER_X)
            displayLRtemporaryInformation(
                "y" STD_SPACE_4_PER_EM "=" STD_SPACE_4_PER_EM,
                ":" STD_SPACE_4_PER_EM, prefix, "a" STD_SUB_2, prefixPre,
                prefixPost, &prefixWidth);
        }
        // L.R. Display
          ...
          if (lrChosen == CF_CAUCHY_FITTING || lrChosen == CF_GAUSS_FITTING ||
              lrChosen == CF_PARABOLIC_FITTING) {
            if (regist == REGISTER_X) {
              displayLRtemporaryInformation("", "", prefix, "a" STD_SUB_0,
                                            prefixPre, prefixPost,
                                            &prefixWidth);
            } else if (regist == REGISTER_Y) {
              strcpy(prefix, "y" STD_SPACE_4_PER_EM "=" STD_SPACE_4_PER_EM);
              while (stringWidth(prefix, &standardFont, prefixPre, prefixPost) +
                         1 <
                     LRWidth) {
                strcat(prefix, STD_SPACE_6_PER_EM);
              }
              strcat(prefix,
                     "a" STD_SUB_1 STD_SPACE_4_PER_EM "=" STD_SPACE_HAIR);
              prefixWidth =
                  stringWidth(prefix, &standardFont, prefixPre, prefixPost) + 1;
            } else if (regist == REGISTER_Z) {
              ...
              strcat(prefix,
                     "a" STD_SUB_2 STD_SPACE_4_PER_EM "=" STD_SPACE_HAIR);
              prefixWidth =
                  stringWidth(prefix, &standardFont, prefixPre, prefixPost) + 1;
            }
          } else {
            if (regist == REGISTER_X) {
              displayLRtemporaryInformation(
                  "y" STD_SPACE_4_PER_EM "=" STD_SPACE_4_PER_EM, "", prefix,
                  "a" STD_SUB_0, prefixPre, prefixPost, &prefixWidth);
            } else if (regist == REGISTER_Y) {
              strcpy(prefix, eatSpacesEnd(getCurveFitModeName(lrChosen)));
              if (lrCountOnes(lrSelection) > 1) {
                strcat(prefix, lrChosen == 0 ? "" : STD_SUP_ASTERISK);
        ...
        // }
          ...
          if (regist == REGISTER_X) {
            ...
            if (regist == REGISTER_Y) {
              ...
              prefixWidth =
                  stringWidth(prefix, &standardFont, false, false) + 1;
            }
          }
        }
        ...
        // L.R. Display
          ...
          if (regist == REGISTER_X) {
            if (orOrtho(lrSelection) == CF_ORTHOGONAL_FITTING) {
              ...
              sprintf(prefix, "L.R. selected to %03" PRIu16,
                      (uint16_t)((lrSelection) & 0x01FF));
            }
            prefixWidth = stringWidth(prefix, &standardFont, true, true) + 1;
            // drawSinglePixelFullWidthLine(Y_POSITION_OF_REGISTER_Y_LINE - 2);
        ...
        else if (temporaryInformation == TI_CONV_MENU_STR &&
                 regist == REGISTER_X) { // convert menu
          strcpy(prefix, " ");
        ...
        else if (temporaryInformation == TI_ABC ||
                 temporaryInformation == TI_ABBCCA ||
                 temporaryInformation == TI_012) { // JM EE \/
          ...
          if (regist == REGISTER_X) {
            ...
            if (prefix[0] != 0) {
              strcat(prefix, " ");
              if (compareString(lastFuncSoftmenuName(), lastFuncCatalogName(),
                                CMP_BINARY) != 0) {
                char prefix_[16];
                prefix_[0] = 0;
              ...
              prefixWidth = stringWidth(prefix, &standardFont, true, true) + 1;
                   ...
                   (regist == REGISTER_X && (temporaryInformation == TI_I ||
                                             temporaryInformation == TI_J))) {
        ...
        if (prefixWidth > 0 && temporaryInformation != TI_VIEW_REGISTER) {
          if (regist == REGISTER_X) {
            showString(prefix, &standardFont, 1,
                       Y_POSITION_OF_REGISTER_X_LINE + TEMPORARY_INFO_OFFSET,
                       vmNormal, true, true);
          } else if (regist == REGISTER_Y) {
            showString(prefix, &standardFont, 1,
                       Y_POSITION_OF_REGISTER_Y_LINE + TEMPORARY_INFO_OFFSET,
                       vmNormal, true, true);
          } else if (regist == REGISTER_Z) {
            showString(prefix, &standardFont, 1,
                       Y_POSITION_OF_REGISTER_Z_LINE + TEMPORARY_INFO_OFFSET,
                       vmNormal, true, true);
          }
        }
        // JM EE ^
        ...
        real34ToDisplayString(
            REGISTER_REAL34_DATA(regist), getRegisterAngularMode(regist),
            tmpString, &numericFont, SCREEN_WIDTH - prefixWidth,
        ...
        showString(tmpString, &numericFont,
                   (temporaryInformation == TI_VIEW_REGISTER &&
                    origRegist == REGISTER_T)
                       ? prefixWidth
                       : SCREEN_WIDTH - w,
      ...
      // JM else if(getRegisterDataType(regist) == dtComplex34) { //JM EE
      // Removed and replaced with the below JM
      // complex34ToDisplayString(REGISTER_COMPLEX34_DATA(regist), tmpString,
      // &numericFont, SCREEN_WIDTH, NUMBER_OF_DISPLAY_DIGITS, true,
      // STD_SPACE_PUNCTUATION);   //JM EE Removed and replaced with the below
        ...
        // JM EE ^
        complex34ToDisplayString(
            REGISTER_COMPLEX34_DATA(regist), tmpString, &numericFont,
            SCREEN_WIDTH - prefixWidth, NUMBER_OF_DISPLAY_DIGITS, LIMITEXP,
            FRONTSPACE, FULLIRFRAC, getComplexRegisterAngularMode(regist),
            getComplexRegisterPolarMode(regist) == amPolar);

        w = stringWidth(tmpString, &numericFont, false, true);
        lineWidth = w;
        if (prefixWidth > 0) {
          showString(prefix, &standardFont, 1, baseY + TEMPORARY_INFO_OFFSET,
                     vmNormal, prefixPre, prefixPost);
        }
        showString(tmpString, &numericFont,
                   (temporaryInformation == TI_VIEW_REGISTER &&
                    origRegist == REGISTER_T)
      ...
      /*Main type dtString*/
        ...
        if (temporaryInformation == TI_COPY_FROM_SHOW && regist == REGISTER_X) {
          ...
          showString(prefix, &standardFont, 1, Y_POSITION_OF_REGISTER_Y_LINE,
                     vmNormal, prefixPre, prefixPost);
          prefix[0] = 0;
          prefixWidth = 0;
...
// JM REGISTER STRING LARGE FONTS
        ...
        // This is for X
        w = stringWidthWithLimitC47(REGISTER_STRING_DATA(regist), stdnumEnlarge,
                                    nocompress, SCREEN_WIDTH, false, true);
        if (temporaryInformation != TI_VIEW_REGISTER && regist == REGISTER_X &&
            w < SCREEN_WIDTH) {
          lineWidth =
              w; // slighly incorrect if special characters are there as well.
          showStringC47(REGISTER_STRING_DATA(regist), stdnumEnlarge, nocompress,
                        SCREEN_WIDTH - w,
                        Y_POSITION_OF_REGISTER_X_LINE + 6 - checkHPoffset,
                        vmNormal, false, true);
        } else // JM
          ...
          if (temporaryInformation != TI_VIEW_REGISTER &&
              regist == REGISTER_X &&
              (w = stringWidthWithLimitC47(REGISTER_STRING_DATA(regist),
                                           numHalf, nocompress, SCREEN_WIDTH,
                                           false, true)) < SCREEN_WIDTH) {
            lineWidth = w;
            showStringC47(REGISTER_STRING_DATA(regist), numHalf, nocompress,
                          SCREEN_WIDTH - w,
                          Y_POSITION_OF_REGISTER_X_LINE + 6 - checkHPoffset,
                          vmNormal, false, true);
          } else // JM
                 ...
                 // This is for Y, Z & T
            if (regist >= REGISTER_Y && regist <= REGISTER_T &&
                (w = stringWidthWithLimitC47(
                     REGISTER_STRING_DATA(regist), numHalf, nocompress,
                     SCREEN_WIDTH - prefixWidth, false, true)) <
                    SCREEN_WIDTH - prefixWidth) {
              lineWidth = w;
              showStringC47(REGISTER_STRING_DATA(regist), numHalf, nocompress,
                            SCREEN_WIDTH - w, baseY + 6 - checkHPoffset,
                            vmNormal, false, true);
            } else // JM
            ...
            {
              // printf("^^^^#### combinationFonts=%d maxiC=%d miniC=%d
              // displaymode=%d\n", combinationFonts, maxiC, miniC,
              // displaymode);
              w = stringWidth(REGISTER_STRING_DATA(regist), &standardFont,
                              false, true);
              if (w >= SCREEN_WIDTH - prefixWidth) {
                char *tmpStrW;
                if (regist == REGISTER_X ||
                    (temporaryInformation == TI_VIEW_REGISTER &&
                     origRegist == REGISTER_T)) {
                  xcopy(tmpString, REGISTER_STRING_DATA(regist),
                        stringByteLength(REGISTER_STRING_DATA(regist)) + 1);
                  if (temporaryInformation == TI_VIEW_REGISTER &&
                      origRegist == REGISTER_T) {
                    createSubstrings(1);
                  }
                  tmpStrW = stringAfterPixels(tmpString, &standardFont,
                                              SCREEN_WIDTH - prefixWidth - 1,
                                              false, true);
                  *tmpStrW = 0;
                  w = stringWidth(tmpString, &standardFont, false, true);
                  if (temporaryInformation == TI_VIEW_REGISTER &&
                      origRegist == REGISTER_T) {
                    showString(tmpString, &standardFont, prefixWidth,
                               Y_POSITION_OF_REGISTER_T_LINE - 3, vmNormal,
                               false, true);
                  } else {
                    showString(tmpString, &standardFont, SCREEN_WIDTH - w,
                               Y_POSITION_OF_REGISTER_X_LINE - 3 -
                                   checkHPoffset,
                               vmNormal, false, true);
                  }
                  ...
                  xcopy(tmpString, tmpString + w,
                        stringByteLength(tmpString + w) + 1);

                  w = stringWidth(tmpString, &standardFont, false, true);
                  if (w >= SCREEN_WIDTH - prefixWidth) {
                    tmpStrW = stringAfterPixels(
                        tmpString, &standardFont,
                        SCREEN_WIDTH - prefixWidth - 14 - 1, false,
                        true); // 14 is the width of STD_ELLIPSIS
                  ...
                  if (temporaryInformation == TI_VIEW_REGISTER &&
                      origRegist == REGISTER_T) {
                    showString(tmpString, &standardFont, prefixWidth,
                               Y_POSITION_OF_REGISTER_T_LINE + 18, vmNormal,
                               false, true);
                  } else {
                    showString(tmpString, &standardFont, SCREEN_WIDTH - w,
                               Y_POSITION_OF_REGISTER_X_LINE + 18 -
                                   checkHPoffset,
                               vmNormal, false, true);
                  }
                } else {
                  ...
                  lineWidth = w;
                  showString(tmpString, &standardFont, SCREEN_WIDTH - w,
                             baseY + 6 - checkHPoffset, vmNormal, false, true);
                }
              } else {
                lineWidth = w;
                xcopy(tmpString, REGISTER_STRING_DATA(regist),
                ...
                if (temporaryInformation == TI_VIEW_REGISTER &&
                    origRegist == REGISTER_T) {
                  showString(tmpString, &standardFont, prefixWidth,
                             baseY + TEMPORARY_INFO_OFFSET, vmNormal, false,
                             true);
                } else {
                  showString(tmpString, &standardFont, SCREEN_WIDTH - w,
                             baseY + 6 - checkHPoffset, vmNormal, false, true);
      ...
      /*Main type dtShortInteger*/
      else if (getRegisterDataType(regist) == dtShortInteger) {
        {
          ...
          showString(
              tmpString, fontForShortInteger,
              SCREEN_WIDTH -
                  stringWidth(tmpString, fontForShortInteger, false, true),
              baseY + (fontForShortInteger == &standardFont ? 6 : 0) -
                  (fontForShortInteger == &numericFont ? checkHPoffset : 0),
              vmNormal, false, true);
          ...
          if (temporaryInformation == TI_VIEW_REGISTER &&
              origRegist == REGISTER_T) {
            lcd_fill_rect(0, Y_POSITION_OF_REGISTER_T_LINE, 50,
                          REGISTER_LINE_HEIGHT, LCD_SET_VALUE);
          }
        }
        ...
        if (regist == REGISTER_X &&
          ...
          prefixWidth = stringWidth(prefix, &standardFont, true, true) + 1;
          if (prefixWidth +
                  stringWidth(tmpString, fontForShortInteger, true, true) + 1 >
              SCREEN_WIDTH) {
            sprintf(prefix, "OF");
            prefixWidth = stringWidth(prefix, &standardFont, true, true) + 1;
          }
        ...
        else if (temporaryInformation == TI_COPY_FROM_SHOW &&
                 regist == REGISTER_X) {
          _fnShowRecallTI(prefix, &prefixWidth);
        } else if (temporaryInformation == TI_STATISTIC_SUMS) {
          _displaySigmaPlus(regist, prefix, &prefixWidth, !noLine);
      ...
      /*Main type dtLongInteger*/
        ...
        if (temporaryInformation == TI_COPY_FROM_SHOW && regist == REGISTER_X) {
          ...
          if (regist == REGISTER_X) {
            int day;
            longInteger_t li;
            getRegisterAsLongInt(
                REGISTER_X, li,
                NULL); // Cannot fail as REGISTER_X is a dtLongInteger
        ...
        // shift longinter prefix on by two space if interfering with the shift
        // indicator, when SB_TIME is selected
        if (regist == REGISTER_T && isShiftOffset) {
          ...
          if (len + 2 < 200) {
            if (prefix[0] == 0) {
              strcpy(prefix, "  ");
              prefixWidth +=
                  20; // stringWidth("  ", &standardFont, true, true) - 2;
            } else {
              for (int i = len; i >= 0; i--) {
                prefix[i + 2] = prefix[i];
              }
        ...
        if (getSystemFlag(FLAG_DREAL)) {
          ...
          w = stringWidth(tmpString,
                          getSystemFlag(FLAG_LARGELI) ? &numericFont
                                                      : &standardFont,
                          false, true);
          int16_t tlen = stringByteLength(tmpString);
          longIntegerRegisterToRealDisplayString(
              regist, tmpString + tlen, TMP_STR_LENGTH - tlen,
              SCREEN_WIDTH - prefixWidth - w, 0, toRemoveTrailingRadix);
          tmpString[TMP_STR_LENGTH - 1] = tmpString[tlen];
        }

        // for the 2^10 UNIT diplay, display long integers in real string, with
        // the Ti suffic
          ...
          longIntegerRegisterToRealDisplayString(
              regist, tmpString + tlen, TMP_STR_LENGTH - tlen,
              SCREEN_WIDTH - prefixWidth - w, 1024, !toRemoveTrailingRadix);
          tmpString[TMP_STR_LENGTH - 1] = tmpString[tlen];
        }

        // normal longinteger handling
        else {
          longIntegerRegisterToDisplayString(regist, tmpString, TMP_STR_LENGTH,
                                             SCREEN_WIDTH - prefixWidth, 50,
                                             getSystemFlag(FLAG_LARGELI));
          tmpString[TMP_STR_LENGTH - 1] = tmpString[0];

          {
            // This section to display too long integers as reals, re-doing the
            // above string in Real. The wastage is minor compared to the Real
            // processing that will follow
            uint16_t pos;
            if (findTwoChars(tmpString, (uint8_t)PRODUCT_SIGN[0],
                             (uint8_t)PRODUCT_SIGN[1], &pos)) {
              strcpy(tmpString,
                     STD_INTEGER_Z_SMALL ": "); // STD_SPACE_4_PER_EM);
              w = stringWidth(tmpString,
                              getSystemFlag(FLAG_LARGELI) ? &numericFont
                                                          : &standardFont,
                              false, true);
...
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
            moreInfoOnError("In function _refreshRegisterLine:",
                            "Long integer representation too wide!", tmpString,
                            NULL);
#endif // (EXTRA_INFO_ON_CALC_ERROR == 1)
            strcpy(tmpString, "Long integer representation too wide!");
          }
      ...
      /*Main type dtDate*/
        ...
        if (temporaryInformation == TI_COPY_FROM_SHOW && regist == REGISTER_X) {
          _fnShowRecallTI(prefix, &prefixWidth);
                   ...
                   TI_VIEW_REGISTER /*== TI_DAY_OF_WEEK*/) { // Change to ignore
                                                             // TI_DAY_OF_WEEK
                                                             // as TI, and
                                                             // permanently
                                                             // display the
      ...
      /*Main type dtReal34Matrix*/
        ...
        if ((origRegist == REGISTER_X && calcMode != CM_MIM) ||
          ...
          if (lastErrorCode != 0) {
            refreshRegisterLine(errorMessageRegisterLine);
                     ...
                     (regist == REGISTER_X && (temporaryInformation == TI_I ||
                                               temporaryInformation == TI_J))) {
            _displayIJ(regist, prefix, &prefixWidth);
          } else if (temporaryInformation == TI_STORCL &&
                     regist == REGISTER_X) {
            viewStoRcl(prefix, &prefixWidth);
          } else if (temporaryInformation == TI_VIEW_REGISTER &&
                     regist == REGISTER_X) { // X, not T
            userTI(currentViewRegister, regist, prefix, &prefixWidth);
          } else if (temporaryInformation == TI_STATISTIC_SUMS) {
            _displaySigmaPlus(regist, prefix, &prefixWidth, noLine);
          ...
          xcopy(preserveErrorMessage, errorMessage,
                ERROR_MESSAGE_LENGTH); // maintain the errormessage string,
                                       // which is used for TI's earlier.
          if (!vectorToDisplayString(regist,
                                     tmpString)) { //   errorMessage string used
            real34MatrixToDisplayString(regist, tmpString);
          }
          xcopy(errorMessage, preserveErrorMessage,
                ERROR_MESSAGE_LENGTH); // maintain the errormessage string,
                                       // which is used for TI's earlier.
          ...
          if (w > SCREEN_WIDTH - 1) {
            w = stringWidth(tmpString, &standardFont, false, true);
            // Iteration to place ellipsis by eating away the left hand digtis
            // not needed. This will be needed, if the maximum vector digits is
            // increased to more than 9 fixed digits
            showString(tmpString, &standardFont, SCREEN_WIDTH - w - 0 + 2,
                       baseY, vmNormal, false, true);
          } else {
            showString(tmpString, &numericFont, SCREEN_WIDTH - w - 1, baseY,
                       vmNormal, false, true);
          }
        }

        if (temporaryInformation == TI_INACCURATE && regist == REGISTER_X) {
          showString("This result may be inaccurate", &standardFont, 1,
                     Y_POSITION_OF_ERR_LINE, vmNormal, true, true);
        }
      }
      ...
      /*Main type dtComplex34Matrix*/
        ...
        if ((origRegist == REGISTER_X && calcMode != CM_MIM) ||
          ...
          w = stringWidth(tmpString, &numericFont, false, true);
          lineWidth = w;
          showString(tmpString, &numericFont, SCREEN_WIDTH - w - 2, baseY,
                     vmNormal, false, true);
        }
        ...
        sprintf(tmpString, "Displaying %s: to be coded!",
                getRegisterDataTypeName(regist, true, false));
        showString(tmpString, &standardFont,
                   SCREEN_WIDTH -
                       stringWidth(tmpString, &standardFont, false, true),
                   baseY + 6, vmNormal, false, true);
  ...
  if (getRegisterDataType(REGISTER_X) == dtReal34Matrix ||
      getRegisterDataType(REGISTER_X) == dtComplex34Matrix ||
      calcMode == CM_MIM || distModeActive || BASEMODEACTIVE ||
      XXFNMODEACTIVE) {
...
static void _showAngularModeGlyph(angularMode_t angularMode, const font_t *font,
...
void displayNim(const char *nim, const char *lastBase, int16_t wLastBaseNumeric,
                int16_t wLastBaseStandard) {
  int16_t w;
  angularMode_t xangularMode = getRegisterAngularMode(REGISTER_X);
  if (stringWidth(nim, &numericFont, true, true) + wLastBaseNumeric <=
      SCREEN_WIDTH - 16) { // 16 is the numeric font cursor width
    xCursor =
        showString(nim, &numericFont, 0, Y_POSITION_OF_NIM_LINE - checkHPoffset,
                   vmNormal, true, true);
    yCursor = Y_POSITION_OF_NIM_LINE;
    cursorFont = &numericFont;

    if (lastIntegerBase != 0 ||
        (aimBuffer[0] != 0 && aimBuffer[strlen(aimBuffer) - 1] == '/')) {
      showString(lastBase, &numericFont, xCursor + 16,
                 Y_POSITION_OF_NIM_LINE - checkHPoffset, vmNormal, true, true);
    } else if ((getRegisterDataType(REGISTER_X) == dtReal34) &&
               (xangularMode < amNone)) {
      _showAngularModeGlyph(xangularMode, &numericFont, xCursor + 16,
                            Y_POSITION_OF_NIM_LINE - checkHPoffset);
    }
  } else if (stringWidth(nim, &standardFont, true, true) + wLastBaseStandard <=
             SCREEN_WIDTH - 8) { // 8 is the standard font cursor width
    xCursor = showString(nim, &standardFont, 0, Y_POSITION_OF_NIM_LINE + 6,
                         vmNormal, true, true);
    yCursor = Y_POSITION_OF_NIM_LINE + 6;
    cursorFont = &standardFont;
    ...
    if (lastIntegerBase != 0 ||
        (aimBuffer[0] != 0 && aimBuffer[strlen(aimBuffer) - 1] == '/')) {
      showString(lastBase, &standardFont, xCursor + 8,
                 Y_POSITION_OF_NIM_LINE + 6, vmNormal, true, true);
    } else if ((getRegisterDataType(REGISTER_X) == dtReal34) &&
               (xangularMode < amNone)) {
      _showAngularModeGlyph(xangularMode, &standardFont, xCursor + 8,
                            Y_POSITION_OF_NIM_LINE + 6);
    }
  } else {
    char *nimw;
    ...
    nimw = stringAfterPixels(tmpString, &standardFont, SCREEN_WIDTH - 1, true,
                             true);
    w = nimw - tmpString;
    *nimw = 0;
    ...
    if (stringWidth(tmpString + 1500 + w, &standardFont, true, true) +
      ...
      showString(tmpString, &standardFont, 0, Y_POSITION_OF_NIM_LINE - 3,
                 vmNormal, true, true);

      xCursor = showString(tmpString + 1500 + w, &standardFont, 0,
                           Y_POSITION_OF_NIM_LINE + 18, vmNormal, true, true);
      yCursor = Y_POSITION_OF_NIM_LINE + 18;
      cursorFont = &standardFont;
...
void clearTamBuffer(void) {
  if (temporaryInformation == TI_SHOWNOTHING)
    return; // to allow a matrix being dispayed without clearing the tam line
            // through it

  if (shiftF || shiftG) {
    // lcd_fill_rect(18, Y_POSITION_OF_TAM_LINE, 120, 32, LCD_SET_VALUE);
    lcd_fill_rect(funcNameOffset_x, Y_POSITION_OF_TAM_LINE,
                  SCREEN_WIDTH - funcNameOffset_x, 32,
                  LCD_SET_VALUE); // JM Clear the whole t-register instead of
                                  // only 120+18 oixels
  } else {
    // lcd_fill_rect(0, Y_POSITION_OF_TAM_LINE, 138, 32, LCD_SET_VALUE);
    lcd_fill_rect(
        0, Y_POSITION_OF_TAM_LINE, SCREEN_WIDTH, 32,
        LCD_SET_VALUE); // JM Clear the whole t-register instead of 138
  }
}

// conditions where an extra space in T register display is not possible, to
// prevent for the f/g indicator to clash, we reduce the size of the f/g
// indicator
#define useSmallShifts                                                         \
  (isShiftOffset && calcMode == CM_NORMAL &&                                   \
   (((!BASEMODEACTIVE || displayStackSHOIDISP == 0) &&                         \
     getRegisterDataType(REGISTER_T) == dtShortInteger &&                      \
     getRegisterShortIntegerBase(REGISTER_T) < 4) ||                           \
    ((dispBase > 0) && (getRegisterDataType(REGISTER_X) == dtShortInteger ||   \
                        getRegisterDataType(REGISTER_X) == dtLongInteger))))
...
void clearShiftState(void) {
  ...
  lcd_fill_rect(X_SHIFT, Y_SHIFT, (fcol > gcol ? fcol : gcol),
                (frow > grow ? frow : grow), LCD_SET_VALUE);
}

void showShiftStateF(void) {
  showGlyph(displayF, &standardFont, X_SHIFT, Y_SHIFT, vmNormal, true, true,
            false); // f is pixel 4+8+3 wide
}

void showShiftStateG(void) {
  showGlyph(displayG, &standardFont, X_SHIFT, Y_SHIFT, vmNormal, true, true,
            false); // g is pixel 4+10+1 wide
}
...
void displayShiftAndTamBuffer(void) {
  ...
  if (tam.mode || calcMode == CM_ASSIGN) {
    if (calcMode == CM_PEM) { // Variable line to display TAM informations
      ...
      showString(tamBuffer, &standardFont, 75 + 20, tamOverPemYPos, vmNormal,
                 false, false);
    } else { // Fixed line to display TAM informations
      clearTamBuffer();
      showString(tamBuffer, &standardFont, funcNameOffset_x,
                 Y_POSITION_OF_TAM_LINE + 6, vmNormal, true, true);
    }
  }
}
...
void reallyClearStatusBar(uint8_t info) {
  lcd_fill_rect(0, 0, (GRAPHMODE ? SCREEN_WIDTH / 3 : SCREEN_WIDTH),
                Y_POSITION_OF_REGISTER_T_LINE, LCD_SET_VALUE);
...
static void _selectiveClearScreen(void) {
  if (screenUpdatingMode == SCRUPD_AUTO) {
    ...
    if (!(screenUpdatingMode &
          (SCRUPD_MANUAL_STATUSBAR | SCRUPD_SKIP_STATUSBAR_ONE_TIME))) {
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
    ...
    // now clear stack area, first the left graph info area, then the actual
    // area covered by the graph if not in graph mode
    if (!(screenUpdatingMode &
          (SCRUPD_MANUAL_STACK | SCRUPD_SKIP_STACK_ONE_TIME))) {
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
      printf("   >>> _selectiveClearScreen: lcd_fill_rect SCRUPD_MANUAL_STACK "
             "| SCRUPD_SKIP_STACK_ONE_TIME\n");
#endif // PC_BUILD && MONITOR_CLRSCR
      lcd_fill_rect(LeftGraphInfoX, topLeftGraphInfoY, widthGraphInfoBox,
                    heightGraphInfoBox, LCD_SET_VALUE);
      refreshNIMdone = false;
      if (!GRAPHMODE) { // in GRAPHMODE, protect the square graph area
        lcd_fill_rect(widthGraphInfoBox, topLeftGraphInfoY,
                      widthGraphInclBorder, heightGraphInfoBox, LCD_SET_VALUE);
      }
    }

    // now clear menu area, first the left graph info area, then the actual area
    // covered by the graph if not in graph mode
    if (!(screenUpdatingMode &
          (SCRUPD_MANUAL_MENU | SCRUPD_SKIP_MENU_ONE_TIME))) {
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
      printf("   >>> _selectiveClearScreen: lcd_fill_rect SCRUPD_MANUAL_MENU | "
             "SCRUPD_SKIP_MENU_ONE_TIME\n");
#endif // PC_BUILD && MONITOR_CLRSCR
      lcd_fill_rect(LeftGraphInfoX, topLeftMenuInclBorderY, widthGraphInfoBox,
                    menuHeightInclBorder, LCD_SET_VALUE);
      if (!GRAPHMODE ||
          menu(0) == -MNU_PLOT_FUNC) { // not in GRAPHMODE, clear the little
                                       // triangle area indicating more menus
        lcd_fill_rect(LeftGraphInfoX, topLeftMenuInclBorderY - 3, 20, 6,
                      LCD_SET_VALUE);
      }
      if (!GRAPHMODE) { // in GRAPHMODE, protect the square graph area
        lcd_fill_rect(widthGraphInfoBox, topLeftMenuInclBorderY,
                      widthGraphInclBorder, menuHeightInclBorder,
                      LCD_SET_VALUE);
      }
    }
  }
...
// #if !defined(TESTSUITE_BUILD)
//   void clearScreenOld(bool_t clearStatusBar, bool_t clearRegisterLines,
//   bool_t clearSoftkeys) {      //JMOLD
//     if(clearStatusBar) {
...
#if !defined(TESTSUITE_BUILD) // clearScreenOld(clrStatusBar, clrRegisterLines,
                              // clrSoftkeys);
void clearScreenOld(
    bool_t clearStatusBar, bool_t clearRegisterLines,
    bool_t clearSoftkeys) { // clrStatusBar, clrRegisterLines, clrSoftkeys
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
  printf("       clearScreenOld calcMode=%u clearStatusBar=%u, "
         "clearRegisterLines=%u, clearSoftkeys=%u\n",
...
void clearScreenGraphs(uint8_t source, bool_t clearTextArea,
                       bool_t clearGraphArea) {
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
...
static void _refreshPemScreen(void) {
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
  printf(">>> BEGIN _refreshPemScreen calcMode=%d previousCalcMode=%d "
         "screenUpdatingMode=%d skippedStackLines=%u\n",
         calcMode, previousCalcMode, screenUpdatingMode,
         skippedStackLines); // JMYY
#endif                       // PC_BUILD &&MONITOR_CLRSCR
  skippedStackLines = false; // See timer.c skippedStackLines
#if defined(DMCP_BUILD)
  keyBuffer_pop(); // This causes key updates while the longer time processing
                   // register updates happen
  if (!runningOnSimOrUSB && // Automatically, when on battery (hence low
                            // processor), change to skip long processing
                            // register printing, recovering the fragmented
                            // screen here: See timer.c fnTimerEndOfActivity()
      !emptyKeyBuffer() && key_empty() == 1) {
  ...
  if (!runningOnSimOrUSB) {
    ...
    if (doRefreshSoftMenu ||
        !(screenUpdatingMode &
          (SCRUPD_MANUAL_MENU |
           SCRUPD_SKIP_MENU_ONE_TIME))) { // battery powered
      clearScreenOld(!clrStatusBar, !clrRegisterLines,
                     clrSoftkeys); // battery powered
...
#define TEST_BATTERY_POWERED_SIMULATION
  ...
  if (doRefreshSoftMenu ||
      !(screenUpdatingMode &
        (SCRUPD_MANUAL_MENU |
         SCRUPD_SKIP_MENU_ONE_TIME))) { // this tests the battery powered option
                                        // on sim
    clearScreenOld(!clrStatusBar, !clrRegisterLines,
                   clrSoftkeys); // this tests the battery powered option on sim
  ...
  if (!(screenUpdatingMode &
        SCRUPD_MANUAL_STATUSBAR)) { // this tests the battery powered option on
                                    // sim
    clearScreenOld(
        clrStatusBar, !clrRegisterLines,
        !clrSoftkeys);  // this tests the battery powered option on sim
...
static void _refreshNormalScreen(void) {
#if defined(PC_BUILD) && defined(ANALYSE_REFRESH)
  printf(">>> BEGIN _refreshNormalScreen calcMode=%d previousCalcMode=%d "
         "screenUpdatingMode=%d\n",
  ...
  if (calcMode == CM_CONFIRMATION) {
    ...
    screenUpdatingMode =
        (aimBuffer[0] == 0)
            ? SCRUPD_AUTO
            : (SCRUPD_MANUAL_STACK | SCRUPD_MANUAL_SHIFT_STATUS);
    screenUpdatingMode |= SCRUPD_SKIP_STATUSBAR_ONE_TIME;
  } else if (calcMode == CM_TIMER) {
    screenUpdatingMode =
        SCRUPD_AUTO; // SCRUPD_MANUAL_STACK | SCRUPD_MANUAL_SHIFT_STATUS;
  ...
  // \n",screenUpdatingMode & SCRUPD_MANUAL_STACK, screenUpdatingMode &
  // SCRUPD_SKIP_STACK_ONE_TIME);

  // The ordering of the 4 lines below is important for SHOW
  // (temporaryInformation == TI_SHOW_REGISTER)
  if ((calcMode != CM_NIM || (skippedStackLines && calcMode == CM_NIM)) &&
      !(screenUpdatingMode &
        (SCRUPD_MANUAL_STACK | SCRUPD_SKIP_STACK_ONE_TIME))) {
    if (calcMode != CM_AIM) {
      if (calcMode != CM_TIMER && !tam.alpha &&
          temporaryInformation != TI_VIEW_REGISTER) {
        refreshRegisterLine(REGISTER_T);
      }
      ...
      if (SHOWMODE) {
        screenUpdatingMode |=
            SCRUPD_MANUAL_MENU; // done with clearing and printing over the menu
                                // area, now protecting the menu area
      }
    } else {
...
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
    printf(">>>>      _refreshNormalScreen NIM_LINE: calcMode=%u  "
           "programRunStop=%d lastErrorCode=%u screenUpdatingMode=%u\n",
  ...
  if (currentSolverStatus & SOLVER_STATUS_INTERACTIVE) {
    ...
    if (!mvarMenu) {
      if (currentSolverStatus & SOLVER_STATUS_USES_FORMULA) {
        if ((currentSolverStatus & SOLVER_STATUS_EQUATION_MODE) ==
            SOLVER_STATUS_EQUATION_INTEGRATE) {
          showSoftmenu(-MNU_Sf);
        } else {
  ...
  if (!(screenUpdatingMode &
    ...
    showSoftmenuCurrentPart();
#if defined(DMCP_BUILD)
    lcd_refresh_dma(); // If this is not here, menu generation is not reliable,
                       // and presses are missed. Not sure why.
#endif                 // DMCP_BUILD
  }
  ...
  for (int y = Y_POSITION_OF_REGISTER_Y_LINE;
       y < Y_POSITION_OF_REGISTER_Y_LINE + 2 * REGISTER_LINE_HEIGHT; y++) {
...
#if defined(PC_BUILD) && defined(MONITOR_CLRSCR)
  printf(">>> END of _refreshNormalScreen calcMode=%d previousCalcMode=%d "
         "screenUpdatingMode=%d\n",
         calcMode, previousCalcMode, screenUpdatingMode); // JMYY
#endif // PC_BUILD &&MONITOR_CLRSCR
}
...
void refreshScreen(uint16_t source) {
  ...
  if (calcMode != CM_AIM && calcMode != CM_NIM && calcMode != CM_PLOT_STAT &&
      calcMode != CM_GRAPH && calcMode != CM_LISTXY &&
      last_CM != 240) { // 240 specifically to prefent this
    last_CM = 254;      // JM Force NON-CM_AIM and NON-CM_NIM to refresh to be
                        // compatible to 43S
  } else {
    if (last_CM == 240) {
      last_CM = calcMode;
  ...
  convertUInt64ToShortIntegerRegister(0, screenUpdatingMode, 2,
                                      TEMP_REGISTER_1);
  ...
  if (screenUpdatingMode == 0)
    ...
    if ((screenUpdatingMode & 0x40))
      strcat(ttt, "SkpMEN ");
    if ((screenUpdatingMode & 0x20))
      strcat(ttt, "SkpSTK ");

    if (!(screenUpdatingMode & 0x08))
      strcat(ttt, "SHFT ");
    if (!(screenUpdatingMode & 0x04))
      strcat(ttt, "MENU ");
    if (!(screenUpdatingMode & 0x02))
      strcat(ttt, "STK ");
    if (!(screenUpdatingMode & 0x01))
      strcat(ttt, "STS ");
  }
  int16_t m = softmenuStack[0].softmenuId;
  char uuu[100];
  stringToASCII(indexOfItems[currentMenu() > 0 ? currentMenu() : -currentMenu()]
                    .itemSoftmenuName,
                uuu);

  printf("   refrsh(%5u): Cnt=%3d %s CM=%2d scr..upd:%3d=%12s=>%26s TI=%4u "
         "CL=%s tam:%5i MENUid=%2d:%4i:%s\n",
         source, refreshScreenCounter++, (last_CM != calcMode) ? "OVR" : "   ",
         calcMode, screenUpdatingMode, sss, ttt, temporaryInformation,
         (alphaCase == AC_LOWER) ? "LO" : "UP", tam.mode, m, currentMenu(),
         uuu);
  fflush(stdout);
#endif // PC_BUILD
  ...
  case CM_TIMER:
    ...
    // printf("screenUpdatingMode4=%u calcmode=%u
    // last_CM=%u\n",screenUpdatingMode, calcMode, last_CM);
  ...
  case CM_PLOT_STAT:
    ...
    if (lastErrorCode != ERROR_NONE) {
      if (currentMenu() == -MNU_HPLOT || currentMenu() == -MNU_PLOT_ASSESS ||
          currentMenu() == -MNU_HPLOT || currentMenu() == -MNU_PLOT_SCATR) {
        popSoftmenu();
        calcMode = CM_NORMAL;
        refreshScreen(84);
      }
...
void fnSNAP(uint16_t unusedButMandatoryParameter) {
  ...
  printf("fnSNAP!\n");
#endif
  resetShiftState(); // JM To avoid f or g top left of the screen, clear to make
                     // sure
  refreshScreen(80);

#if defined(PC_BUILD) // added the xcopy commands needed for hardware, to better
                      // duplicate the hardware standardScreenDump
  xcopy(tmpString, errorMessage,
        ERROR_MESSAGE_LENGTH + AIM_BUFFER_LENGTH + NIM_BUFFER_LENGTH +
            TAM_BUFFER_LENGTH); // backup portion of the "message buffer" area
                                // in DMCP used by ERROR..AIM..NIM buffers, to
                                // the tmpstring area in DMCP. DMCP uses this
                                // area during create_screenshot.
  fnScreenDump(0);
  xcopy(errorMessage, tmpString,
        ERROR_MESSAGE_LENGTH + AIM_BUFFER_LENGTH + NIM_BUFFER_LENGTH +
            TAM_BUFFER_LENGTH); //   This total area must be less than the
                                //   tmpString storage area, which it is.
#elif defined(DMCP_BUILD)
  standardScreenDump();
#endif
  ...
  xcopy(ss, tamBuffer,
        TAM_BUFFER_LENGTH); // Backup the TamBuffer, in case we are in a TAM
                            // screen when doing screenshot
  if (calcMode == CM_AIM) {
...
void fnScreenDump(uint16_t unusedButMandatoryParameter) {
  ...
  strftime(bmpFileName, sizeof(bmpFileName), "SNAP_%Y%m%d_%H%M%S.bmp",
           timeInfo);
  ...
  uint32 = 0x00000082;
  fwrite(&uint32, 1, 4,
         bmp); // Offset 0x0a 10  Offset where the bitmap data can be found
  ...
  uint32 = 0x000030c0;
  fwrite(&uint32, 1, 4,
         bmp); // Offset 0x22 34  Size of bitmap data (including padding)
...
#if !defined(TESTSUITE_BUILD)
static int32_t _getPositionFromRegister(calcRegister_t regist,
                                        int16_t maxValuePlusOne) {
  int32_t value;
  ...
  if (getRegisterDataType(regist) == dtReal34) {
    ...
    if (real34CompareLessThan(REGISTER_REAL34_DATA(regist), const34_0) ||
        real34CompareLessEqual(&maxValue34, REGISTER_REAL34_DATA(regist))) {
      ...
      moreInfoOnError("In function _getPositionFromRegister:", tmpString,
                      "this value is negative or too big!", NULL);
#endif // PC_BUILD
      return -1;
    }
    ...
    convertLongIntegerRegisterToLongInteger(regist, lgInt);
    if (longIntegerCompareUInt(lgInt, 0) < 0 ||
        longIntegerCompareUInt(lgInt, maxValuePlusOne) >= 0) {
      displayCalcErrorMessage(ERROR_OUT_OF_RANGE, ERR_REGISTER_LINE,
                              REGISTER_X);
#if defined(PC_BUILD)
    ...
    displayCalcErrorMessage(ERROR_INVALID_DATA_TYPE_FOR_OP, ERR_REGISTER_LINE,
                            REGISTER_X);
#if defined(PC_BUILD)
    sprintf(errorMessage, "register %" PRId16 " is %s:", regist,
            getRegisterDataTypeName(regist, true, false));
    moreInfoOnError("In function _getPositionFromRegister:", errorMessage,
                    "not suited for addressing!", NULL);
#endif // PC_BUILD
    return -1;
  }
...
void fnClLcd(uint16_t unusedButMandatoryParameter) {
  ...
  getPixelPos(&x, &y);
  if (lastErrorCode == ERROR_NONE) {
    screenUpdatingMode |= SCRUPD_MANUAL_STATUSBAR | SCRUPD_MANUAL_STACK |
                          SCRUPD_MANUAL_MENU | SCRUPD_MANUAL_SHIFT_STATUS;
...
void fnAGraph(uint16_t regist) {
  ...
  if (lastErrorCode == ERROR_NONE) {
    if (getRegisterDataType(regist) == dtShortInteger) {
      ...
      screenUpdatingMode |=
          SCRUPD_MANUAL_STACK | SCRUPD_MANUAL_MENU | SCRUPD_MANUAL_SHIFT_STATUS;
      if ((SCREEN_HEIGHT - y - 1 - (int)shortIntegerWordSize) <=
          Y_POSITION_OF_REGISTER_T_LINE) {
        screenUpdatingMode |= SCRUPD_MANUAL_STATUSBAR;
      }
      ...
      for (uint32_t i = 0; i < shortIntegerWordSize; ++i) {
        ...
        case 1:
          if (!(val & 1))
            setWhitePixel(x, SCREEN_HEIGHT - y - 1 - i); /* fallthrough */
        case 0:
          if (val & 1)
      ...
      sprintf(errorMessage, "register %" PRId16 " is %s:", regist,
              getRegisterDataTypeName(regist, true, false));
      moreInfoOnError("In function fnAGraph:", errorMessage,
                      "not suited for addressing!", NULL);
#endif // PC_BUILD
    }
  }
