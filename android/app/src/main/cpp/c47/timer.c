...
void fnTimerEndOfActivity(uint16_t param) {
  ...
  if (skippedStackLines) { // update screen after 1 or 2 sec timout, to restore
                           // the half-updated screen in battery mode. See
                           // refreshRegisterLine() in screen.c
    screenUpdatingMode = SCRUPD_AUTO;
    skippedStackLines = false;
    refreshScreen(32);
    // if(skippedStackLines) {
    //   print_linestr("more skippedStackLines #######", false);
    //   fnTimerStart(TO_KB_ACTV, TO_KB_ACTV, skippedStackLines ?
    //   TO_KB_ACTV_MEDIUM/5 : TO_KB_ACTV_SHORT);
    // }
  }
#if defined(DM42_POWERMARKS)
...
#endif                                                    // DMCP_BUILD
#if defined(PC_BUILD)
    timer[nr].timer_will_expire =
        (gint64)(now + time * 1000); // time * 1000 is in us
...
static uint32_t _currentTime(void) {
  ...
  return (uint32_t)timeInfo.hour * 3600000u + (uint32_t)timeInfo.min * 60000u +
         (uint32_t)timeInfo.sec * 1000u + (uint32_t)timeInfo.csec * 10u;
#else  // !DMCP_BUILD
...
static void _realToUInt32(const real_t *re, enum rounding mode,
                          uint32_t *value32, bool_t *overflow) {
  uint8_t bcd[76], sign;
  real_t real;
  longInteger_t lgInt;
...
#else  // 64 bit
  *value32 =
      (lgInt->_mp_size == 0 ? 0 : lgInt->_mp_d[0] & 0x00000000ffffffffULL);
...
bool_t inputHelper(uint16_t regist, uint32_t *val, bool_t *overflow) {
  ...
  default: {
    ...
    sprintf(errorMessage, "cannot recall %s to the stopwatch",
...
void fnShowTimerApp(void) {
  ...
  if (calcMode == CM_TIMER) {
    ...
    if (remainingMsecCountdown > 0) {
      remainingMsec = (int64_t)remainingMsecCountdown -
                      (int64_t)msec; // allow it to run negative to be sure to
                                     // catch it has run out, hence the int64
    ...
    if (timerTotalTime > 0) {
      ...
      if (timerCraAndDeciseconds & 0x80u) {
        sprintf(tmpString,
                "%2" PRIu32 ":%02" PRIu32 ":%02" PRIu32
                ".%" PRIu32 STD_SUP_BOLD_T "  ",
                tmsec / 3600000u, tmsec % 3600000u / 60000u,
                tmsec % 60000u / 1000u, tmsec % 1000u / 100u);
      } else {
        sprintf(tmpString,
                "%2" PRIu32 ":%02" PRIu32 ":%02" PRIu32 STD_SUP_BOLD_T
                    STD_SPACE_PUNCTUATION STD_SPACE_FIGURE "  ",
                tmsec / 3600000u, tmsec % 3600000u / 60000u,
                tmsec % 60000u / 1000u);
      }
    }
    ...
    if (timerCraAndDeciseconds & 0x80u) {
      sprintf(tmpString + stringByteLength(tmpString),
              "%2" PRIu32 ":%02" PRIu32 ":%02" PRIu32 ".%" PRIu32 " ",
              msec / 3600000u, msec % 3600000u / 60000u, msec % 60000u / 1000u,
              msec % 1000u / 100u);
    } else {
      sprintf(tmpString + stringByteLength(tmpString),
              "%2" PRIu32 ":%02" PRIu32
              ":%02" PRIu32 STD_SPACE_PUNCTUATION STD_SPACE_FIGURE " ",
              msec / 3600000u, msec % 3600000u / 60000u, msec % 60000u / 1000u);
    }

    if (rbr1stDigit) {
      sprintf(tmpString + stringByteLength(tmpString), "[%02" PRIu32 "]",
              (uint32_t)(timerCraAndDeciseconds & 0x7fu));
    } else if (aimBuffer[AIM_BUFFER_LENGTH / 2] == 0) {
      sprintf(tmpString + stringByteLength(tmpString),
              "[" STD_CURSOR STD_SPACE_FIGURE "]");
    } else {
      sprintf(tmpString + stringByteLength(tmpString),
              "[%" PRId32 STD_CURSOR "]",
              (int32_t)(aimBuffer[AIM_BUFFER_LENGTH / 2] - '0'));
    }
    showString(tmpString, &numericFont, 1, Y_POSITION_OF_REGISTER_T_LINE,
               vmNormal, true, true);
    ...
    if (remainingMsecCountdown > 0) {
      if (remainingMsec > 0) {
        ...
        if (timerCraAndDeciseconds & 0x80u) {
          sprintf(tmpString,
                  "%2" PRIu32 ":%02" PRIu32 ":%02" PRIu32
                  ".%" PRIu32 STD_HOURGLASS_WH "  ",
                  (uint32_t)remainingMsec / 3600000u,
                  (uint32_t)remainingMsec % 3600000u / 60000u,
                  (uint32_t)remainingMsec % 60000u / 1000u,
                  (uint32_t)remainingMsec % 1000u / 100u);
        } else {
          sprintf(tmpString,
                  "%2" PRIu32 ":%02" PRIu32 ":%02" PRIu32 STD_HOURGLASS_WH
                      STD_SPACE_PUNCTUATION STD_SPACE_FIGURE "  ",
                  (uint32_t)remainingMsec / 3600000u,
                  (uint32_t)remainingMsec % 3600000u / 60000u,
                  (uint32_t)remainingMsec % 60000u / 1000u);
        }
        showString(tmpString, &numericFont, 1, Y_POSITION_OF_REGISTER_Z_LINE,
                   vmNormal, true, true);
...
void fnRegAddTimerApp(uint16_t unusedButMandatoryParameter) { // ENTER
  ...
  if (rbr1stDigit) {
    ...
    timerCraAndDeciseconds = (timerCraAndDeciseconds & 0x80u) +
                             (uint8_t)(aimBuffer[AIM_BUFFER_LENGTH / 2] - '0');
    rbr1stDigit = true;
  }
#endif // !TESTSUITE_BUILD
}
...
void fnAddTimerApp(uint16_t unusedButMandatoryParameter) { // Send TIM to STATS
  ...
  real34Add(REGISTER_REAL34_DATA(REGISTER_X), const34_1,
            REGISTER_REAL34_DATA(REGISTER_X));
  liftStack();
...
void fnDigitKeyTimerApp(uint16_t digit) {
  ...
  if (rbr1stDigit || aimBuffer[AIM_BUFFER_LENGTH / 2] == 0) {
    ...
    timerCraAndDeciseconds =
        (timerCraAndDeciseconds & 0x80u) +
        (uint8_t)(aimBuffer[AIM_BUFFER_LENGTH / 2] - '0') * 10u + digit;
    rbr1stDigit = true;
  }
#endif // !TESTSUITE_BUILD
}
...
#if (EXTRA_INFO_ON_CALC_ERROR == 1)
    sprintf(errorMessage, "the %s does not fit to uint32_t",
            getRegisterDataTypeName(regist, true, false));
