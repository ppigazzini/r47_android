...
static int register_main_activity_natives(JNIEnv *env);
extern void JNICALL Java_com_example_r47_MainActivity_setLcdColors(JNIEnv *env,
                                                                   jobject thiz,
                                                                   jint text,
                                                                   jint bg);
...
void yieldToAndroidWithMs(int ms) {
  ...
  // 2. Perform housekeeping while unlocked
  // Process any pending Java tasks (like the EXIT button)
  // while we are unlocked, so that sendKey can grab the mutex if it needs to.
  processCoreTasksNative();

  // Sleep to ensure UI thread gets a chance to grab the mutex
  if (ms > 0) {
    usleep(ms * 1000);
  } else {
...
void yieldToAndroid() { yieldToAndroidWithMs(1); }
...
Java_com_example_r47_MainActivity_loadStateNative(JNIEnv *env, jobject thiz) {
  ...
  if (ram) {
    ...
    extern void defineCurrentProgramFromGlobalStepNumber(
        int16_t globalStepNumber);
...
JNIEXPORT void JNICALL Java_com_example_r47_MainActivity_getDisplayPixels(
  ...
  if (pthread_mutex_trylock(&screenMutex) != 0) {
    // Core is busy (possibly in a PAUSE loop), skip this frame to keep UI
    // responsive
    return;
  }
