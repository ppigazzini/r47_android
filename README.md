...
## 🎯 Features
- **Scene-Driven Android Keypad**: `native` renders all 43 keys, including the stateful F1-F6 softkeys, from one native snapshot instead of relying on per-label JNI polling. `r47_background` reuses that same snapshot for keypad labels and softkey text without repainting the key bodies baked into the background asset.
