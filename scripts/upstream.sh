...
usage() {
    cat <<'EOF'
Usage:
    scripts/upstream.sh resolve [--auto|--locked|--latest] [--write-lock] [--format shell|none] [--url <url>] [--ref <ref>] [--commit <sha>]
    scripts/upstream.sh sync [--auto|--locked|--latest] [--write-lock] [--if-missing] [--force] [--url <url>] [--ref <ref>] [--commit <sha>]
...
write_lockfile() {
    ...
    cat > "$temp_path" <<EOF
# Generated by scripts/upstream.sh.
# Git ignores this file on purpose.
# Remove upstream_commit or rerun scripts/upstream.sh resolve --latest --write-lock to follow the newest upstream revision.
